...
#define MMTF_SPEC_VERSION_MINOR 1
	 ...
	 */
	struct GroupType
	{ // NOTE: can't use MSGPACK_DEFINE_MAP due to char
		...
		{
			return (formalChargeList == c.formalChargeList
	...
	struct Entity
	{
		...
		{
			return (chainIndexList == c.chainIndexList
	...
	struct BioAssembly
	{
		...
		{
			return (transformList == c.transformList && name == c.name);
		}
	 ...
	 * constructor and can be used to check if value was never set (only
	 * relevant for optional values):
	...
	struct StructureData
	{
		 ...
		 * @return True if all required fields are set and vector sizes and
		 * indices are consistent.
		 */
		bool hasConsistentData(bool verbose = false,
							   uint32_t chain_name_max_length = 4) const;

		/**
		 * @brief Read out the contents of mmtf::StructureData in a PDB-like
		 * fashion Columns are in order: ATOM/HETATM AtomId Element AtomName
		 * AltLoc GroupId GroupType InsCode ChainName x y z B-factor Occupancy
		 * Charge
	 ...
	 *
	 * @warning Use the other is_hetatm function whenever possible. This one
	 * should only be used if you don't have info like an 'entity_list' to work
	 ...
	 *
	 * Used in StructureData.print to mark atoms as HETATM. This is a shorthand
	 * for `is_hetatm(type) || !is_polymer(chain_index, entity_list)`.
	 *
	 * Efficient code needing this information should preferably use the or
	 * statement above, precompute the `is_polymer` output for each chain and
	 * the `is_hetatm(type)` output for each group instead of calling this on
	 * each atom.
	 *
	...
	namespace
	{
		...
		// -> either default or "YYYY-MM-DD" (only string format checked, not
		// date)
		...
		bool hasValidIndices(const std::vector<T> &v, Tnum num)
		{
			if (v.empty())
				...
				return hasValidIndices(&v[0], v.size(), num);
		}
					...
					const std::vector<Entity> &entity_list)
	{
		for (std::size_t i = 0; i < entity_list.size(); ++i)
		{
			if (std::find(entity_list[i].chainIndexList.begin(),
						  entity_list[i].chainIndexList.end(), chain_index)
	...
	bool is_hetatm(const char *type)
	{
		const char *hetatm_type[] = {
			...
			"D-PEPTIDE LINKING", "D-SACCHARIDE",
			"D-SACCHARIDE 1,4 AND 1,4 LINKING",
			"D-SACCHARIDE 1,4 AND 1,6 LINKING", "DNA OH 3 PRIME TERMINUS",
			"DNA OH 5 PRIME TERMINUS", "DNA LINKING", "L-DNA LINKING",
			"L-RNA LINKING", "L-BETA-PEPTIDE, C-GAMMA LINKING",
			"L-GAMMA-PEPTIDE, C-DELTA LINKING",
			"L-PEPTIDE COOH CARBOXY TERMINUS", "L-PEPTIDE NH3 AMINO TERMINUS",
			//"L-PEPTIDE LINKING", // All canonical L AA
			"L-SACCHARIDE", "L-SACCHARIDE 1,4 AND 1,4 LINKING",
			"L-SACCHARIDE 1,4 AND 1,6 LINKING", "RNA OH 3 PRIME TERMINUS",
			"RNA OH 5 PRIME TERMINUS", "RNA LINKING", "NON-POLYMER", "OTHER",
			//"PEPTIDE LINKING", // GLY
			"PEPTIDE-LIKE", "SACCHARIDE", 0
		};
	...
	{
		return (
			mmtfVersion == c.mmtfVersion && mmtfProducer == c.mmtfProducer
			&& unitCell == c.unitCell && spaceGroup == c.spaceGroup
			&& structureId == c.structureId && title == c.title
			...
			&& resolution == c.resolution && rFree == c.rFree
			&& rWork == c.rWork && numBonds == c.numBonds
			&& numAtoms == c.numAtoms && numGroups == c.numGroups
			&& numChains == c.numChains && numModels == c.numModels
			&& groupList == c.groupList && bondAtomList == c.bondAtomList
			&& bondOrderList == c.bondOrderList && xCoordList == c.xCoordList
			&& yCoordList == c.yCoordList && zCoordList == c.zCoordList
			&& bFactorList == c.bFactorList && atomIdList == c.atomIdList
			&& altLocList == c.altLocList && occupancyList == c.occupancyList
			&& groupIdList == c.groupIdList && groupTypeList == c.groupTypeList
			&& secStructList == c.secStructList && insCodeList == c.insCodeList
			&& sequenceIndexList == c.sequenceIndexList
			&& chainIdList == c.chainIdList && chainNameList == c.chainNameList
	...
	inline bool StructureData::hasConsistentData(
		bool verbose, uint32_t chain_name_max_length) const
	{
		...
		if (!hasRightSizeOptional(unitCell, 6))
		{
			if (verbose)
			{
				std::cout << "inconsistent unitCell (unitCell length != 6)"
						  << std::endl;
			}
			return false;
		}
		...
		if (!isValidDateFormatOptional(depositionDate))
		{
			if (verbose)
			{
				std::cout << "inconsistent depositionDate (does not match "
							 "'YYYY-MM-DD' "
							 "or empty)"
						  << std::endl;
			}
		...
		for (size_t i = 0; i < ncsOperatorList.size(); ++i)
		{
			if ((int)ncsOperatorList[i].size() != 16)
			{
				if (verbose)
				{
					std::cout << "inconsistent ncsOperatorList idx: " << i
							  << " found size: " << ncsOperatorList[i].size()
							  << " != 16" << std::endl;
				}
				return false;
			}
		...
		for (size_t i = 0; i < groupList.size(); ++i)
		{
			...
			if (g.atomNameList.size() != num_atoms)
			{
				...
				{
					std::cout
						<< "inconsistent group::atomNameList size at idx: " << i
						<< std::endl;
				}
				return false;
			}
			if (g.elementList.size() != num_atoms)
			{
				...
				{
					std::cout
						<< "inconsistent group::elementList size at idx: " << i
						<< std::endl;
				}
				return false;
			}
			if (!isDefaultValue(g.bondOrderList))
			{
				...
				{
					if (std::find(allowed_bond_orders.begin(),
								  allowed_bond_orders.end(), g.bondOrderList[j])
						== allowed_bond_orders.end())
					{
						if (verbose)
						{
							std::cout << "Cannot have bond order of: "
									  << (int)g.bondOrderList[j]
									  << " allowed bond orders are: -1, 1, 2, "
										 "3 or 4.  at idx: "
									  << j << std::endl;
						}
						return false;
			...
			{
				if (isDefaultValue(g.bondOrderList)
					|| isDefaultValue(g.bondAtomList))
				{
					if (verbose)
					{
						std::cout
							<< "Cannot have bondResonanceList without both "
							<< "bondOrderList and bondAtomList! at idx: " << i
							<< std::endl;
					}
					return false;
				}
				if (g.bondOrderList.size() != g.bondResonanceList.size())
				{
					if (verbose)
					{
						std::cout << "inconsistent group::bondOrderSize size: "
								  << g.bondOrderList.size()
								  << " != group::bondResonanceList size: "
								  << g.bondResonanceList.size()
								  << " at idx: " << i << std::endl;
					}
					return false;
				}
				...
				{
					if (g.bondResonanceList[j] < -1
						|| g.bondResonanceList[j] > 1)
					{
						if (verbose)
						{
							std::cout << "group::bondResonanceList had a "
										 "Resonance of: "
									  << (int)g.bondResonanceList[j]
									  << " and only -1, 0, or 1 are allowed"
									  << std::endl;
						}
						return false;
					}
					if (g.bondOrderList[j] == -1 && g.bondResonanceList[j] != 1)
					{
						...
						{
							std::cout
								<< "group::bondResonanceList had a Resonance "
								   "of: "
								<< (int)g.bondResonanceList[j]
								<< " and group::bondOrderList had an order of "
								<< (int)g.bondOrderList[j]
								<< " we require unknown bondOrders to have "
								   "resonance"
								<< std::endl;
						}
						return false;
			...
			if (!hasValidIndices(g.bondAtomList, num_atoms))
			{
				if (verbose)
				{
					std::cout << "inconsistent group::bondAtomList indices "
								 "(not all in [0, "
							  << num_atoms - 1 << "]) at idx: " << i
							  << std::endl;
				}
				return false;
			}
		...
		if (!isDefaultValue(bondOrderList))
		{
			...
			{
				if (std::find(allowed_bond_orders.begin(),
							  allowed_bond_orders.end(), bondOrderList[i])
					== allowed_bond_orders.end())
				{
					if (verbose)
					{
						std::cout << "Cannot have bond order of: "
								  << (int)bondOrderList[i]
								  << " allowed bond orders are: -1, 1, 2, 3 or "
									 "4.  at idx: "
								  << i << std::endl;
					}
					return false;
			...
			for (size_t i = 0; i < bondResonanceList.size(); ++i)
			{
				...
				if (bondOrderList[i] == -1 && bondResonanceList[i] != 1)
				{
					if (verbose)
					{
						std::cout << "bondResonanceList had a Resonance of: "
								  << (int)bondResonanceList[i]
								  << " and bondOrderList had an order of "
								  << (int)bondOrderList[i]
								  << " we require unknown bondOrders to have "
									 "resonance"
								  << std::endl;
		...
		for (size_t i = 0; i < entityList.size(); ++i)
		{
			...
			{
				sequenceIndexSize[ent.chainIndexList[j]]
					= ent.sequence.length();
			}
		}
			...
			for (int j = 0; j < chainsPerModel[model_idx]; ++j, ++chain_idx)
			{
				...
				if (chainIdList[chain_idx].size() > chain_name_max_length)
				{
					if (verbose)
					{
						std::cout
							...
							<< chain_idx
							<< " size: " << chainIdList[chain_idx].size()
							<< std::endl;
					}
					return false;
				}
					...
					&& chainNameList[chain_idx].size() > chain_name_max_length)
				{
					if (verbose)
					{
						std::cout
							...
							<< chain_idx
							<< " size: " << chainNameList[chain_idx].size()
							<< std::endl;
					}
					return false;
				}
				...
				for (int k = 0; k < groupsPerChain[chain_idx]; ++k, ++group_idx)
				{
					...
					const GroupType &group
						= groupList[groupTypeList[group_idx]];
		...
		if (!all_bond_orderLists_are_default)
		{
			if (bond_count_from_order != numBonds)
			{
				if (verbose)
				{
					std::cout << "inconsistent numBonds vs bond order count"
							  << std::endl;
				}
				return false;
			}
		...
		if (!all_bond_resonanceLists_are_default)
		{
			if (bond_count_from_resonance != numBonds)
			{
				if (verbose)
				{
					std::cout << "inconsistent numBonds vs bond resonance count"
							  << std::endl;
				}
				return false;
			}
		...
		if (!all_bond_atomLists_are_default)
		{
			if (bond_count_from_atom != numBonds)
			{
				if (verbose)
				{
					std::cout << "inconsistent numBonds vs bond atom list count"
							  << std::endl;
				}
				return false;
			}
	...
	inline std::string StructureData::print(std::string delim)
	{
		...
		for (int i = 0; i < numModels; i++, modelIndex++)
		{
			...
			{
				// traverse groups
				for (int k = 0; k < groupsPerChain[chainIndex];
					 k++, groupIndex++)
				{
					...
					for (int l = 0; l < groupAtomCount; l++, atomIndex++)
					{
						...
						if (!mmtf::isDefaultValue(altLocList))
						{
							if (altLocList[atomIndex] == ' '
								...
								out << altLocList[atomIndex] << delim;
						}
						else
							out << "." << delim;
						// Group name
						...
						if (!mmtf::isDefaultValue(insCodeList))
						{
							if (insCodeList[groupIndex] == ' '
								...
								out << int(insCodeList[groupIndex]) << delim;
						}
						else
							out << ". ";
						// x, y, z
