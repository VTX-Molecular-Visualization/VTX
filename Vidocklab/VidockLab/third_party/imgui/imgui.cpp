...
#	pragma clang diagnostic ignored \
		"-Wunknown-pragmas" // warning : unknown warning group
							// '-Wformat-pedantic *'        // not all warnings
							// are known by all clang versions.. so ignoring
...
#	pragma clang diagnostic ignored \
		"-Wexit-time-destructors" // warning : declaration requires an exit-time
								  ...
								  // users code that has been disabled before
								  // exit it might cause problems. ImGui coding
...
namespace ImGui
{
	static bool BeginChildEx(const char *name, ImGuiID id,
...
ImGuiStyle::ImGuiStyle()
{
	...
	PopupRounding = 0.0f; // Radius of popup window corners rounding. Set to
						  // 0.0f to have rectangular child windows
	PopupBorderSize
		= 1.0f; // Thickness of border around popup or tooltip windows.
				// Generally set to 0.0f or 1.0f. Other values not well tested.
	FramePadding = ImVec2(
...
bool ImGuiListClipper::Step()
{
	...
	if (StepNo
		== 2) // Step 2: dummy step only required if an explicit items_height
			  // was passed to constructor or Begin() and user still call
							   ...
							   ImGuiNavHighlightFlags flags)
{
	...
	if (flags & ImGuiNavHighlightFlags_TypeDefault)
	{
		...
		window->DrawList->AddRect(
			...
			GetColorU32(ImGuiCol_NavHighlight), rounding, ImDrawCornerFlags_All,
			THICKNESS);
		if (!fully_visible)
								  ...
								  bool tab_stop)
{
	...
	const bool is_tab_stop
		= (window->DC.ItemFlags
		   & (ImGuiItemFlags_NoTabStop | ImGuiItemFlags_Disabled))
		  == 0;
...
ImDrawList *ImGui::GetOverlayDrawList() { return &GImGui->OverlayDrawList; }
...
void ImGui::NewFrame()
{
			...
			if (g.IO.IniFilename != NULL)
				...
				g.IO.WantSaveIniSettings
					= true; // Let user know they can call
							// SaveIniSettingsToMemory(). user will need to
	...
	g.HoveredId = 0;
	g.HoveredIdAllowOverlap = false;
	if (g.ActiveIdIsAlive != g.ActiveId && g.ActiveIdPreviousFrame == g.ActiveId
		&& g.ActiveId != 0)
		ClearActiveID();
	if (g.ActiveId)
...
static bool ImGui::BeginChildEx(const char *name, ImGuiID id,
								const ImVec2 &size_arg, bool border,
								ImGuiWindowFlags flags)
{
	ImGuiContext &g = *GImGui;
									...
									ImGuiWindowFlags flags)
{
	...
	window->DC.CursorMaxPos
		= window->Pos; // So first call to CalcSizeContents() doesn't return
					   // crazy values
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow *window, const ImVec2 &size_contents)
{
	...
	else
	{
		...
		if (is_popup || is_menu) // Popups and menus bypass style.WindowMinSize
								 // by default, but we give then a non-zero
								 // minimum size to facilitate understanding
								  ...
								  float perp_padding, float thickness)
{
	...
	if (border_n == 0)
		return ImRect(rect.Min.x + perp_padding, rect.Min.y - thickness,
					  rect.Max.x - perp_padding, rect.Min.y + thickness); // Top
	if (border_n == 1)
				...
				= g.IO.MousePos - g.ActiveIdClickOffset
				  + ImLerp(grip.InnerDir * grip_hover_outer_size,
						   ...
						   grip.CornerPosN); // Corner of the window
											 // corresponding to our corner grip
			CalcResizePosSizeFromAnyCorner(window, corner_target,
										   grip.CornerPosN, &pos_target,
										   &size_target);
		}
...
static void ImGui::RenderOuterBorders(ImGuiWindow *window)
{
	...
	if (border_held != -1)
	{
		...
		static const ImGuiResizeBorderDef resize_border_def[4] = {
			...
			{ ImVec2(-1, 0), ImVec2(1, 0), ImVec2(1, 1),
			  IM_PI * 0.00f }, // Right
			{ ImVec2(0, -1), ImVec2(1, 1), ImVec2(0, 1),
			  IM_PI * 0.50f }, // Bottom
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		if (!window->Collapsed)
		{
			...
			window->ScrollbarX
				= (flags & ImGuiWindowFlags_AlwaysHorizontalScrollbar)
				  || ((window->SizeContents.x > size_x_for_scrollbars
													- (window->ScrollbarY
														   ? style.ScrollbarSize
														   : 0.0f))
					  && !(flags & ImGuiWindowFlags_NoScrollbar)
		...
		else
		{
			...
			if (!(flags & ImGuiWindowFlags_NoTitleBar))
			{
				ImU32 title_bar_col = GetColorU32(title_bar_is_highlight
													  ? ImGuiCol_TitleBgActive
													  : ImGuiCol_TitleBg);
				window->DrawList->AddRectFilled(
	...
	window->SkipItems = (window->Collapsed || !window->Active || window->Hidden)
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
bool ImGui::Begin(const char *name, bool *p_open, const ImVec2 &size_first_use,
				  float bg_alpha_override, ImGuiWindowFlags flags)
{
...
void ImGui::End()
{
	...
	// Stop logging
	if (!(window->Flags
		  & ImGuiWindowFlags_ChildWindow)) // FIXME: add more options for scope
										   // of logging
		LogFinish();
...
void ImGui::PopStyleVar(int count)
{
	...
	while (count > 0)
	{
		...
		// GDataTypeSize[info->AtomType] * info->Count), the overhead in Debug
		// is not worth it.
...
void ImGui::SetWindowScrollX(ImGuiWindow *window, float new_scroll_x)
{
	window->DC.CursorMaxPos.x
		+= window->Scroll.x; // SizeContents is generally computed based on
							 // CursorMaxPos which is affected by scroll
	...
	window->DC.CursorPos
		+= (window->Pos
			- old_pos); // As we happen to move the window while it is being
						// appended to (which is a bad idea - will smear) let's
						// at least offset the cursor
	window->DC.CursorMaxPos
		+= (window->Pos
...
void ImGui::SetNextWindowFocus()
{
	...
	g.NextWindowData.FocusCond
		= ImGuiCond_Always; // Using a Cond member for consistency (may
							// transition all of them to single flag set for
							// fast Clear() op)
}
...
void ImGui::SetScrollHereY(float center_y_ratio)
{
	...
	target_y += (window->DC.PrevLineSize.y * center_y_ratio)
				+ (GImGui->Style.ItemSpacing.y * (center_y_ratio - 0.5f)
				   * 2.0f); // Precisely aim above, in the middle or below the
							// last line.
...
void ImGui::BeginTooltip()
{
	...
	if (g.DragDropWithinSourceOrTarget)
	{
		...
		ImVec2 tooltip_pos = g.IO.MousePos
							 + ImVec2(16 * g.Style.MouseCursorScale,
		...
		&& IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup))
	{
		ImGuiID id
			= str_id ? window->GetID(str_id)
					 : window->DC
						   .LastItemId; // If user hasn't passed an ID, we can
		...
		float horizontal_overlap
			= g.Style.ItemInnerSpacing
				  .x; // We want some overlap to convey the relative depth of
					  // each menu (currently the amount of overlap is
	...
	if (window->Flags & ImGuiWindowFlags_Tooltip)
	{
		...
		if (!g.NavDisableHighlight && g.NavDisableMouseHover
			...
			r_avoid = ImRect(
				...
				ref_pos.y + 24 * sc); // FIXME: Hard-coded based on mouse cursor
									  // shape expectation. Exact dimension not
									  // very important.
	...
	if (g.NavId == id)
	{
		g.NavWindow
			= window; // Always refresh g.NavWindow, because some operations
					  // such as FocusItem() don't have a window.
	...
	else
	{
		...
		ImVec2 pos
			= g.NavWindow->Pos
			  + ImVec2(
				  rect_rel.Min.x
					  + ImMin(g.Style.FramePadding.x * 4, rect_rel.GetWidth()),
				  rect_rel.Max.y
					  - ImMin(g.Style.FramePadding.y, rect_rel.GetHeight()));
...
float ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode)
{
	...
	if (t < 0.0f
		&& mode == ImGuiInputReadMode_Released) // Return 1.0f when just
												// released, no repeat, ignore
												// analog input.
		...
		if (page_up_held != page_down_held) // If either (not both) are pressed
		{
			...
			else
			{
				...
				if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true))
				{
					...
					g.NavMoveDir
						= ImGuiDir_Down; // Because our scoring rect is offset,
										 // we intentionally request the
										 // opposite direction (so we can always
...
static void ImGui::NavUpdateWindowing()
{
	...
	bool start_windowing_with_keyboard
		= !g.NavWindowingTarget && g.IO.KeyCtrl && IsKeyPressedMap(ImGuiKey_Tab)
		  && (g.IO.ConfigFlags & ImGuiConfigFlags_NavEnableKeyboard);
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
	...
	const float width = preserve_width ? GetColumnWidthEx(
							columns, column_index, columns->IsBeingResized)
									   : 0.0f;

	if (!(columns->Flags & ImGuiColumnsFlags_NoForceWithinWindow))
		offset = ImMin(offset, columns->MaxX
								   - g.Style.ColumnsMinSpacing
...
void ImGui::LoadIniSettingsFromMemory(const char *ini_data, size_t ini_size)
{
	...
	for (char *line = buf; line < buf_end; line = line_end + 1)
	{
		...
		{
			// Parse "[AtomType][Name]". Note that 'Name' can itself contains []
										   ...
										   ImGuiTextBuffer *buf)
{
	...
	for (int i = 0; i != g.SettingsWindows.Size; i++)
	{
		...
		if (const char *p
			= strstr(name, "###")) // Skip to the "###" marker if any. We don't
...
void ImGui::ShowMetricsWindow(bool *p_open)
{
	...
	struct Funcs
	{
		static void NodeDrawList(ImGuiWindow *window, ImDrawList *draw_list,
								 const char *label)
		{
				 ...
				 elem_offset += pcmd->ElemCount, pcmd++)
			{
				...
				ImGuiListClipper clipper(
					pcmd->ElemCount / 3); // Manually coarse clip our print out
										  // of individual vertices to save CPU,
		...
		{
			if (!ImGui::TreeNode(window, "%s '%s', %d @ 0x%p", label,
								 window->Name,
			...
			ImGui::BulletText("Scroll: (%.2f/%.2f,%.2f/%.2f)", window->Scroll.x,
							  GetWindowScrollMaxX(window), window->Scroll.y,
							...
							columns->ID, columns->Count, columns->Flags))
					{
						...
						for (int column_n = 0; column_n < columns->Columns.Size;
							...
							ImGui::BulletText(
								...
								column_n, columns->Columns[column_n].OffsetNorm,
								OffsetNormToPixels(
									columns,
