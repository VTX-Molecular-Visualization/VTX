...
#	pragma clang diagnostic ignored \
		"-Wold-style-cast" // warning : use of old-style cast // yes, they are
						   // more terse.
#	pragma clang diagnostic ignored \
		"-Wdeprecated-declarations" // warning : 'xx' is deprecated: The POSIX
									// name for this item.. // for strdup used
									// in demo code (so user can copy & paste
									// the code)
#	pragma clang diagnostic ignored \
...
#		pragma clang diagnostic ignored \
			"-Wzero-as-null-pointer-constant" // warning : zero as null pointer
											  // constant                  //
											  // some standard header variations
											  // use #define NULL 0
#	endif
...
static void ShowDemoWindowWidgets()
{
	...
	if (ImGui::TreeNode("Basic"))
	{
...

		{
			...
			ShowHelpMarker(
				...
				"the full BeginCombo/EndCombo API, and demonstration of "
				"various flags.\n");
		}

		{
			...
			ShowHelpMarker(
				...
				"ESCAPE to revert.\n\nPROGRAMMER:\nYou can use the "
				"ImGuiInputTextFlags_CallbackResize facility if you need to "
				"wire InputText() to a dynamic string type. See "
	...
	if (ImGui::TreeNode("Trees"))
	{
		...
		if (ImGui::TreeNode("Advanced, with Selectable nodes"))
		{
			...
			ImGui::PushStyleVar(
				...
				ImGui::GetFontSize() * 3); // Increase spacing to differentiate
	...
	if (ImGui::TreeNode("Images"))
	{
		...
		if (ImGui::IsItemHovered())
		{
			...
			ImGui::Image(my_tex_id, ImVec2(region_sz * zoom, region_sz * zoom),
						 uv0, uv1, ImColor(255, 255, 255, 255),
						 ImColor(255, 255, 255, 128));
			ImGui::EndTooltip();
		}
	...
	if (ImGui::TreeNode("Combo"))
	{
		...
		ImGui::Combo("combo 4 (function)", &item_current_4,
					 &FuncHolder::ItemGetter, items, IM_ARRAYSIZE(items));
	...
	if (ImGui::TreeNode("Selectables"))
	{
		...
		if (ImGui::TreeNode("Alignment"))
		{
			ShowHelpMarker(
				...
				"style.SelectableTextAlign but it can be overriden on a "
				"per-item basis using PushStyleVar().");
...
#	endif
		...
		ImGui::DragScalar("drag float", ImGuiDataType_Float, &f32_v, 0.005f,
						  &f32_zero, &f32_one, "%f", 1.0f);
		ImGui::DragScalar("drag float ^2", ImGuiDataType_Float, &f32_v, 0.005f,
						  &f32_zero, &f32_one, "%f", 2.0f);
		ImGui::SameLine();
		ShowHelpMarker(
			"You can use the 'power' parameter to increase tweaking precision "
			"on one side of the range.");
		ImGui::DragScalar("drag double", ImGuiDataType_Double, &f64_v, 0.0005f,
						  &f64_zero, NULL, "%.10f grams", 1.0f);
		ImGui::DragScalar("drag double ^2", ImGuiDataType_Double, &f64_v,
						  0.0005f, &f64_zero, &f64_one, "0 < %.10f < 1", 2.0f);
		...
		ImGui::SliderScalar("slider float low", ImGuiDataType_Float, &f32_v,
							&f32_zero, &f32_one);
		ImGui::SliderScalar("slider float low^2", ImGuiDataType_Float, &f32_v,
							&f32_zero, &f32_one, "%.10f", 2.0f);
		...
		ImGui::SliderScalar("slider double low", ImGuiDataType_Double, &f64_v,
							&f64_zero, &f64_one, "%.10f grams", 1.0f);
		ImGui::SliderScalar("slider double low^2", ImGuiDataType_Double, &f64_v,
							&f64_zero, &f64_one, "%.10f", 2.0f);
		ImGui::SliderScalar("slider double high", ImGuiDataType_Double, &f64_v,
							&f64_lo_a, &f64_hi_a, "%e grams", 1.0f);
	...
	if (ImGui::TreeNode("Drag and Drop"))
	{
...

		{
			...
			for (int n = 0; n < IM_ARRAYSIZE(s_names); n++)
			{
				...
				if ((n % 3) != 0)
					ImGui::SameLine();
				ImGui::Button(s_names[n], ImVec2(60, 60));
				...
				if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None))
				{
					...
					if (mode == Mode_Move)
					{
						ImGui::Text("Move %s", s_names[n]);
					}
					if (mode == Mode_Swap)
					{
						ImGui::Text("Swap %s", s_names[n]);
					}
				...
				if (ImGui::BeginDragDropTarget())
				{
						...
						= ImGui::AcceptDragDropPayload("DND_DEMO_CELL"))
					{
						...
						if (mode == Mode_Swap)
						{
							const char *tmp = s_names[n];
							s_names[n] = s_names[payload_n];
							s_names[payload_n] = tmp;
						}
					}
					ImGui::EndDragDropTarget();
				}
	...
	if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)"))
	{
		...
		if (embed_all_inside_a_child_window)
			ImGui::BeginChild("outer_child",
							  ImVec2(0, ImGui::GetFontSize() * 20), true);
...
static void ShowDemoWindowLayout()
{
	...
	if (ImGui::TreeNode("Tabs"))
	{
		...
		if (ImGui::TreeNode("Advanced & Close Button"))
		{
			...
			// Tab Bar
			const char *s_names[4]
				= { "Artichoke", "Beetroot", "Celery", "Daikon" };
			static bool opened[4]
				= { true, true, true, true }; // Persistent user state
			for (int n = 0; n < IM_ARRAYSIZE(opened); n++)
			{
				...
				ImGui::Checkbox(s_names[n], &opened[n]);
			}
			...
			if (ImGui::BeginTabBar("MyTabBar", tab_bar_flags))
			{
				for (int n = 0; n < IM_ARRAYSIZE(opened); n++)
					if (opened[n]
						&& ImGui::BeginTabItem(s_names[n], &opened[n]))
					{
						ImGui::Text("This is the %s tab!", s_names[n]);
						if (n & 1)
	...
	if (ImGui::TreeNode("Groups"))
	{
		ShowHelpMarker(
			...
			"BeginGroup() basically locks the horizontal position. EndGroup() "
			"bundles the whole group so that you can use functions such as "
			"IsItemHovered() on it.");
		ImGui::BeginGroup();
		{
			ImGui::BeginGroup();
	...
	{
		ShowHelpMarker(
			"This is testing the vertical alignment that gets applied on text "
			"to keep it aligned with widgets. Lines only composed of text or "
		...
		bool node_open = ImGui::TreeNode(
			"Node##2"); // Common mistake to avoid: if we want to SameLine after
						// TreeNode we need to do it before we add child
						// content.
		ImGui::SameLine(0.0f, spacing);
	...
	if (ImGui::TreeNode("Horizontal Scrolling"))
	{
		...
		ImGui::BeginChild(
			"scrolling", ImVec2(0, ImGui::GetFrameHeightWithSpacing() * 7 + 30),
...
static void ShowDemoWindowPopups()
{
	...
	if (ImGui::TreeNode("Popups"))
	{
		...
		ImGui::TextUnformatted(selected_fish == -1 ? "<None>"
												   : s_names[selected_fish]);
		if (ImGui::BeginPopup("my_select_popup"))
		{
			ImGui::Text("Aquarium");
			ImGui::Separator();
			for (int i = 0; i < IM_ARRAYSIZE(s_names); i++)
				if (ImGui::Selectable(s_names[i]))
					selected_fish = i;
			ImGui::EndPopup();
		}
		...
		{
			for (int i = 0; i < IM_ARRAYSIZE(s_names); i++)
				ImGui::MenuItem(s_names[i], "", &toggles[i]);
...
static void ShowDemoWindowColumns()
{
	...
	if (ImGui::TreeNode("Basic"))
	{
		...
		ImGui::NextColumn();
		ImGui::Separator();
		const char *s_names[3] = { "One", "Two", "Three" };
...
void ImGui::ShowStyleEditor(ImGuiStyle *ref)
{
	...
	if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None))
	{
		...
		if (ImGui::BeginTabItem("Colors"))
		{
			...
			if (ImGui::Button("Export Unsaved"))
			{
				if (output_dest == 0)
					...
					ImGui::LogToTTY();
				ImGui::LogText(
					"ImVec4* s_colors = ImGui::GetStyle().Colors;" IM_NEWLINE);
				for (int i = 0; i < ImGuiCol_COUNT; i++)
				{
					...
					if (!output_only_modified
						...
						ImGui::LogText(
							"s_colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, "
							"%.2ff, %.2ff);" IM_NEWLINE,
			...
			static ImGuiTextFilter filter;
			filter.Draw("Filter s_colors", ImGui::GetFontSize() * 16);
			...
			ImGui::BeginChild("##s_colors", ImVec2(0, 0), true,
		...
		if (ImGui::BeginTabItem("Fonts"))
		{
			...
			ShowHelpMarker(
				"Read FAQ and misc/fonts/README.txt for details on font "
				"loading.");
			ImGui::PushItemWidth(120);
			for (int i = 0; i < atlas->Fonts.Size; i++)
			{
				...
				if (font_details_opened)
				{
					...
					ImGui::InputFloat("Font offset", &font->DisplayOffset.y, 1,
									  1, "%.0f");
					...
					ImGui::Text("Texture surface: %d pixels (approx) ~ %dx%d",
								font->MetricsTotalSurface, (int)surface_sqrt,
								(int)surface_sqrt);
									...
									if (glyph)
										font->RenderChar(
											...
											(ImWchar)(
												base
												+ n)); // We use
													   // ImFont::RenderChar as
													   // a shortcut because we
													   // don't have UTF-8
...
static void ShowExampleAppLog(bool *p_open)
{
	...
	if (ImGui::SmallButton("[Debug] Add 5 entries"))
	{
		...
		for (int n = 0; n < 5; n++)
		{
			...
			const char *words[]
				= { "Bumfuzzled",	"Cattywampus",  "Snickersnee",
...
static void ShowExampleAppPropertyEditor(bool *p_open)
{
	...
	struct funcs
	{
		static void ShowDummyObject(const char *prefix, int uid)
		{
			...
			if (node_open)
			{
				static float dummy_members[8]
					= { 0.0f, 0.0f, 1.0f, 3.1416f, 100.0f, 999.0f };
				for (int i = 0; i < 8; i++)
				{
...
static void ShowExampleAppCustomRendering(bool *p_open)
{
	...
	ImGui::ColorEdit4("Color", &col.x);
	{
		...
		for (int n = 0; n < 2; n++)
		{
			...
			draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col32,
							   0.0f, ImDrawCornerFlags_All, th);
			x += sz + spacing;
			draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col32,
							   10.0f, ImDrawCornerFlags_All, th);
			x += sz + spacing;
			draw_list->AddRect(
				ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 10.0f,
				ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotRight, th);
			x += sz + spacing;
			draw_list->AddTriangle(ImVec2(x + sz * 0.5f, y),
								   ImVec2(x + sz, y + sz - 0.5f),
								   ImVec2(x, y + sz - 0.5f), col32, th);
			x += sz + spacing;
	...
	ImGui::Separator();
	{
		...
		ImVec2 canvas_pos = ImGui::GetCursorScreenPos(); // ImDrawList API uses
														 // screen coordinates!
		ImVec2 canvas_size = ImGui::GetContentRegionAvail(); // Resize canvas to
															 // what's available
