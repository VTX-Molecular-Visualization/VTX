...
namespace ImGui
{
	...
	// imgui contexts. All those functions are not reliant on the current
	// context.
	IMGUI_API ImGuiContext *CreateContext(ImFontAtlas *shared_font_atlas
										  = NULL);
	IMGUI_API void DestroyContext(ImGuiContext *ctx
								  = NULL); // NULL = destroy current context
	...
	// Main
	IMGUI_API ImGuiIO &GetIO(); // access the IO structure
								// (mouse/keyboard/gamepad inputs, time, various
								// configuration options/flags)
	IMGUI_API ImGuiStyle &GetStyle(); // access the Style structure (colors,
									  // sizes). Always use PushStyleCol(),
									  // PushStyleVar() to modify style
									  // mid-frame.
	...
	IMGUI_API void EndFrame(); // ends the ImGui frame. automatically called by
							   // Render(), you likely don't need to call that
							   // yourself directly. If you don't need to render
							   // data (skipping rendering) you may call
							   // EndFrame() but you'll have wasted CPU already!
							   // If you don't need to render, better to not
							   // create any imgui windows and not call
							   // NewFrame() at all!
	IMGUI_API void Render();   // ends the ImGui frame, finalize the draw data.
							 // (Obsolete: optionally call io.RenderDrawListsFn
							 // if set. Nowadays, prefer calling your render
							 // function yourself.)
	IMGUI_API ImDrawData *GetDrawData(); // valid after Render() and until the
										 // next call to NewFrame(). this is
										 // what you have to render. (Obsolete:
										 // this used to be passed to your
										 // io.RenderDrawListsFn() function.)

	// Demo, Debug, Information
	IMGUI_API void ShowDemoWindow(
		bool *p_open
		= NULL); // create demo/test window (previously called ShowTestWindow).
				 // demonstrate most ImGui features. call this to learn about
				 // the library! try to make it always available in your
				 // application!
	IMGUI_API void ShowAboutWindow(
		bool *p_open = NULL); // create about window. display Dear ImGui
							  // version, credits and build/system information.
	...
	IMGUI_API bool ShowStyleSelector(
		const char *label); // add style selector block (not a window),
							// essentially a combo listing the default styles.
	IMGUI_API void ShowFontSelector(
		const char *label); // add font selector block (not a window),
	...
	IMGUI_API const char *GetVersion(); // get the compiled version string e.g.
										// "1.23"

	// Styles
	IMGUI_API void StyleColorsDark(ImGuiStyle *dst
								   = NULL); // new, recommended style (default)
	IMGUI_API void StyleColorsClassic(ImGuiStyle *dst
									  = NULL); // classic imgui style
	IMGUI_API void StyleColorsLight(
		ImGuiStyle *dst
		= NULL); // best used with borders and a custom, thicker font

	// Windows
	// - Begin() = push window to the stack and start appending to it. End() =
	...
	//   call, regardless of its return value! [this is due to legacy reason and
	//   is inconsistent with most other functions such as BeginMenu/EndMenu,
	...
	// their own child.
	// - For each independent axis of 'size': ==0.0f: use remaining host window
	// size / >0.0f: fixed size / <0.0f: use remaining window size minus
	// abs(size) / Each axis can use a different mode, e.g. ImVec2(0,400).
	...
	//   BeginPopup/EndPopup, etc. where the EndXXX call should only be called
	...
	IMGUI_API ImDrawList *GetWindowDrawList(); // get draw list associated to
											   // the current window, to append
											   // your own drawing primitives

	IMGUI_API ImVec2
	GetWindowPos(); // get current window position in screen space (useful if
	...
	IMGUI_API float GetWindowWidth();  // get current window width (shortcut for
									   // GetWindowSize().x)
	IMGUI_API float GetWindowHeight(); // get current window height (shortcut
									   // for GetWindowSize().y)
	IMGUI_API ImVec2
	...
	IMGUI_API ImVec2
	GetWindowContentRegionMin(); // content boundaries min (roughly
	...
	IMGUI_API void SetNextWindowContentSize(
		const ImVec2
			&size); // set next window content size (~ enforce the range of
					// scrollbars). not including window decorations (title bar,
					// menu bar, etc.). set an axis to 0.0f to leave it
	...
	IMGUI_API void SetNextWindowBgAlpha(
		float alpha); // set next window background color alpha. helper to
					  // easily modify ImGuiCol_WindowBg/ChildBg/PopupBg. you
	...
	IMGUI_API void SetWindowSize(
		const ImVec2 &size,
		ImGuiCond cond
		= 0); // (not recommended) set current window size - call within
	...
	IMGUI_API void SetWindowFocus(); // (not recommended) set current window to
									 // be focused / front-most. prefer using
									 // SetNextWindowFocus().
	IMGUI_API void SetWindowFontScale(
		float scale); // set font scale. Adjust IO.FontGlobalScale if you want
					  // to scale all windows
	IMGUI_API void SetWindowPos(const char *name, const ImVec2 &pos,
								ImGuiCond cond
								= 0); // set named window position.
	IMGUI_API void SetWindowSize(
		const char *name, const ImVec2 &size,
		ImGuiCond cond = 0); // set named window size. set axis to 0.0f to force
							 // an auto-fit on this axis.
	IMGUI_API void SetWindowCollapsed(const char *name, bool collapsed,
									  ImGuiCond cond
									  = 0); // set named window collapsed state
	IMGUI_API void SetWindowFocus(
		const char *name); // set named window to be focused / front-most. use
	...
	IMGUI_API const ImVec4 &GetStyleColorVec4(
		ImGuiCol
			idx); // retrieve style color as stored in ImGuiStyle structure. use
				  // to feed back into PushStyleColor(), otherwise use
				  // GetColorU32() to get style color with style alpha baked in.
	IMGUI_API ImFont *GetFont();   // get current font
	IMGUI_API float GetFontSize(); // get current font size (= height in pixels)
								   // of current font with current scale applied
	IMGUI_API ImVec2
	...
	IMGUI_API void PushItemWidth(
		float item_width); // width of items for the common item+label case,
						   // pixels. 0.0f = default to ~2/3 of windows width,
						   // >0.0f: width in pixels, <0.0f align xx pixels to
						   // the right of window (so -1.0f always align width
						   // to the right side)
	...
	IMGUI_API void PushAllowKeyboardFocus(
		bool allow_keyboard_focus); // allow focusing using TAB/Shift-TAB,
									// enabled by default but you can disable it
									// for certain widgets
	IMGUI_API void PopAllowKeyboardFocus();
	IMGUI_API void PushButtonRepeat(
		bool repeat); // in 'repeat' mode, Button*() functions return repeated
					  // true in a typematic manner (using
					  // io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that
					  // you can call IsItemActive() after any Button() to tell
	...
	IMGUI_API void Separator(); // separator, generally horizontal. inside a
								// menu bar or in horizontal layout mode, this
	...
	IMGUI_API void NewLine(); // undo a SameLine() or force a new line when in
							  // an horizontal-layout context.
	...
	IMGUI_API void Indent(float indent_w
						  = 0.0f); // move content position toward the right, by
	...
	IMGUI_API void EndGroup(); // unlock horizontal starting position + capture
							   // the whole group bounding box into one "item"
							   // (so you can use IsItemHovered() or layout
							   // primitives such as SameLine() on whole group,
							   // etc.)
	IMGUI_API ImVec2 GetCursorPos(); // cursor position in window coordinates
									 // (relative to window position)
	IMGUI_API float GetCursorPosX(); //   (some functions are using
									 //   window-relative coordinates, such as:
									 //   GetCursorPos, GetCursorStartPos,
									 //   GetContentRegionMax,
									 //   GetWindowContentRegion* etc.
	IMGUI_API float GetCursorPosY(); //    other functions such as
									 //    GetCursorScreenPos or everything in
									 //    ImDrawList::
	...
	IMGUI_API ImVec2
	GetCursorScreenPos(); // cursor position in absolute screen coordinates
						  // [0..io.DisplaySize] (useful to work with ImDrawList
						  // API)
	IMGUI_API void SetCursorScreenPos(
		const ImVec2 &pos); // cursor position in absolute screen coordinates
							// [0..io.DisplaySize]
	IMGUI_API void AlignTextToFramePadding(); // vertically align upcoming text
											  // baseline to FramePadding.y so
											  // that it will align properly to
											  // regularly framed items (call if
											  // you have text on a line before
											  // a framed item)
	...
	// If you are creating widgets in a loop you most
	//   likely want to push a unique identifier (e.g. object pointer, loop
	//   index) to uniquely differentiate them.
	...
	GetID(const char *str_id); // calculate unique ID (hash of whole ID stack +
							   // given parameter). e.g. if you want to query
	...
	IMGUI_API void TextUnformatted(
		...
		= NULL); // raw text without formatting. Roughly equivalent to
				 // Text("%s", text) but: A) doesn't require null terminated
				 // string if 'text_end' is specified, B) it's faster, no memory
				 // copy is done, no buffer size limits, recommended for long
				 // chunks of text.
	...
	IMGUI_API void TextColoredV(const ImVec4 &col, const char *fmt,
	...
	IMGUI_API void TextWrapped(const char *fmt, ...) IM_FMTARGS(
		1); // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...);
			...
			// window if there's no other widgets to extend the window width,
			// yoy may need to set a size using SetNextWindowSize().
	IMGUI_API void TextWrappedV(const char *fmt, va_list args) IM_FMTLIST(1);
	IMGUI_API void LabelText(const char *label, const char *fmt, ...)
		IM_FMTARGS(2); // display text+label aligned the same way as value+label
					   // widgets
	...
	IMGUI_API bool RadioButton(
		...
		int v_button); // shortcut to handle the above pattern when value is an
					   // integer
	IMGUI_API void ProgressBar(float fraction,
							   const ImVec2 &size_arg = ImVec2(-1, 0),
							   const char *overlay = NULL);
	IMGUI_API void Bullet(); // draw a small circle and keep the cursor on the
							 // same line. advance cursor x position by
							 // GetTreeNodeToLabelSpacing(), same distance that
							 // TreeNode() uses

	// Widgets: Combo Box
	// - The new BeginCombo()/EndCombo() api allows you to manage your contents
	// and selection state however you want it, by creating e.g. Selectable()
	// items.
	...
	IMGUI_API void EndCombo(); // only call EndCombo() if BeginCombo() returns
							   // true!
	...
	// functions, note that a 'float v[X]' function argument is the same as
	// 'float* v', the array syntax is just a way to document the number of
	// elements that are expected to be accessible. You can pass address of your
	// first element out of a contiguous set, e.g. &myvector.x
	// - Adjust format string to decorate the value with a prefix, a suffix, or
	// adapt the editing and display precision e.g. "%.3f" -> 1.234; "%5.2f
	// secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
	...
	// navigation, minimum speed is Max(v_speed,
	// minimum_step_at_given_precision).
	IMGUI_API bool DragFloat(const char *label, float *v, float v_speed = 1.0f,
							 float v_min = 0.0f, float v_max = 0.0f,
							 const char *format = "%.3f",
							 float power
							 = 1.0f); // If v_min >= v_max we have no bound
	IMGUI_API bool DragFloat2(const char *label, float v[2],
							  float v_speed = 1.0f, float v_min = 0.0f,
							  float v_max = 0.0f, const char *format = "%.3f",
							  float power = 1.0f);
	IMGUI_API bool DragFloat3(const char *label, float v[3],
							  float v_speed = 1.0f, float v_min = 0.0f,
							  float v_max = 0.0f, const char *format = "%.3f",
							  float power = 1.0f);
	IMGUI_API bool DragFloat4(const char *label, float v[4],
							  float v_speed = 1.0f, float v_min = 0.0f,
							  float v_max = 0.0f, const char *format = "%.3f",
							  float power = 1.0f);
	...
	IMGUI_API bool DragInt(const char *label, int *v, float v_speed = 1.0f,
						   ...
						   const char *format
						   = "%d"); // If v_min >= v_max we have no bound
	...
	IMGUI_API bool DragScalar(const char *label, ImGuiDataType data_type,
							  void *v, float v_speed, const void *v_min = NULL,
	...
	IMGUI_API bool DragScalarN(const char *label, ImGuiDataType data_type,
							   void *v, int components, float v_speed,
	...
	// - Most of the ImGuiInputTextFlags flags are only useful for InputText()
	// and not for InputFloatX, InputIntX, InputDouble etc.
	IMGUI_API bool InputText(const char *label, char *buf, size_t buf_size,
							 ImGuiInputTextFlags flags = 0,
							 ImGuiInputTextCallback callback = NULL,
							 void *user_data = NULL);
	IMGUI_API bool InputTextMultiline(const char *label, char *buf,
									  size_t buf_size,
									  const ImVec2 &size = ImVec2(0, 0),
									  ImGuiInputTextFlags flags = 0,
									  ImGuiInputTextCallback callback = NULL,
	...
	// - Note that in C++ a 'float v[X]' function argument is the _same_ as
	// 'float* v', the array syntax is just a way to document the number of
	// elements that are expected to be accessible. You can the pass the address
	// of a first float element out of a contiguous structure, e.g. &myvector.x
	...
	IMGUI_API void SetColorEditOptions(
		ImGuiColorEditFlags
			flags); // initialize current options (generally on application
					...
					// type, etc. User will be able to change many settings,
					// unless you pass the _NoOptions flag to your calls.

	// Widgets: Trees
	// - TreeNode functions return true when the node is open, in which case you
	// need to also call TreePop() when you are finished displaying the tree
	// node contents.
	IMGUI_API bool TreeNode(const char *label);
	IMGUI_API bool TreeNode(const char *str_id, const char *fmt, ...)
		IM_FMTARGS(2); // helper variation to completely decorelate the id from
					   // the displayed string. Read the FAQ about why and how
					   // to use ID. to align arbitrary text at the same level
					   // as a TreeNode() you can use Bullet().
	...
	IMGUI_API void TreePush(
		const char *str_id); // ~ Indent()+PushId(). Already called by
							 // TreeNode() when returning true, but you can call
	...
	IMGUI_API float GetTreeNodeToLabelSpacing(); // horizontal distance
												 // preceding label when using
												 // TreeNode*() or Bullet() ==
												 // (g.FontSize +
												 // style.FramePadding.x*2) for
	...
	// Widgets: Selectables
	// - A selectable highlights when hovered, and can display another color
	// when selected.
	...
	IMGUI_API bool Selectable(
		...
		= ImVec2(0,
				 0)); // "bool selected" carry the selection state (read-only).
					  ...
					  // width, size.x>0.0: specify width. size.y==0.0: use
	...
	// Widgets: List Boxes
	// - FIXME: To be consistent with all the newer API,
	// ListBoxHeader/ListBoxFooter should in reality be called
	...
	IMGUI_API bool ListBoxHeader(
		...
		const ImVec2 &size = ImVec2(
			0, 0)); // use if you want to reimplement ListBox() will custom data
					// or interactions. if the function return true, you can
					// output elements then call ListBoxFooter() afterwards.
	IMGUI_API bool ListBoxHeader(const char *label, int items_count,
								 int height_in_items = -1); // "
	IMGUI_API void ListBoxFooter(); // terminate the scrolling region. only call
									// ListBoxFooter() if ListBoxHeader()
									// returned true!
	...
	// Output single value in "name: value" format (tip: freely declare more in
	// your code to handle your types. you can add functions to the ImGui
	// namespace)
	...
	IMGUI_API bool BeginMenuBar();	 // append to menu-bar of current window
								   // (requires ImGuiWindowFlags_MenuBar flag
								   // set on parent window).
	IMGUI_API void EndMenuBar(); // only call EndMenuBar() if BeginMenuBar()
								 // returns true!
	IMGUI_API bool BeginMenu(const char *label,
							 bool enabled
							 = true); // create a sub-menu entry. only call
									  // EndMenu() if this returns true!
	IMGUI_API void EndMenu(); // only call EndMenu() if BeginMenu() returns
							  // true!
	...
	IMGUI_API bool MenuItem(const char *label, const char *shortcut,
							bool *p_selected,
							bool enabled
							= true); // return true when activated + toggle
	...
	IMGUI_API void SetTooltip(const char *fmt, ...)
		IM_FMTARGS(1); // set a text-only tooltip, typically use with
	...
	IMGUI_API void OpenPopup(
		const char *str_id); // call to mark popup as open (don't call every
							 // frame!). popups are closed when user click
							 // outside, or if CloseCurrentPopup() is called
							 // within a BeginPopup()/EndPopup() block. By
							 // default, Selectable()/MenuItem() are calling
							 // CloseCurrentPopup(). Popup identifiers are
							 // relative to the current ID-stack (so OpenPopup
							 // and BeginPopup needs to be at the same level).
	...
	IMGUI_API bool BeginPopupContextItem(
		...
		int mouse_button
		= 1); // helper to open and begin popup when clicked on last item. if
			  // you can pass a NULL str_id only if the previous item had an id.
			  // If you want to use that on a non-interactive item such as
			  // Text() you need to pass in an explicit ID here. read comments
			  // in .cpp!
	...
	IMGUI_API bool BeginPopupModal(
		...
		= 0); // modal dialog (regular window with title bar, block interactions
			  ...
			  // clicking outside)
	IMGUI_API void EndPopup(); // only call EndPopup() if BeginPopupXXX()
							   // returns true!
	...
	IMGUI_API void CloseCurrentPopup(); // close the popup we have begin-ed
										// into. clicking on a MenuItem or
										// Selectable automatically close the
										// current popup.
	...
	IMGUI_API void SetColumnOffset(
		int column_index,
		float offset_x); // set position of column line (in pixels, from the
	...
	IMGUI_API bool BeginTabBar(const char *str_id,
							   ...
							   = 0); // create and append into a TabBar
	IMGUI_API void EndTabBar();		 // only call EndTabBar() if BeginTabBar()
									 // returns true!
	IMGUI_API bool BeginTabItem(
		const char *label, bool *p_open = NULL,
		ImGuiTabItemFlags flags
		= 0); // create a Tab. Returns true if the Tab is selected.
	IMGUI_API void EndTabItem(); // only call EndTabItem() if BeginTabItem()
								 // returns true!
	IMGUI_API void SetTabItemClosed(
		const char
			*tab_or_docked_window_label); // notify TabBar or Docking system of
										  // a closed tab/window ahead (useful
										  // to reduce visual flicker on
										  // reorderable tab bars). For tab-bar:
										  // call after BeginTabBar() and before
										  // Tab submissions. Otherwise call
										  // with a window name.
	...
	// during logging.
	IMGUI_API void LogToTTY(int max_depth
							= -1); // start logging to tty (stdout)
	IMGUI_API void LogToFile(int max_depth = -1,
							 const char *filename
							 = NULL); // start logging to file
	IMGUI_API void LogToClipboard(int max_depth
								  = -1); // start logging to OS clipboard
	...
	IMGUI_API bool BeginDragDropTarget(); // call after submitting an item that
										  // may receive a payload. If this
										  // returns true, you can call
	...
	IMGUI_API const ImGuiPayload *AcceptDragDropPayload(
		...
		= 0); // accept contents of a given type. If
			  // ImGuiDragDropFlags_AcceptBeforeDelivery is set you can peek
	...
	IMGUI_API const ImGuiPayload *GetDragDropPayload(); // peek directly into
														// the current payload
														// from anywhere. may
														// return NULL. use
	...
	// - Most of the functions are referring to the last/previous item we
	// submitted.
	...
	IMGUI_API bool IsItemActive();  // is the last item active? (e.g. button
									// being held, text field being edited. This
									// will continuously return true while
									// holding mouse button on an item. Items
									// that don't interact will always return
									// false)
	IMGUI_API bool IsItemFocused(); // is the last item focused for
	...
	IMGUI_API bool IsItemVisible(); // is the last item visible? (items may be
									// out of sight because of
									// clipping/scrolling)
	IMGUI_API bool IsItemEdited();  // did the last item modify its underlying
									// value this frame? or was pressed? This is
									// generally the same as the "bool" return
									// value of many widgets.
	IMGUI_API bool IsItemActivated();   // was the last item just made active
										// (item was previously inactive).
	IMGUI_API bool IsItemDeactivated(); // was the last item just made inactive
										// (item was previously active). Useful
										// for Undo/Redo patterns with widgets
										// that requires continuous editing.
	IMGUI_API bool
	IsItemDeactivatedAfterEdit(); // was the last item just made inactive and
	...
	IMGUI_API ImVec2 GetItemRectMin();  // get upper-left bounding rectangle of
										// the last item (screen space)
	IMGUI_API ImVec2 GetItemRectMax();  // get lower-right bounding rectangle of
										// the last item (screen space)
	IMGUI_API ImVec2 GetItemRectSize(); // get size of last item
	IMGUI_API void SetItemAllowOverlap(); // allow last item to be overlapped by
										  // a subsequent item. sometimes useful
										  // with invisible buttons,
										  // selectables, etc. to catch unused
										  // area.
	...
	IMGUI_API int GetFrameCount(); // get global imgui frame count. incremented
								   // by 1 every frame.
	IMGUI_API ImDrawList *GetOverlayDrawList(); // this draw list will be the
												// last rendered one, useful to
												// quickly draw overlays
												// shapes/text
	IMGUI_API ImDrawListSharedData *GetDrawListSharedData(); // you may use this
															 // when creating
															 // your own
															 // ImDrawList
															 // instances
	...
	IMGUI_API void SetStateStorage(
		ImGuiStorage *storage); // replace current window storage with our own
								// (if you want to manipulate it yourself,
	...
	IMGUI_API void CalcListClipping(
		...
		int *out_items_display_end); // calculate coarse clipping for large list
									 // of evenly sized items. Prefer using the
									 // ImGuiListClipper higher-level helper if
									 // you can.
	...
	IMGUI_API void EndChildFrame(); // always call EndChildFrame() regardless of
									// BeginChildFrame() return values (which
	...
	IMGUI_API bool IsKeyDown(
		int user_key_index); // is key being held. ==
							 // io.KeysDown[user_key_index]. note that imgui
							 // doesn't know the semantic of each entry of
							 // io.KeysDown[]. Use your own indices/enums
							 // according to how your backend/engine stored them
							 // into io.KeysDown[]!
	IMGUI_API bool IsKeyPressed(
		int user_key_index,
		bool repeat
		= true); // was key pressed (went from !Down to Down). if repeat=true,
	...
	IMGUI_API int GetKeyPressedAmount(
		...
		float rate); // uses provided repeat rate/delay. return a count, most
					 // often 0 or 1 but might be >1 if RepeatRate is small
					 // enough that DeltaTime > RepeatRate
	...
	IMGUI_API ImVec2
	GetMousePos(); // shortcut to ImGui::GetIO().MousePos provided by user, to
				   // be consistent with other calls
	IMGUI_API ImVec2
	GetMousePosOnOpeningCurrentPopup(); // retrieve backup of mouse position at
										// the time of opening popup we have
										// BeginPopup() into
	IMGUI_API ImVec2 GetMouseDragDelta(
		int button = 0,
		float lock_threshold
		= -1.0f); // return the delta from the initial clicking position. This
				  // is locked and return 0.0f until the mouse moves past a
				  // distance threshold at least once. If lock_threshold < -1.0f
				  // uses io.MouseDraggingThreshold
	IMGUI_API void ResetMouseDragDelta(int button = 0); //
	IMGUI_API ImGuiMouseCursor
	GetMouseCursor(); // get desired cursor type, reset in ImGui::NewFrame(),
					  // this is updated during the frame. valid before
					  // Render(). If you use software rendering by setting
	...
	IMGUI_API void CaptureKeyboardFromApp(
		...
		= true); // attention: misleading name! manually override
				 // io.WantCaptureKeyboard flag next frame (said flag is
				 // entirely left for your application to handle). e.g. force
				 // capture keyboard when your widget is being hovered. This is
				 // equivalent to setting "io.WantCaptureKeyboard =
				 // want_capture_keyboard_value"; after the next NewFrame()
				 // call.
	...
	IMGUI_API void LoadIniSettingsFromDisk(
		const char
			*ini_filename); // call after CreateContext() and before the first
							// call to NewFrame(). NewFrame() automatically
							// calls LoadIniSettingsFromDisk(io.IniFilename).
	...
	// - If you reload the contents of imgui.cpp at runtime, you may need to
	// call SetCurrentContext() + SetAllocatorFunctions() again.
...
enum ImGuiWindowFlags_
{
	...
	= 1 << 20, // Append '*' to title without affecting the ID, as a convenience
			   // to avoid using the ### operator. When used in a tab/docking
			   // context, tab is selected on closure and closure is deferred by
			   // one frame to allow code to cancel the closure (with a
	...
	= 1
	  << 23, // [BETA] Allow gamepad/keyboard navigation to cross over parent
			 // border to this child (only use on child that have no scrolling!)
...
enum ImGuiTabItemFlags_
{
	...
	= 1 << 0, // Append '*' to title without affecting the ID, as a convenience
...
enum ImGuiFocusedFlags_
{
	...
	= 1 << 0, // IsWindowFocused(): Return true if any children of the window is
			  // focused
...
enum ImGuiDragDropFlags_
{
	...
	= 1 << 1, // By default, when dragging we clear data so that IsItemHovered()
			  ...
			  // tooltips. This flag disable this behavior so you can still call
...
enum ImGuiBackendFlags_
{
	...
	ImGuiBackendFlags_HasMouseCursors
	= 1 << 1, // Back-end supports honoring GetMouseCursor() value to change the
			  // OS cursor shape.
...
enum ImGuiColorEditFlags_
{
	...
	= 1 << 8, //              // ColorPicker: disable bigger color preview on
			  //              right side of the picker, use small colored square
			  //              preview instead.
...
struct ImGuiIO
{
	...
	IMGUI_API void ClearInputCharacters(); // Clear the text input buffer
										   // manually
...
// allocated memory via the placement "new(ptr) AtomType()" syntax. Defining a
...
enum ImDrawListFlags_
{
	...
	= 1
	  << 1 // Filled shapes have anti-aliased edges (*2 the number of vertices)
};
...
struct ImDrawList
{
	...
	IMGUI_API void AddDrawCmd(); // This is useful if you need to forcefully
								 ...
								 // primitives are merged into the same
...
struct ImDrawData
{
	...
	IMGUI_API void DeIndexAllBuffers(); // Helper to convert all buffers from
										// indexed to non-indexed, in case you
...
struct ImFontAtlas
{
	...
	IMGUI_API ImFont *AddFontFromMemoryCompressedBase85TTF(
		...
		= NULL); // 'compressed_font_data_base85' still owned by caller.
				 ...
				 // parameter.
	IMGUI_API void ClearInputData(); // Clear input data (all ImFontConfig
									 // structures including sizes, TTF data,
									 // glyph ranges, etc.) = all the data used
									 // to build the texture and fonts.
	IMGUI_API void ClearTexData();   // Clear output texture data (CPU side).
								   // Saves RAM once the texture has been copied
								   // to graphics memory.
	IMGUI_API void ClearFonts(); // Clear output font data (glyphs storage, UV
								 // coordinates).
	...
	IMGUI_API const ImWchar *GetGlyphRangesDefault();  // Basic Latin, Extended
													   // Latin
	IMGUI_API const ImWchar *GetGlyphRangesKorean();   // Default + Korean
													   // characters
	...
	IMGUI_API const ImWchar *GetGlyphRangesCyrillic(); // Default + about 400
													   // Cyrillic characters
