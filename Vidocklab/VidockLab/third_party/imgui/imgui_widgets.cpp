...
// [SECTION] Data AtomType and Data Formatting Helpers
...
#	pragma clang diagnostic ignored \
		"-Wformat-nonliteral" // warning : format string is not a string literal
							  // // passing non-literal to vsnformat(). yes,
							  // user passing incorrect format strings can crash
							  // the code.
#	pragma clang diagnostic ignored \
...
// Data AtomType helpers
static inline int DataTypeFormatString(char *buf, int buf_size,
									   ImGuiDataType data_type,
									   const void *data_ptr,
									   const char *format);
static void DataTypeApplyOp(ImGuiDataType data_type, int op, void *output,
							void *arg_1, const void *arg_2);
...
void ImGui::TextUnformatted(const char *text, const char *text_end)
{
	...
	{
		const float wrap_width = wrap_enabled ? CalcWrapWidthForPos(
									 window->DC.CursorPos, wrap_pos_x)
											  : 0.0f;
...
void ImGui::TextV(const char *fmt, va_list args)
{
	...
	const char *text_end
		= g.TempBuffer
		  + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt,
							args);
...
void ImGui::BulletTextV(const char *fmt, va_list args)
{
	...
	RenderBullet(
		bb.Min
		+ ImVec2(style.FramePadding.x + g.FontSize * 0.5f, line_height * 0.5f));
	RenderText(
		bb.Min
			+ ImVec2(g.FontSize + style.FramePadding.x * 2, text_base_offset_y),
					 ...
					 ImGuiButtonFlags flags)
{
	...
	if ((flags & ImGuiButtonFlags_AlignTextBaseLine)
		&& style.FramePadding.y
			   < window->DC
					 .CurrentLineTextBaseOffset) // Try to vertically align
												 // buttons that are
						  ...
						  ImGuiButtonFlags flags)
{
	...
	RenderArrow(bb.Min
					+ ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f),
...
ImGuiID ImGui::GetScrollbarID(ImGuiLayoutType direction)
{
	...
	ImGuiWindow *window = g.CurrentWindow;
	return window->GetID(
		(direction == ImGuiLayoutType_Horizontal) ? "#SCROLLX" : "#SCROLLY");
}
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
	...
	float alpha = 1.0f;
	if ((direction == ImGuiLayoutType_Vertical)
	...
	if (held && allow_interaction && grab_h_norm < 1.0f)
	{
		...
		const float scroll_v_norm
			= ImSaturate((clicked_v_norm - *click_delta_to_grab_center_v
						 ...
						 / (1.0f - grab_h_norm));
		scroll_v
			= (float)(int)(0.5f + scroll_v_norm * scroll_max); //(win_size_contents_v
															   //- win_size_v));
		if (horizontal)
...
//-------------------------------------------------------------------------
// [SECTION] Data AtomType and Data Formatting Helpers [Internal]
//-------------------------------------------------------------------------
...
#else
		IM_ASSERT(
			0 && "DragInt(): Invalid format string!"); // Old versions used a
...
static inline int DataTypeFormatString(char *buf, int buf_size,
									   ImGuiDataType data_type,
									   const void *data_ptr, const char *format)
{
						  ...
						  const char *format, float power, ImGuiDragFlags flags)
{
	...
	if (is_power)
	{
		...
		v_cur = v_min
				+ (TYPE)ImPow(ImSaturate((float)v_new_norm_curved), power)
					  * (v_max - v_min);
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void *v,
...
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *v,
					   float v_speed, const void *v_min, const void *v_max,
					   const char *format, float power)
{
	...
	if (start_text_input || (g.ActiveId == id && g.ScalarAsInputTextId == id))
	{
		...
		return InputScalarAsWidgetReplacement(frame_bb, id, label, data_type, v,
											  format);
	}
	...
	const char *value_buf_end
		= value_buf
		  + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type,
								 v, format);
...
bool ImGui::DragScalarN(const char *label, ImGuiDataType data_type, void *v,
						int components, float v_speed, const void *v_min,
						const void *v_max, const char *format, float power)
{
	...
	{
		PushID(i);
		value_changed |= DragScalar("", data_type, v, v_speed, v_min, v_max,
									format, power);
							...
							ImGuiSliderFlags flags, ImRect *out_grab_bb)
{
	...
	if (g.ActiveId == id)
	{
		...
		else if (g.ActiveIdSource == ImGuiInputSource_Nav)
		{
			...
			else if (delta != 0.0f)
			{
				...
				if ((clicked_t >= 1.0f && delta > 0.0f)
					|| (clicked_t <= 0.0f
						&& delta < 0.0f)) // This is to avoid applying the
										  // saturation when already past the
										  // limits
					set_new_value = false;
				else
		...
		if (set_new_value)
		{
			...
			// Round to user desired precision based on format string
			v_new = RoundScalarWithFormatT<TYPE, SIGNEDTYPE>(format, data_type,
															 v_new);
...
bool ImGui::SliderBehavior(const ImRect &bb, ImGuiID id,
						   ImGuiDataType data_type, void *v, const void *v_min,
						   const void *v_max, const char *format, float power,
						   ImGuiSliderFlags flags, ImRect *out_grab_bb)
{
	switch (data_type)
						 ...
						 const char *format, float power)
{
	...
	ImRect grab_bb;
	const bool value_changed
		= SliderBehavior(frame_bb, id, data_type, v, v_min, v_max, format,
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect &bb, ImGuiID id,
										   const char *label,
										   ImGuiDataType data_type,
										   void *data_ptr, const char *format)
{
	...
	DataTypeFormatString(data_buf, IM_ARRAYSIZE(data_buf), data_type, data_ptr,
						 format);
...
bool ImGui::InputScalar(const char *label, ImGuiDataType data_type,
						void *data_ptr, const void *step, const void *step_fast,
	...
	if (step != NULL)
	{
		...
		if (InputText("", buf, IM_ARRAYSIZE(buf),
			...
			value_changed = DataTypeApplyOpFromText(
				buf, g.InputTextState.InitialText.Data, data_type, data_ptr,
				format);
		PopItemWidth();
...
bool ImGui::InputScalarN(const char *label, ImGuiDataType data_type, void *v,
						 int components, const void *step,
						 const void *step_fast, const char *format,
						 ImGuiInputTextFlags flags)
{
	ImGuiWindow *window = GetCurrentWindow();
	if (window->SkipItems)
...
namespace ImGuiStb
{
	...
	static float STB_TEXTEDIT_GETWIDTH(STB_TEXTEDIT_STRING *obj,
									   int line_start_idx, int char_idx)
	{
...
#define STB_TEXTEDIT_MOVEWORDRIGHT STB_TEXTEDIT_MOVEWORDRIGHT_IMPL
	...
	static void STB_TEXTEDIT_DELETECHARS(STB_TEXTEDIT_STRING *obj, int pos,
										 int n)
	{
...
// misc/cpp/imgui_stdlib.h (FIXME: Rather messy function partly because we are
						...
						void *callback_user_data)
{
	...
	if (is_multiline)
	{
		...
		draw_window->DC.NavLayerActiveMaskNext
			|= draw_window->DC
				   .NavLayerCurrentMask; // This is to ensure that EndChild()
	...
	const bool user_scrolled
		= is_multiline && g.ActiveId == 0 && edit_state.ID == id
		  && g.ActiveIdPreviousFrame
				 == draw_window->GetIDNoKeepAlive("#SCROLLY");
	...
	if (g.ActiveId == id)
	{
		if (!is_editable && !g.ActiveIdIsJustActivated)
		{
			...
			const char *buf_end = NULL;
			edit_state.CurLenW
				= ImTextStrFromUtf8(edit_state.TextW.Data,
									edit_state.TextW.Size, buf, NULL, &buf_end);
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
	{
		...
		const bool is_shortcut_key
			= (is_osx ? (io.KeySuper && !io.KeyCtrl)
			  ...
			  && !io.KeyAlt && !io.KeyShift; // OS X style: Shortcuts using
		...
		else if (is_paste)
		{
			if (const char *clipboard = GetClipboardText())
			{
				...
				ImWchar *clipboard_filtered = (ImWchar *)MemAlloc(
				...
				!= 0)
			{
				...
				if (event_flag)
				{
					...
					if (callback_data.BufDirty)
					{
						...
						edit_state.CurLenA
							= callback_data
								  .BufTextLen; // Assume correct length and
											   // valid UTF-8 from user, saves
											   // us an extra strlen()
						edit_state.CursorAnimReset();
					}
				}
		...
		if (edit_state.StbState.select_start != edit_state.StbState.select_end)
		{
			...
			for (const ImWchar *p = text_selected_begin; p < text_selected_end;)
			{
				...
				if (rect_pos.y < clip_rect.y)
				{
					...
					// text_selected_end - p);  // FIXME-OPT: Could use this when
					// wchar_t are 16-bits p = p ? p + 1 : text_selected_end;
						  ...
						  | ImGuiInputTextFlags_CharsUppercase))
		{
			...
			if (alpha)
				sscanf(p, "%02X%02X%02X%02X", (unsigned int *)&i[0],
					   ...
					   (unsigned int
							*)&i[3]); // Treat at unsigned (%X is unsigned)
			else
									   ...
									   ImVec2 half_sz, float bar_w)
{
	ImGui::RenderArrowPointingAt(
		draw_list, ImVec2(pos.x + half_sz.x + 1, pos.y),
		ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Right, IM_COL32_BLACK);
						 ...
						 ImGuiColorEditFlags flags, const float *ref_col)
{
	...
	float sv_picker_size = ImMax(
		bars_width * 1, CalcItemWidth()
							- (alpha_bar ? 2 : 1)
								  * (bars_width
									 + style.ItemInnerSpacing
										   .x)); // Saturation/Value picking box
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel)
	{
		...
		InvisibleButton("hsv", ImVec2(sv_picker_size + style.ItemInnerSpacing.x
										  + bars_width,
									  sv_picker_size));
		if (IsItemActive())
		{
	...
	else if (flags & ImGuiColorEditFlags_PickerHueBar)
	{
		...
		if (IsItemActive())
		{
			S = ImSaturate((io.MousePos.x - picker_pos.x)
						   / (sv_picker_size - 1));
			V = 1.0f
				- ImSaturate((io.MousePos.y - picker_pos.y)
							 / (sv_picker_size - 1));
			value_changed = value_changed_sv = true;
		}
		if (!(flags & ImGuiColorEditFlags_NoOptions))
	...
	if (!(flags & ImGuiColorEditFlags_NoSidePreview))
	{
		...
		ColorButton(
			"##current", col_v4,
			(flags
			 & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview
				| ImGuiColorEditFlags_AlphaPreviewHalf
	...
	if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		if (flags & ImGuiColorEditFlags_HSV
			|| (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4("##hsv", col,
										sub_flags | ImGuiColorEditFlags_HSV);
		if (flags & ImGuiColorEditFlags_HEX
			|| (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4("##hex", col,
										sub_flags | ImGuiColorEditFlags_HEX);
		PopItemWidth();
	}
...
bool ImGui::TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
	...
	else
	{
		is_open = storage->GetInt(
					  id, (flags & ImGuiTreeNodeFlags_DefaultOpen) ? 1 : 0)
				  != 0;
	}
							 ...
							 const char *label, const char *label_end)
{
	...
	const float text_width
		= g.FontSize
		  + (label_size.x > 0.0f ? label_size.x + padding.x * 2
	...
	if (!is_leaf)
	{
		...
		if (g.NavId == id && g.NavMoveRequest && g.NavMoveDir == ImGuiDir_Right
			&& !is_open) // If there's something upcoming on the line we may
						 // want to give it the priority?
		{
			toggled = true;
			NavMoveRequestCancel();
		}
	...
	else
	{
		...
		if (flags & ImGuiTreeNodeFlags_Bullet)
			...
			RenderArrow(frame_bb.Min
							+ ImVec2(padding.x,
									 g.FontSize * 0.15f + text_base_offset_y),
...
bool ImGui::BeginMenu(const char *label, bool enabled)
{
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
	{
		...
		window->DC.CursorPos.x
			+= (float)(int)(style.ItemSpacing.x
							* (-1.0f
							   + 0.5f)); // -1 spacing to compensate the spacing
										 // added when Selectable() did a
										 // SameLine(). It would also work to
	...
	else
	{
		...
		RenderArrow(pos
						+ ImVec2(window->MenuColumns.Pos[2] + extra_w
									 + g.FontSize * 0.30f,
								 0.0f),
					ImGuiDir_Right);
		if (!enabled)
									 ...
									 // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		want_close = (menu_is_open && !hovered && g.HoveredWindow == window
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar)
{
	...
	for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
	{
		...
		width_total_contents += (tab_n > 0 ? g.Style.ItemInnerSpacing.x : 0.0f)
								+ tab->WidthContents;
		...
		&& (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown))
	{
		...
		if (tab_bar->Tabs.Size > 1)
			ImQsort(width_sort_buffer.Data, (size_t)width_sort_buffer.Size,
					sizeof(ImGuiTabBarSortItem), TabBarSortItemComparer);
...
static ImGuiTabItem *ImGui::TabBarScrollingButtons(ImGuiTabBar *tab_bar)
{
	...
	if (want_clip_rect)
		PushClipRect(
			tab_bar->BarRect.Min,
			tab_bar->BarRect.Max + ImVec2(g.Style.ItemInnerSpacing.x, 0.0f),
			true);
	...
	if (ArrowButtonEx(
			"##<", ImGuiDir_Left, arrow_button_size,
	...
	window->DC.CursorPos = ImVec2(
		...
		tab_bar->BarRect.Min.y);
	if (ArrowButtonEx(
			"##>", ImGuiDir_Right, arrow_button_size,
			ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat))
		select_dir = +1;
	PopStyleColor(2);
						 ...
						 ImGuiTabItemFlags flags)
{
	...
	IM_ASSERT(g.CurrentTabBar.Size > 0
...
void ImGui::EndTabItem()
{
	...
	IM_ASSERT(tab_bar->LastTabItemIdx >= 0
			  && "Needs to be called between BeginTabItem() and EndTabItem()");
	ImGuiTabItem *tab = &tab_bar->Tabs[tab_bar->LastTabItemIdx];
					  ...
					  ImGuiTabItemFlags flags)
{
	...
	bool just_closed = TabItemLabelAndCloseButton(display_draw_list, bb, flags,
												  tab_bar->FramePadding, label,
												  id, close_button_id);
	...
	if (g.HoveredId == id && !held && g.HoveredIdNotActiveTimer > 0.50f)
		if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip))
			SetTooltip("%.*s", (int)(FindRenderedTextEnd(label) - label),
					   label);
...
void ImGui::SetTabItemClosed(const char *label)
{
	...
	bool is_within_manual_tab_bar
		= (g.CurrentTabBar.Size > 0)
		  && !(g.CurrentTabBar.back()->Flags & ImGuiTabBarFlags_DockNode);
									   ...
									   ImGuiID tab_id, ImGuiID close_button_id)
{
	...
	if (flags & ImGuiTabItemFlags_UnsavedDocument)
	{
		...
		RenderTextClippedEx(draw_list, unsaved_marker_pos,
							bb.Max - frame_padding, TAB_UNSAVED_MARKER, NULL,
							NULL);
	}
	...
	if (label_size.x > text_ellipsis_clip_bb.GetWidth())
	{
		...
		float label_size_clipped_x
			= g.Font
				  ->CalcTextSizeA(
					  g.FontSize,
					  text_ellipsis_clip_bb.GetWidth() - ellipsis_width + 1.0f,
					  0.0f, label, label_display_end, &label_end)
				  .x;
		if (label_end == label
			&& label_end < label_display_end) // Always display at least 1
											  // character if there's no room
											  // for character + ellipsis
		{
			label_end
				= label
				  + ImTextCountUtf8BytesFromChar(label, label_display_end);
			label_size_clipped_x = g.Font
									   ->CalcTextSizeA(g.FontSize, FLT_MAX,
													   0.0f, label, label_end)
									   .x;
		}
			   ...
			   && ImCharIsBlankA(label_end[-1])) // Trim trailing space
		{
			...
			label_size_clipped_x
				-= g.Font
					   ->CalcTextSizeA(g.FontSize, FLT_MAX, 0.0f, label_end,
									   label_end + 1)
					   .x; // Ascii blanks are always 1 byte
		}
		RenderTextClippedEx(draw_list, text_pixel_clip_bb.Min,
							text_pixel_clip_bb.Max, label, label_end,
							&label_size, ImVec2(0.0f, 0.0f));
		...
		if (!close_button_visible && ellipsis_x + ellipsis_width <= bb.Max.x)
			RenderPixelEllipsis(draw_list,
								...
								ellipsis_dot_count, GetColorU32(ImGuiCol_Text));
	}
	else
	{
		RenderTextClippedEx(draw_list, text_pixel_clip_bb.Min,
							text_pixel_clip_bb.Max, label, label_display_end,
							&label_size, ImVec2(0.0f, 0.0f));
	}
