...
namespace ImGui
{
	class FileBrowser
	{
	  public:
		...
		// returns true when there is a selected filename and the "ok" button
		// was clicked
		...
		// returns selected filename. make sense only when HasSelected returns
		// true
	  ...
	  private:
		...
		// IMPROVE: overflow when selectedFilename_.length() >
...
inline ImGui::FileBrowser::FileBrowser( ImGuiFileBrowserFlags flags ) :
	flags_( flags ), openFlag_( false ), closeFlag_( false ),
	isOpened_( false ), ok_( false ),
...
inline void ImGui::FileBrowser::Display()
{
	...
	if ( openFlag_ ) OpenPopup( openLabel_.c_str() );
	isOpened_ = false;
...
#endif
		PushID( secIdx );
		if ( secIdx > 0 ) SameLine();
		if ( SmallButton( sec.u8string().c_str() ) ) newPwdLastSecIdx = secIdx;
		PopID();
		++secIdx;
	}
		...
		reserveHeight += GetItemsLineHeightWithSpacing();
	{
		...
		for ( auto & rsc : fileRecords_ )
		{
			...
			if ( !rsc.name.empty() && rsc.name[ 0 ] == '$' ) continue;
	...
	const std::filesystem::path & pwd )
{
	...
	for ( auto & p : std::filesystem::directory_iterator( pwd ) )
	{
		...
		rcd.showName
			= ( rcd.isDir ? "[D] " : "[F] " ) + p.path().filename().u8string();
