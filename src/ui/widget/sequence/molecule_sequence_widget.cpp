		...
		namespace Widget
		{
			namespace Sequence
			{
				...
				void MoleculeSequenceWidget::_setupSlots()
				{
					...
					connect(
						scrollBar, &QScrollBar::valueChanged, this, &MoleculeSequenceWidget::_onScrollBarValueChanged );
				}
				...
				{
					_sequenceLabel->setText( QString::fromStdString(
						_model->getPdbIdCode() + Style::SEQUENCE_CHAIN_NAME_SEPARATOR
						+ p_currentChainDisplayed.getName() + Style::SEQUENCE_CHAIN_NAME_SEPARATOR ) );
				}
				...
				void MoleculeSequenceWidget::refresh()
				{
					...
					for ( Model::Chain * const chain : _model->getChains() )
					{
						...
						chainNameWidget->setText( QString::fromStdString( Style::SEQUENCE_CHAIN_NAME_SEPARATOR
																		  + chain->getName()
						...
						ChainSequenceWidget * const chainSequenceDisplay
							= WidgetFactory::get().instantiateWidget<ChainSequenceWidget>( this,
																						   "chainSequenceWidget" );
				...
				void MoleculeSequenceWidget::mousePressEvent( QMouseEvent * p_event )
				{
					...
					_closestResidueFromStartPosition = _getClosestResidue(
						_startPressPosition, _startPressPosition.x() < _lastDragSelectionPosition.x(), true );
					_selectionModifier = _getSelectionModifier( p_event );

					const ClickModifier clickModifier = _getClickModifier( p_event );

					const bool targetWasSelected
						= _startResidueHovered != nullptr && _isSelected( _startResidueHovered );
						 ...
						 && !( _startResidueHovered == nullptr && _lastResidueHovered == nullptr ) )
					{
						...
						Model::Residue * const closestResidueFromPreviousClick
							= _getClosestResidue( _lastDragSelectionPosition,
												  _startPressPosition.x() > _lastDragSelectionPosition.x(),
												  true );
						...
						const bool select
							= _startResidueHovered == nullptr ? true : !_isSelected( _startResidueHovered );
						...
						if ( select )
						{
							...
							while ( previousResidue != nullptr && _isSelected( previousResidue ) )
							{
								...
								previousResidue			= searchForward
															  ? _getNextResidue( *firstResidueOfSelection, false )
				...
				void MoleculeSequenceWidget::_toggleSelect( std::vector<Model::Residue *> & p_residues ) const
				{
					...
					for ( std::vector<Model::Residue *>::const_iterator it = p_residues.cbegin();
						  it != p_residues.cend();
						  it++ )
					{
