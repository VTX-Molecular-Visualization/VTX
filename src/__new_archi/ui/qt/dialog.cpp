...
namespace VTX::UI::QT
{
	...
	{
		const int res = QMessageBox::warning( _getMainWindow(),
											  p_title,
											  p_message,
	...
	void Dialog::openDownloadMoleculeDialog() { Widget::Dialog::DownloadMoleculeDialog::openDialog(); }
	void Dialog::openDownloadMoleculeDialog( const QString & p_pdbCode )
	{
		Widget::Dialog::DownloadMoleculeDialog::openDialog( p_pdbCode );
	}
	...
	void Dialog::openLoadMoleculeDialog()
	{
		...
		const QStringList filenames = QFileDialog::getOpenFileNames(
			_getMainWindow(), "Open molecule", defaultPath, Util::Filesystem::LOAD_MOLECULE_FILTERS, defaultFilter );
	...
	void Dialog::openExportMoleculeDialog()
	{
		...
		const QString filename = QFileDialog::getSaveFileName( _getMainWindow(),
															   "Export molecule",
															   defaultPath,
	...
	void Dialog::openLoadTrajectoryDialog( Model::Molecule & p_target )
	{
		...
		// const QString filename = QFileDialog::getOpenFileName( _getMainWindow(),
		//													   "Open trajectory",
		//													   defaultPath,
	...
	void Dialog::openSaveSessionDialog( Worker::CallbackThread * const p_callback )
	{
		...
		const QString filename = QFileDialog::getSaveFileName(
			_getMainWindow(), "Save session", defaultPath, Util::Filesystem::SAVE_SCENE_FILTERS, defaultFilter );
	...
	void Dialog::openLoadSessionDialog()
	{
		...
		const QStringList filenames = QFileDialog::getOpenFileNames(
			_getMainWindow(), "Open session", defaultPath, Util::Filesystem::OPEN_FILE_FILTERS, defaultFilter );
	...
	void Dialog::openGLInitializationFail()
	{
		...
		QMessageBox::critical(
			_getMainWindow(), "Error", msg.c_str(), QMessageBox::StandardButton::Ok, QMessageBox::StandardButton::Ok );
	...
	MainWindow * Dialog::_getMainWindow() { return &dynamic_cast<MainWindow &>( QT_APP()->getMainWindow() ); }
