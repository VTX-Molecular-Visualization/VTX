...
void main()
{
    ...
    #define r inData.radii.x
    #define r2 inData.radii.y
    #define R inData.radii.z
    #define R2 inData.radii.w
    
    vec3 rayDirection = normalize(inData.viewImpPos);

    // Ray base point
    // Get close to the torus to limit approximation errors;
    vec3 rayOrigin = inData.viewImpPos.xyz - inData.viewTorusPosition - (R + r) * rayDirection;

    rayOrigin = inData.viewTorusRotationMatrix * rayOrigin;
    rayDirection = inData.viewTorusRotationMatrix * rayDirection;
    ...
    const float intersectionVSphereDistance = length( intersection - inData.osVisibilitySphere.xyz );
    if( intersectionVSphereDistance > inData.osVisibilitySphere.w  )
    {
    ...
    {
        const mat3 transposedVTorusRotMatrix = transpose(inData.viewTorusRotationMatrix);
        ...
        intersection += inData.viewTorusPosition.xyz;
        ...
        // Output data.
        outViewPositionNormal = viewPositionNormalCompressed;
        outColor			  = vec4( 1., 0.75, 0. , 32. ); // w = specular shininess.
    }
}
