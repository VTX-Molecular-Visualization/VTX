...
void main()
{
	...
	vec4 projVoxel
		= uProjMatrix
		  * vec4( outData.sphereRadius, outData.sphereRadius, outData.spherePosition.z, outData.spherePosition.w );
	vec2 projSize = s * projVoxel.xy / projVoxel.w;

	gl_Position = uProjMatrix * outData.spherePosition;
	// TODO check point size : max ?
