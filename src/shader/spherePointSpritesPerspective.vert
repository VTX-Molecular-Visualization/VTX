...
void main()
{
	...
	vec2 ndcTestXY = vec2( dot( camTest, uProjMatrix[ 0 ] ), dot( camTest, uProjMatrix[ 1 ] ) )
	...
	vec2 clipImpPosZW = ( camSpherePos.z + sphereRad ) * uProjMatrix[ 2 ].zw + uProjMatrix[ 3 ].zw;

	float ndcZ = clipImpPosZW.x / clipImpPosZW.y;

	gl_Position = vec4( ( minPS + maxPS ) * 0.5f, ndcZ, 1.f );

	gl_PointSize = ( max( abs( maxPS.x - minPS.x ) * 640, abs( maxPS.y - minPS.y ) * 360 ) );
