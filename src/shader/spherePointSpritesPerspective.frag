...
void main()
{
	vec4 coord = gl_FragCoord * vec4( 2.f / 1280.f, 2.f / 720.f, 2.0, 0.0 ) + vec4( -1.0, -1.0, -1.0, 1.0 );
	...
	float radicand = sphereRad * sphereRad - d2s;			  // square of difference of projected length and lambda
	if ( radicand < 0.0 )
	{
