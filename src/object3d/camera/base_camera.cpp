...
namespace VTX
{
	namespace Object3D
	{
		namespace Camera
		{
			...
			void BaseCamera::setScreenSize( const int p_width,
											const int p_height )
			{
			...
			{
				_fov = glm::clamp(
					_fov - p_delta, CAMERA_FOV_MIN, CAMERA_FOV_MAX );
			}

			void BaseCamera::_moveFront( const float delta )
			{
				_pos += delta * _front;
			}

			void BaseCamera::_moveLeft( const float delta )
			{
				_pos += delta * _left;
			}

			void BaseCamera::_moveUp( const float delta )
			{
				_pos += delta * _up;
			}
			...
			void BaseCamera::_update()
			{
				...
				_front				 = Vec3f( cosTheta * sinf( _phi ),
								  sinf( _theta ),
								  cosTheta * cosf( _phi ) );
				_left
					= Vec3f( sinf( _phi + PI_2f ), 0.f, cosf( _phi + PI_2f ) );
