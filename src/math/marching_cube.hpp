	...
	namespace Math
	{
		class MarchingCube
		{
		  public:
			...
			std::vector<std::vector<Vec3f>> triangulateCell( const GridCell & p_gridCell,
															 const float	  p_isovalue ) const;

		  private:
			uint _computeCubeIndex( const GridCell & p_gridCell, const float p_isovalue ) const;

			Vec3f _interpolate( const Vec3f & p_point1,
								const float	  p_val1,
								const Vec3f & p_point2,
								const float	  p_val2,
								const float	  p_isovalue ) const;

			std::vector<Vec3f> _getIntersectionCoordinates( const GridCell & p_gridCell,
															const float		 p_isovalue,
															const uint		 p_cubeIndex ) const;

			std::vector<std::vector<Vec3f>> _getTriangles( const std::vector<Vec3f> & p_intersections,
														   const uint				  p_cubeIndex ) const;
		}; // namespace Math
	}	   // namespace Math
} // namespace VTX
