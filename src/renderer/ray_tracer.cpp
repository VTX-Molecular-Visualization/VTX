...
namespace VTX
{
	namespace Renderer
	{
		...
		{
			const Object3D::Camera & camera = p_scene.getCamera();
			...
			const uint numTilesX = ( _width + TILE_SIZE - 1 ) / TILE_SIZE;
			const uint numTilesY = ( _height + TILE_SIZE - 1 ) / TILE_SIZE;
			...
			{
				const uint threadId = 0;
				_renderTile( pixels, camera, nbPixelSamples, i, threadId, numTilesX, numTilesY );
			}
		...
		{
			const uint tileY = p_taskIndex / p_numTilesX;
			const uint tileX = p_taskIndex - tileY * p_numTilesX;
			const uint x0	 = tileX * TILE_SIZE;
			const uint x1	 = glm::min( x0 + TILE_SIZE, _width );
			const uint y0	 = tileY * TILE_SIZE;
			const uint y1	 = glm::min( y0 + TILE_SIZE, _height );
		...
		Vec3f RayTracer::_renderPixel( const Object3D::Camera & p_camera,
									   const float				p_x,
									   const float				p_y,
									   const uint				p_nbPixelSamples )
		{
			...
			const Vec3f & camPos   = p_camera.getPosition();
			...
			const Vec3f downLeftCorner = camPos + halfWidth * camLeft - halfHeight * camUp + camFront;
			const Vec3f du			   = -2.f * halfWidth * camLeft;
			const Vec3f dv			   = 2.f * halfHeight * camUp;
			...
			for ( uint s = 0; s < p_nbPixelSamples; s++ )
			{
				...
				const Ray ray( p_camera.getPosition(),
							   glm::normalize( downLeftCorner + ( sx / _width ) * du
											   + ( ( _height - sy ) / _height ) * dv - camPos ) );
