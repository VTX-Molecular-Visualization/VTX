...
namespace TNT
{
	...
	class Array2D
	{
	  ...
	  public:
		...
		inline void displayForDebug()
		{
			const T SVD_W00 = v_[ 0 ][ 0 ];
			const T SVD_W01 = v_[ 0 ][ 1 ];
			const T SVD_W02 = v_[ 0 ][ 2 ];
			const T SVD_W10 = v_[ 1 ][ 0 ];
			const T SVD_W11 = v_[ 1 ][ 1 ];
			const T SVD_W12 = v_[ 1 ][ 2 ];
			const T SVD_W20 = v_[ 2 ][ 0 ];
			const T SVD_W21 = v_[ 2 ][ 1 ];
			const T SVD_W22 = v_[ 2 ][ 2 ];
	...
	template<class T>
	Array2D<T>::Array2D( const Array2D<T> & A ) : data_( A.data_ ), v_( A.v_ ), m_( A.m_ ), n_( A.n_ )
	{
	}
	...
	template<class T>
	Array2D<T>::Array2D( int m, int n, const T & val ) : data_( m * n ), v_( m ), m_( m ), n_( n )
	{
		if ( m > 0 && n > 0 )
		{
			data_ = val;
