...
{
	const vec3 rd = uniformsCamera.isCameraPerspective == 1 ? gsData.viewImpPos : vec3( 0.f, 0.f, -1.f );
...
float sdToroidalPatch( const in vec3 point, out vec3 closestPoint )
{
	...
	return -distance( point, closestPoint ) + uniformsRepresentation[ gsCircle.representation ].SESProbeRadius;
}
