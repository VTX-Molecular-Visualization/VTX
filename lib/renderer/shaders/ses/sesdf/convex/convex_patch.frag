...
layout( std140, binding = 2 ) restrict readonly buffer Segments { uvec4 segments[]; };

layout( std140, binding = 3 ) readonly buffer Probes { vec4 probes[]; };

layout( std140, binding = 6 ) readonly buffer Sectors { vec4 sectors[]; };
...
{
	const float probeRadius = uniformsRepresentation[ gsPatchData.representation ].SESProbeRadius;
	c1.w += probeRadius;
	c2.w += probeRadius;
...
bool isInPatch( const vec3 viewP )
{
	...
	const vec3 centerToPoint = ( uniformsModel[ gsPatchData.model ].matrixModelViewInv
								 * vec4( ( viewP - gsPatchData.vAtomData.xyz ) / gsPatchData.wsAtomData.w, 0. ) )
								   .xyz;
	bool isIn = true;
...
void handleImpostor()
{
	...
	outColor = vec4( gsPatchData.color.rgb, 1.f );
