...
const vec3	directions[ NUM_DIRECTIONS ]
	= vec3[ NUM_DIRECTIONS ]( vec3( 1.0f, 0.0f, 0.0f ), vec3( 0.0f, 1.0f, 0.0f ), vec3( 1.0f, 1.0f, 0.0f ) );
...
void getVisibility( vec3 position, vec3 direction, float radius, vec3 projectedNormal, vec3 V )
{
	...
	for ( int i = -NUM_STEPS; i < NUM_STEPS; i++ )
	{
		...
		// z is a sum because position.z is negative, and samplePosFrontDepth is positive (depth buffer is positive,
		// data is negative)
		...
		localIllumination
			+= sampleColor * unoccluded * abs( dot( horizVec, projectedNormal ) * dot( horizVec, -normal_sample ) );
