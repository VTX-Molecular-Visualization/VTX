...
namespace bcs
{
	...
	namespace
	{
		__global__ void copyUint2Add(
			uint2 * const		dst,
			const uint2 * const src,
		...
		)
		{
			...
			{
				const uint2 value = src[ i ];
				dst[ i ]		  = make_uint2( value.x + xOffset, value.y + yOffset );
			}
		}

		__global__ void copyUint4Add(
			uint4 * const		dst,
		...
		)
		{
			for ( uint32_t i = blockIdx.x * blockDim.x + threadIdx.x; i < count; i += blockDim.x * gridDim.x )
			{
				...
				dst[ i ] = make_uint4( value.x + xOffset, value.y + yOffset, value.z + zOffset, value.w + wOffset );
			}
		}

		__global__ void copyInt4Add(
			int4 * const	   dst,
			const int4 * const src,
			const uint32_t	   count,
			const int		   xyzOffset
		)
		{
			for ( uint32_t i = blockIdx.x * blockDim.x + threadIdx.x; i < count; i += blockDim.x * gridDim.x )
			{
				const int4 value = src[ i ];
				dst[ i ]		 = make_int4( value.x + xyzOffset, value.y + xyzOffset, value.z + xyzOffset, value.w );
			}
		}
		...
		__global__ void copyProbeNeighbors(
			...
			const int4 * const	 probeIds,
			const uint32_t		 probeCount,
			const uint32_t		 srcNeighborStride,
			const uint32_t		 dstNeighborStride
		)
		{
			for ( uint32_t probeIndex = blockIdx.x * blockDim.x + threadIdx.x; probeIndex < probeCount;
				  probeIndex += blockDim.x * gridDim.x )
			{
				const uint32_t neighborCount = uint32_t( probeIds[ probeIndex ].w );

				for ( uint32_t neighborIndex = 0; neighborIndex < neighborCount; ++neighborIndex )
				{
					const uint32_t srcIndex = probeIndex * srcNeighborStride + neighborIndex;
					const uint32_t dstIndex = probeIndex * dstNeighborStride + neighborIndex;
		...
		void launchCopyKernel( const uint32_t count, KernelFunc kernel, Args... args )
		{
			if ( count == 0 )
			{
				return;
			...
			auto [ blockNb, threadNb ] = KernelConfig::From( count, 256 );
			kernel<<<blockNb, threadNb>>>( args... );
			cudaCheck( "SES external buffer write failed" );
		}
	} // namespace
	...
	{
		{
			...
			*sesContext.hVisibleCircleNb = 0;
			...
			if ( *sesContext.hIntersectedCircleNb > 0 )
			{
				...
				auto [ dIntersections, startandIdList ]
					= sesdf::findIntersections<MaxNeighborPerAtom>( sesContext, m_graphics );
	...
	void Sesdf::writeData( const sesdf::SesdfWriteBuffers & output )
	{
		...
		if ( m_convexPatchNb > 0 )
		{
			launchCopyKernel(
				...
				m_dConvexPatches.get<uint2>(),
				m_convexPatchNb,
				output.sectorIndexOffset,
				output.sectorIndexOffset
			);
		}

		const std::size_t sectorPadding
			= m_fullCircleNb * sizeof( uint2 ) + ( ( m_fullCircleNb * sizeof( uint2 ) ) % sizeof( float4 ) );
		float4 * sectors = reinterpret_cast<float4 *>( m_dFCircleAndSectors.get() + sectorPadding );
		if ( m_sectorNb > 0 && output.sectors != nullptr )
		{
			cudaCheck(
				"SES sectors external buffer write failed",
				cudaMemcpy( output.sectors, sectors, m_sectorNb * sizeof( float4 ), cudaMemcpyDeviceToDevice )
			);
		}
		...
		if ( m_fullCircleNb > 0 )
		{
			launchCopyKernel(
				...
				m_dFCircleAndSectors.get<uint2>(),
				m_fullCircleNb,
				output.atomIndexOffset,
				output.atomIndexOffset
			);
		}
		...
		if ( m_segmentNb > 0 )
		{
			launchCopyKernel(
				...
				m_dSegments.get<uint4>(),
				m_segmentNb,
				output.atomIndexOffset,
				output.atomIndexOffset,
				output.probeIndexOffset,
				output.probeIndexOffset
			);
		}

		if ( m_intersectionNb > 0 )
		{
			float4 * concavePatchPositions
				= reinterpret_cast<float4 *>( m_dIntersections.get() + m_intersectionNb * sizeof( float4 ) );
			if ( output.concavePatchesPosition != nullptr )
			{
				cudaCheck(
					"SES probes external buffer write failed",
					cudaMemcpy(
						output.concavePatchesPosition,
						concavePatchPositions,
						m_intersectionNb * sizeof( float4 ),
