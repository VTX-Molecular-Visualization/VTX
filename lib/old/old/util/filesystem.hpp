...
namespace VTX
{
	namespace Util
	{
		namespace Filesystem
		{
			...
			inline FILE_TYPE_ENUM getFileTypeEnum( const Util::FilePath & p_path )
			{
											 ...
											 std::vector<std::vector<Util::FilePath>> & p_filepathPerMode )
			{
				...
				for ( const Util::FilePath & path : p_filepaths )
				{
			...
			inline static Util::FilePath getFilePathFromQUrl( const QUrl & p_url )
			{
				return Util::FilePath( p_url.toLocalFile().toStdString() );
			}
			inline static std::vector<Util::FilePath> getFilePathVectorFromQUrlList( const QList<QUrl> & p_urls )
			{
				std::vector<Util::FilePath> res = std::vector<Util::FilePath>();
			...
			static const Util::FilePath STYLESHEET_FILE_WINDOWS = Util::FilePath( ":/stylesheet_windows.css" );
			static const Util::FilePath STYLESHEET_FILE_LINUX	= Util::FilePath( ":/stylesheet_linux.css" );
