...
namespace VTX::View::UI::Widget::Measurement
{
	AngleSceneView::AngleSceneView( Model::Measurement::Angle * const p_model, QWidget * const p_parent ) :
		App::Old::Core::View::BaseView<Model::Measurement::Angle>( p_model ), SceneItemWidget( p_parent )
	{
	}

	void AngleSceneView::notify( const App::Old::Core::Event::VTXEvent * const p_event )
	{
	...
	void AngleSceneView::_setupSlots()
	{
		SceneItemWidget::_setupSlots();

		connect( this, &QTreeWidget::itemChanged, this, &AngleSceneView::_onItemChanged );
		connect( this, &QTreeWidget::itemDoubleClicked, this, &AngleSceneView::_onItemDoubleClicked );
	}

	void AngleSceneView::_onItemChanged( QTreeWidgetItem * const p_item, const int p_column )
	{
		if ( p_column == 0 )
		{
	...
	void AngleSceneView::_onItemDoubleClicked( const QTreeWidgetItem * const p_item, const int p_column ) const
	{
		if ( p_column == 0 )
		{
	...
	void AngleSceneView::_fillItemSelection( const App::Old::Application::Selection::SelectionModel & p_selection,
											 QItemSelection &									 p_itemSelection )
	{
		...
		p_selection.getItemsOfType( App::Old::ID::Model::MODEL_MEASUREMENT_ANGLE, selectedLabels );
	...
	void AngleSceneView::_onCustomContextMenuCalled( const QPoint & p_clicPos )
	{
