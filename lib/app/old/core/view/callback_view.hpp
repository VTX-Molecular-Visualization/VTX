...
namespace VTX::App::Old::Core::View
{
	...
	class CallbackView : public App::Old::Core::View::BaseView<T1>
	{
		VTX_VIEW

	  public:
		void setCallback( T2 * const p_caller,
						  void ( T2::*p_notifyCallback )( const App::Old::Core::Event::VTXEvent * const ) )
		{
			_notifyTarget	= p_caller;
			_notifyCallback = p_notifyCallback;
		}
		void setCallback( T2 * const p_caller,
						  void ( T2::*p_notifyCallback )( const T1 * const p_model,
														  const App::Old::Core::Event::VTXEvent * const ) )
		{
			_notifyTarget			 = p_caller;
	  ...
	  protected:
		CallbackView( T1 * const p_model ) : App::Old::Core::View::BaseView<T1>( p_model ) {};

	  private:
		T2 * _notifyTarget														  = nullptr;
		void ( T2::*_notifyCallback )( const App::Old::Core::Event::VTXEvent * const ) = nullptr;
		void ( T2::*_notifyWithModelCallback )( const T1 * const p_model, const App::Old::Core::Event::VTXEvent * const )
			= nullptr;
	};
