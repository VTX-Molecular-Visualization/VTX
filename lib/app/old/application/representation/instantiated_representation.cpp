...
namespace VTX::App::Old::Application::Representation
{
	...
	InstantiatedRepresentation::InstantiatedRepresentation() :
		BaseModel( App::Old::ID::Model::MODEL_INTANTIATED_REPRESENTATION )
	{
	}
	...
	{
		_registerEvent( VTX::App::Old::Event::Global::MOLECULE_COLOR_CHANGE );
	}

	InstantiatedRepresentation::~InstantiatedRepresentation() {}

	void InstantiatedRepresentation::onLinkedRepresentationChange( const App::Old::Core::Event::VTXEvent * const p_event )
	{
		if ( p_event->name == App::Old::Event::Model::REPRESENTATION_TYPE_CHANGE
			 || p_event->name == App::Old::Event::Model::DATA_CHANGE )
		{
			if ( _target != nullptr )
				_updateTarget( MOLECULE_COMPUTATION_FLAG::ENUM::ALL );
		}

		_notifyViews( new App::Old::Core::Event::VTXEvent( *p_event ) );
	}

	void InstantiatedRepresentation::receiveEvent( const App::Old::Core::Event::VTXEvent & p_event )
	{
		if ( p_event.name == VTX::App::Old::Event::Global::MOLECULE_COLOR_CHANGE )
		{
			bool useMoleculeColor = false;

			if ( _colorMode.isValid() )
			{
				useMoleculeColor = useMoleculeColor
								   || _colorMode.getValue() == VTX::Core::ChemDB::Color::COLOR_MODE::ATOM_PROTEIN
								   || _colorMode.getValue() == VTX::Core::ChemDB::Color::COLOR_MODE::PROTEIN;
			}

			if ( _ribbonData.isValid() )
			{
				useMoleculeColor = useMoleculeColor
								   || _ribbonData.getValue().colorMode
										  == VTX::Core::ChemDB::Color::SECONDARY_STRUCTURE_COLOR_MODE::PROTEIN;
			}
	...
	void InstantiatedRepresentation::setLinkedRepresentation( const RepresentationPreset * const p_linkedRepresentation,
	...
	const BaseRepresentable * const InstantiatedRepresentation::getConstTarget() const { return _target; }
	BaseRepresentable * const		InstantiatedRepresentation::getTarget() const { return _target; };
	...
	void InstantiatedRepresentation::setColorMode( const VTX::Core::ChemDB::Color::COLOR_MODE & p_colorMode,
												   ...
												   const bool									p_notify )
	{
		...
		if ( p_recomputeBuffers )
			_updateTarget( MOLECULE_COMPUTATION_FLAG::ENUM::ColorBuffer );
	...
	void InstantiatedRepresentation::setSphereRadius( const float p_radius, const bool p_notify )
	{
		...
		case REPRESENTATION_ENUM::CARTOON:
		case REPRESENTATION_ENUM::SES:
			// !V0.1
			// case REPRESENTATION_ENUM::TRACE:
			break;

		case REPRESENTATION_ENUM::COUNT:
		default: break;
		}
	...
	void InstantiatedRepresentation::setCylinderRadius( const float p_radius, const bool p_notify )
	{
		...
		case REPRESENTATION_ENUM::SAS:
		case REPRESENTATION_ENUM::SES:
		case REPRESENTATION_ENUM::COUNT: break;
		}
	...
	void InstantiatedRepresentation::setRibbonColorMode(
	...
	void InstantiatedRepresentation::applyData( const InstantiatedRepresentation & p_source,
												...
												const bool						   p_notify )
	{
		if ( p_flag & MEMBER_FLAG::ENUM::SPHERE_RADIUS_FIXED && p_source.hasToDrawSphere() )
			_sphereData.getValue().radiusFixed = p_source.getSphereData().radiusFixed;

		if ( p_flag & MEMBER_FLAG::ENUM::SPHERE_RADIUS_ADD && p_source.hasToDrawSphere() )
			_sphereData.getValue().radiusAdd = p_source.getSphereData().radiusAdd;

		if ( p_flag & MEMBER_FLAG::ENUM::CYLINDER_RADIUS && p_source.hasToDrawCylinder() )
			_cylinderData.getValue().radius = p_source.getCylinderData().radius;

		if ( p_flag & MEMBER_FLAG::ENUM::CYLINDER_COLOR_BLENDING_MODE && p_source.hasToDrawCylinder() )
			_cylinderData.getValue().colorBlendingMode = p_source.getCylinderData().colorBlendingMode;

		if ( p_flag & MEMBER_FLAG::ENUM::RIBBON_COLOR_MODE && p_source.hasToDrawRibbon() )
			_ribbonData.getValue().colorMode = p_source.getRibbonData().colorMode;

		if ( p_flag & MEMBER_FLAG::ENUM::RIBBON_COLOR_BLENDING_MODE && p_source.hasToDrawRibbon() )
			_ribbonData.getValue().colorBlendingMode = p_source.getRibbonData().colorBlendingMode;

		if ( p_flag & MEMBER_FLAG::ENUM::SES_RESOLUTION && p_source.hasToDrawSES() )
			_sesData.getValue().resolution = p_source.getSESData().resolution;

		if ( p_flag & MEMBER_FLAG::ENUM::COLOR_MODE )
			setColorMode( p_source.getColorMode(), p_recomputeBuffers, false );

		if ( p_flag & MEMBER_FLAG::ENUM::COLOR )
	...
	{
		MEMBER_FLAG res = MEMBER_FLAG::ENUM::NONE;

		if ( hasToDrawSphere() && _sphereData.isOverrided() )
		{
			res = res | MEMBER_FLAG::ENUM::SPHERE_RADIUS_ADD | MEMBER_FLAG::ENUM::SPHERE_RADIUS_FIXED;
		}

		if ( hasToDrawCylinder() && _cylinderData.isOverrided() )
		{
			res = res | MEMBER_FLAG::ENUM::CYLINDER_RADIUS | MEMBER_FLAG::ENUM::CYLINDER_COLOR_BLENDING_MODE;
		}

		if ( hasToDrawRibbon() && _ribbonData.isOverrided() )
		{
			res = res | MEMBER_FLAG::ENUM::RIBBON_COLOR_MODE | MEMBER_FLAG::ENUM::RIBBON_COLOR_BLENDING_MODE;
		}

		if ( hasToDrawSES() && _sesData.isOverrided() )
		{
			res = res | MEMBER_FLAG::ENUM::SES_RESOLUTION;
		}

		if ( _colorMode.isOverrided() )
		{
			res = res | MEMBER_FLAG::ENUM::COLOR_MODE;
		}

		if ( _color.isOverrided() )
		{
			res = res | MEMBER_FLAG::ENUM::COLOR;
		}
	...
	bool InstantiatedRepresentation::isMemberOverrided( const MEMBER_FLAG & p_member ) const
	{
		switch ( p_member )
		{
		case MEMBER_FLAG::ENUM::SPHERE_RADIUS_FIXED: return _sphereData.isOverrided(); break;
		case MEMBER_FLAG::ENUM::SPHERE_RADIUS_ADD: return _sphereData.isOverrided(); break;
		case MEMBER_FLAG::ENUM::CYLINDER_RADIUS: return _cylinderData.isOverrided(); break;
		case MEMBER_FLAG::ENUM::CYLINDER_COLOR_BLENDING_MODE: return _cylinderData.isOverrided(); break;
		case MEMBER_FLAG::ENUM::RIBBON_COLOR_MODE: return _ribbonData.isOverrided(); break;
		case MEMBER_FLAG::ENUM::RIBBON_COLOR_BLENDING_MODE: return _ribbonData.isOverrided(); break;
		case MEMBER_FLAG::ENUM::SES_RESOLUTION: return _sesData.isOverrided(); break;
		case MEMBER_FLAG::ENUM::COLOR: return _color.isOverrided(); break;
		case MEMBER_FLAG::ENUM::COLOR_MODE: return _colorMode.isOverrided(); break;

		default:
			VTX_WARNING( "MEMBER_FLAG {}  not managed in InstantiatedRepresentation::isMemberOverrided.",
						 int( p_member ) );
			return false;
		}
	}

	void InstantiatedRepresentation::_updateTarget( const MOLECULE_COMPUTATION_FLAG & p_flag )
	{
		if ( _target == nullptr )
		{
			return;
		...
		if ( p_flag & MOLECULE_COMPUTATION_FLAG::ENUM::Targets )
		{
			_target->getMolecule()->computeRepresentationTargets();
		}

		if ( p_flag & MOLECULE_COMPUTATION_FLAG::ENUM::ColorBuffer )
		{
