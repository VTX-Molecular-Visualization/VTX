#include "app/old/render/renderer/gl/pass/shading.hpp"
...
namespace VTX::App::Old::Render::Renderer::GL::Pass
{
	void Shading::init( const uint p_width, const uint p_height, const GL & )
	{
		...
		_diffuseShading
			= VTX_PROGRAM_MANAGER().createProgram( "DiffuseShading", { FilePath( "shading/shading_diffuse.frag" ) } );
		_glossyShading
	...
	void Shading::render( const App::Old::Application::Scene & p_scene, const GL & p_renderer )
	{
		_fbo.bind();
