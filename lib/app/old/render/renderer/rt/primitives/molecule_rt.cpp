...
#include "app/old/render/renderer/rt/materials/metal.hpp"
#include "app/old/render/renderer/rt/materials/phong_material.hpp"
...
{
	MoleculeRT::MoleculeRT( const App::Old::Component::Chemistry::Molecule * p_molecule )
	{
		const App::Old::Application::Representation::REPRESENTATION_ENUM rep
			= //
			  // App::Old::Application::Representation::REPRESENTATION_ENUM::SAS;
			App::Old::Application::Representation::REPRESENTATION_ENUM::VAN_DER_WAALS;
		// App::Old::Application::Representation::REPRESENTATION_ENUM::BALL_AND_STICK;
		// App::Old::Application::Representation::REPRESENTATION_ENUM::STICK;
		// VTX::App::Old::Application::Setting::Rendering::representation;
		...
		std::vector<Primitive::BasePrimitive *> primitives;
		if ( rep == App::Old::Application::Representation::REPRESENTATION_ENUM::BALL_AND_STICK
			 || rep == App::Old::Application::Representation::REPRESENTATION_ENUM::STICK )
		{
		...
		// std::map<App::Old::Component::Chemistry::Chain *, Vec3f> mapColors;
		...
		std::map<App::Old::Component::Chemistry::Chain *, Material::BaseMaterial *> mapMtls;
		...
		float radius = rep == App::Old::Application::Representation::REPRESENTATION_ENUM::BALL_AND_STICK ? 0.4f : 0.25f;

		for ( uint i = 0; i < nbAtoms; ++i )
		{
			if ( p_molecule->isAtomVisible( i ) )
			{
				App::Old::Component::Chemistry::Chain * chainPtr = p_molecule->getAtom( i )->getChainPtr();

				if ( mapMtls.find( chainPtr ) == mapMtls.end() )
				{
					_materials.emplace_back( //
						new Material::MatteMaterial( chainPtr->getColor(), 0.3f ) );
					// new PhongMaterial( chainPtr->getColor() * 0.7f, chainPtr->getColor() * 0.3f, 64.f ) );
					mapMtls[ chainPtr ] = _materials.back();
				}

				primitives.emplace_back(
					new Primitive::Sphere( tAtomPositions[ i ],
										   rep == App::Old::Application::Representation::REPRESENTATION_ENUM::VAN_DER_WAALS
											   ? p_molecule->getAtomRadius( i )
										   : rep == App::Old::Application::Representation::REPRESENTATION_ENUM::SAS
			 ...
			 || rep == App::Old::Application::Representation::REPRESENTATION_ENUM::STICK )
		{
			...
			{
				const App::Old::Component::Chemistry::Bond & bond = *p_molecule->getBond( i );
				const Vec3f &							a1	 = tAtomPositions[ bond.getIndexFirstAtom() ];
				const Vec3f &							a2	 = tAtomPositions[ bond.getIndexSecondAtom() ];

				primitives.emplace_back( new Primitive::Cylinder(
		...
		//	const App::Old::Component::Chemistry::Residue & r = p_molecule->getResidue( i );
		...
		const App::Old::Component::Object3D::Helper::AABB & aabb		= _bvh.getAABB();
