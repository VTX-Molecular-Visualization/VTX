...
#include "app/old/internal/math/sampler.hpp"
										...
										const float	  p_tMax ) const
	{
		...
		if ( p_scene.intersect( p_ray, p_tMin, p_tMax, intersection ) )
		{
			...
			for ( uint i = 0; i < _nbSamples; ++i )
			{
				...
				Vec3f sampleDir = App::Old::Internal::Math::Sampler::cosineWeightedHemisphere( u, v );
				float samplePdf = App::Old::Internal::Math::Sampler::cosineWeightedHemispherePdf( u );
