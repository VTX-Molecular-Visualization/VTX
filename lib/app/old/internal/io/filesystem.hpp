#ifndef __VTX_APP_OLD_INTERNAL_IO_FILESYSTEM__
#define __VTX_APP_OLD_INTERNAL_IO_FILESYSTEM__
...
namespace VTX::App::Old::Internal::IO::Filesystem
{
	static FilePath EXECUTABLE_ABSOLUTE_PATH = Util::Filesystem::getExeDirectory();
	...
	inline const FilePath getUniqueSnapshotsPath( const Internal::IO::Serialization::ImageExport::Format p_format )
	{
		std::string extension;

		switch ( p_format )
		{
		case Internal::IO::Serialization::ImageExport::Format::PNG: extension = "png"; break;
		case Internal::IO::Serialization::ImageExport::Format::JPEG: extension = "jpg"; break;
		case Internal::IO::Serialization::ImageExport::Format::BMP: extension = "bmp"; break;
		default:
	...
	{
		const FilePath defaultFolder = FilePath( VTX::App::Old::Application::Setting::getLastExportedMoleculeFolder() );

		const int nbMoleculeInSelection
			= App::Old::Application::Selection::SelectionManager::get().getSelectionModel().getMoleculeSelectedCount();

		const App::Old::Component::Chemistry::Molecule * exportedMolecule;
		if ( nbMoleculeInSelection > 0 )
		{
			const App::Old::Core::Model::ID & moleculeID = App::Old::Application::Selection::SelectionManager::get()
															   .getSelectionModel()
															   .getMoleculesMap()
															   .begin()
															   ->first;
			exportedMolecule = &( VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Molecule>( moleculeID ) );
		}
		else if ( App::Old::VTXApp::get().getScene().getMolecules().size() > 0 )
		{
			exportedMolecule = App::Old::VTXApp::get().getScene().getMolecules().begin()->first;
		}
		else
		{
	...
	{
		if ( !App::Old::VTXApp::get().getScenePathData().getCurrentPath().empty() )
		{
			return App::Old::VTXApp::get().getScenePathData().getCurrentPath();
		}

		const FilePath defaultFolder = FilePath( VTX::App::Old::Application::Setting::getLastSavedSessionFolder() );
	...
	inline std::string getImageExportDefaultFilter()
	{
		...
		{
		case App::Old::Internal::IO::Serialization::ImageExport::Format::PNG: filter = "PNG (*.png)"; break;
		case App::Old::Internal::IO::Serialization::ImageExport::Format::JPEG: filter = "JPEG (*.jpg *.jpeg)"; break;
		case App::Old::Internal::IO::Serialization::ImageExport::Format::BMP: filter = "BMP (*.bmp)"; break;
