...
{
	void LibAssimp::readFile( const FilePath & p_path, App::Old::Component::Object3D::MeshTriangle & p_mesh )
	{
		/*
		...
		for ( uint m = 0; m < nbMeshes; ++m )
		{
			...
			for ( uint f = 0; f < mesh->mNumFaces; ++f, ++currentTriangle )
			{
				...
				// App::Old::Component::Object3D::MeshTriangle::Triangle & tri = p_mesh.getTriangle( currentTriangle
		);
	...
	void LibAssimp::readFile( const FilePath & p_path, App::Old::Component::Chemistry::Molecule & p_molecule )
	{
		/*
		Assimp::Importer Importer;
		...
		App::Old::Component::Chemistry::Molecule::AtomPositionsFrame & frame = p_molecule.getAtomPositionFrame( 0 );
		...
		for ( uint chainIdx = 0; chainIdx < chainCount; ++chainIdx, ++chainGlobalIdx )
		{
			...
			// New chain.
			p_molecule.addChain();
			App::Old::Component::Chemistry::Chain & chain = *p_molecule.getChain( chainGlobalIdx );
			...
			for ( uint residueIdx = 0; residueIdx < mesh->mNumFaces; ++residueIdx, ++residueGlobalIdx )
			{
				...
				App::Old::Component::Chemistry::Residue * const residue = p_molecule.getResidue( residueGlobalIdx );
					  ...
					  ++atomIdx, ++atomGlobalIdx, ++bondGlobalIdx )
				{
					...
					// New atom.
					p_molecule.addAtom();
					App::Old::Component::Chemistry::Atom & atom = *p_molecule.getAtom( atomGlobalIdx );
					atom.setResiduePtr( residue );
					atom.setIndex( atomGlobalIdx );
					atom.setSymbol( App::Old::Component::Chemistry::Atom::SYMBOL::UNKNOWN );
					...
					// Bond.
					p_molecule.addBond();
					App::Old::Component::Chemistry::Bond & bond = *p_molecule.getBond( atomGlobalIdx );
