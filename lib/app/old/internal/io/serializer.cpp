...
namespace VTX::App::Old::Internal::IO
{
	...
	{
		const FilePath moleculePath = App::Old::VTXApp::get().getScenePathData().getFilepath( &p_molecule );

		const Writer::ChemfilesWriter * const writer
			= App::Old::VTXApp::get().getScenePathData().getData( &p_molecule ).getWriter();
		...
		if ( p_json.contains( "LABELS" ) )
		{
			for ( const nlohmann::json & jsonLabel : p_json.at( "LABELS" ) )
			{
				if ( jsonLabel.contains( "LABEL" ) )
				{
					...
					{
						App::Old::VTX_ID typeID = jsonLabelData.at( "TYPE_ID" ).get<VTX_ID>();
								  ...
								  Component::Chemistry::Molecule &	   p_molecule ) const
	{
		...
		if ( molPath.is_relative() )
		{
			const FilePath sceneFolder = Internal::IO::Filesystem::getSceneSaveDirectory(
				App::Old::VTXApp::get().getScenePathData().getCurrentPath() );
		...
		try
		{
			...
			App::Old::VTXApp::get().getScenePathData().registerLoading( &p_molecule, molPath );
		}
	...
	const Component::Chemistry::Atom * Serializer::deserializeAtomReference( const nlohmann::json & p_json ) const
	{
		...
		const Application::Scene::MapMoleculePtrFloat & sceneMolecules = App::Old::VTXApp::get().getScene().getMolecules();
																  ...
																  const Writer::ChemfilesWriter * p_writer ) const
	{
		...
		nlohmann::json jsonRepresentation		= { { "TARGET_TYPE", App::Old::ID::Model::MODEL_MOLECULE },
													{ "INDEX", 0 },
		...
		for ( const Component::Chemistry::Chain * const chain : p_molecule.getChains() )
		{
			...
			{
				jsonRepresentation = { { "TARGET_TYPE", App::Old::ID::Model::MODEL_CHAIN },
			...
			for ( uint residueIndex = 0; residueIndex < chain->getResidueCount(); residueIndex++ )
			{
				...
				if ( residue->hasCustomRepresentation() )
				{
					...
					jsonRepresentation = { { "TARGET_TYPE", App::Old::ID::Model::MODEL_RESIDUE },
														  ...
														  Component::Chemistry::Molecule &	   p_molecule ) const
	{
		for ( const nlohmann::json & jsonRepresentations : p_json )
		{
			...
			const App::Old::VTX_ID type	= jsonRepresentations.at( "TARGET_TYPE" ).get<VTX_ID>();
			const uint		  index = jsonRepresentations.at( "INDEX" ).get<uint>();

			const bool dataValid = ( type == App::Old::ID::Model::MODEL_MOLECULE )
								   || ( type == App::Old::ID::Model::MODEL_CHAIN && index < p_molecule.getChainCount() )
								   || ( type == App::Old::ID::Model::MODEL_RESIDUE && index < p_molecule.getResidueCount() );
