...
namespace VTX::App::Old::Internal::IO::Writer
{
	...
	void ChemfilesWriter::writeFile( const FilePath & p_path, const App::Old::Component::Chemistry::Molecule & p_molecule )
	{
		_prepareChemfiles();
	...
	void ChemfilesWriter::fillTrajectoryFrames( chemfiles::Trajectory &				  p_trajectory,
												App::Old::Component::Chemistry::Molecule & p_molecule ) const
	{
		// Fill other frames.
											...
											const App::Old::Component::Chemistry::Molecule & p_molecule )
	{
		...
		for ( uint i = 0; i < p_molecule.getFrameCount(); i++ )
		{
			...
			for ( const App::Old::Component::Chemistry::Chain * const chain : p_molecule.getChains() )
			{
				if ( chain == nullptr )
					continue;
				...
				for ( uint residueIndex = chain->getIndexFirstResidue(); residueIndex <= chain->getIndexLastResidue();
					  residueIndex++ )
				{
					const VTX::App::Old::Component::Chemistry::Residue * const residue
					...
					chemResidue.set( "secondary_structure",
									 Util::App::Old::SecondaryStructure::enumToPdbFormattedSecondaryStructure(
					...
					{
						const VTX::App::Old::Component::Chemistry::Atom * const atom = p_molecule.getAtom( firstResAtomIdx );
			...
			{
				const VTX::App::Old::Component::Chemistry::Bond * bnd = p_molecule.getBond( bond );
	...
	uint ChemfilesWriter::getNewChainIndex( const App::Old::Component::Chemistry::Chain & p_chain ) const
	{
		return _vecNewChainIndexes[ p_chain.getIndex() ];
	}
	uint ChemfilesWriter::getNewResidueIndex( const App::Old::Component::Chemistry::Residue & p_residue ) const
	{
		return _vecNewResidueIndexes[ p_residue.getIndex() ];
	}
	uint ChemfilesWriter::getNewAtomIndex( const App::Old::Component::Chemistry::Atom & p_atom ) const
	{
