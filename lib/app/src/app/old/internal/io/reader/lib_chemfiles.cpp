...
namespace VTX::App::Old::Internal::IO::Reader
{
	...
	LibChemfiles::LibChemfiles( const VTX::App::Old::Core::Worker::BaseThread * const p_loader ) : ChemfilesIO( p_loader ) {}

	void LibChemfiles::readFile( const FilePath & p_path, App::Old::Component::Chemistry::Molecule & p_molecule )
	{
		_prepareChemfiles();
	...
	bool LibChemfiles::readDynamic( const FilePath &								   p_path,
									std::vector<App::Old::Component::Chemistry::Molecule *> p_potentialTargets )
	{
		_prepareChemfiles();
	...
	void LibChemfiles::fillTrajectoryFrame( App::Old::Component::Chemistry::Molecule & p_molecule,
											const uint							  p_moleculeFrameIndex,
											const std::vector<Vec3f> &			  p_atomPositions ) const
	{
		App::Old::Component::Chemistry::Molecule::AtomPositionsFrame & moleculeFrame
		...
		const uint																	p_trajectoryFrameStart ) const
	{
		...
		for ( uint frameIdx = 0; frameIdx < p_trajectory.nsteps() - p_trajectoryFrameStart; ++frameIdx )
		{
			...
			for ( const std::pair<App::Old::Component::Chemistry::Molecule *, uint> & pairMoleculeStartFrame : p_targets )
			{
				fillTrajectoryFrame(
		...
		for ( const std::pair<App::Old::Component::Chemistry::Molecule *, uint> & pairMoleculeFirstFrame : p_targets )
		{
			App::Old::Component::Chemistry::Molecule & molecule = *( pairMoleculeFirstFrame.first );
										...
										const bool							  p_recomputeBonds ) const
	{
		...
		App::Old::Component::IO::MoleculeConfiguration & config	 = p_molecule.getConfiguration();
		...
		App::Old::Component::Chemistry::Molecule::AtomPositionsFrame & modelFrame = p_molecule.getAtomPositionFrame( 0 );
		modelFrame.resize( frame.size() );

		App::Old::Component::Chemistry::Chain * modelChain;
		...
		for ( uint residueIdx = 0; residueIdx < residues.size(); ++residueIdx )
		{
			...
			if ( createNewChain )
			{
				...
				modelChain->setColor( App::Old::Component::Chemistry::Chain::getChainIdColor( chainId ) );
			...
			// Create residue.
			App::Old::Component::Chemistry::Residue * modelResidue
				= VTX::MVC_MANAGER().instantiateModel<App::Old::Component::Chemistry::Residue>();
			...
			else
			{
				...
				else
				{
					...
					unknownResidueData->symbolName = Util::App::Old::Molecule::getResidueFullName( residueSymbol );
			...
			modelResidue->setColor( App::Old::Component::Chemistry::Residue::getResidueColor( *modelResidue ) );
			...
			for ( std::vector<size_t>::const_iterator it = residue.begin(); it != residue.end(); it++ )
			{
				...
				// Create atom.
				App::Old::Component::Chemistry::Atom * modelAtom
					= VTX::MVC_MANAGER().instantiateModel<App::Old::Component::Chemistry::Atom>();
				...
				modelAtom->setName( atom.name() );
				// modelAtom->setColor( App::Old::Component::Chemistry::Atom::SYMBOL_COLOR[ int( modelAtom->getSymbol() ) ]
				// );
		...
		if ( p_trajectory.nsteps() > 1 )
		{
			...
			// fillFrames.detach();
			std::pair<App::Old::Component::Chemistry::Molecule *, uint> pairMoleculeFirstFrame = { &p_molecule, 1 };
		...
		for ( uint boundIdx = 0; boundIdx < uint( bonds.size() ); ++boundIdx )
		{
			...
			App::Old::Component::Chemistry::Residue * residueStart
				= p_molecule.getAtom( uint( bond[ 0 ] ) )->getResiduePtr();
			App::Old::Component::Chemistry::Residue * residueEnd = p_molecule.getAtom( uint( bond[ 1 ] ) )->getResiduePtr();
		...
		for ( uint residueIdx = 0; residueIdx < residues.size(); ++residueIdx )
		{
			App::Old::Component::Chemistry::Residue * const residue			= p_molecule.getResidue( residueIdx );
			...
			for ( uint i = 0; i < vectorBonds.size(); ++i, ++counter )
			{
				...
				App::Old::Component::Chemistry::Bond * modelBond
					= VTX::MVC_MANAGER().instantiateModel<App::Old::Component::Chemistry::Bond>();
		...
		if ( !VTX::App::Old::Application::Setting::COMPUTE_BOND_ORDER_ON_CHEMFILE )
		{
			...
			if ( !allBondsRecomputed )
			{
				...
				Util::App::Old::Molecule::recomputeBondOrders( p_molecule );
			}
			bondComputationChrono.stop();
		...
		const std::vector<App::Old::Component::Chemistry::Molecule *> & p_potentialTargets ) const
	{
		...
		std::vector<std::pair<App::Old::Component::Chemistry::Molecule *, uint>> validTargets
			= std::vector<std::pair<App::Old::Component::Chemistry::Molecule *, uint>>();
		validTargets.reserve( p_potentialTargets.size() );

		for ( App::Old::Component::Chemistry::Molecule * const molecule : p_potentialTargets )
		{
			if ( molecule->getAtomCount() == frameAtomCount )
			{
				...
				catch ( const std::exception & p_e )
				{
					...
					std::vector<App::Old::Component::Chemistry::Molecule::AtomPositionsFrame> & frames
						= molecule->getAtomPositionFrames();
					std::vector<App::Old::Component::Chemistry::Molecule::AtomPositionsFrame>::reverse_iterator itFrame
						= frames.rbegin();
		...
		for ( const std::pair<App::Old::Component::Chemistry::Molecule *, uint> & pairMoleculeFirstFrame : validTargets )
		{
