...
{
	RenderEffectLibrary & RenderEffectLibrary::get() { return App::Old::VTXApp::get().getRenderEffectLibrary(); }

	RenderEffectLibrary::RenderEffectLibrary() :
		BaseModel( App::Old::ID::Model::MODEL_RENDERER_RENDER_EFFECT_PRESET_LIBRARY )
	{
	...
	RenderEffectLibrary ::~RenderEffectLibrary()
	{
		App::Old::Action::Renderer::SavePreset * const saveAction = new App::Old::Action::Renderer::SavePreset( *this );
										 ...
										 const bool					p_notify )
	{
		...
		App::Old::Core::View::CallbackView<RenderEffectPreset, RenderEffectLibrary> * const callbackView
			= VTX::MVC_MANAGER()
				  .instantiateView<App::Old::Core::View::CallbackView<RenderEffectPreset, RenderEffectLibrary>>(
					  p_preset, VTX::App::Old::ID::View::RENDER_EFFECT_LIBRARY_ON_ITEMS );
		...
		VTX_EVENT<int>( VTX::App::Old::Event::Global::RENDER_EFFECT_ADDED, presetAddedIndex );
	};
	...
	RenderEffectPreset * const RenderEffectLibrary::removePreset( const int p_index )
	{
		...
		if ( 0 <= p_index && p_index < _presets.size() )
		{
			...
			VTX_EVENT<int>( VTX::App::Old::Event::Global::RENDER_EFFECT_REMOVED, p_index );
		}
		else
		{
	...
	void RenderEffectLibrary::applyPreset( RenderEffectPreset & p_preset )
	{
		...
		_notifyViews( App::Old::Event::Model::APPLIED_PRESET_CHANGE );
		VTX_EVENT( VTX::App::Old::Event::Global::APPLIED_RENDER_EFFECT_CHANGE );
	}
	...
	void RenderEffectLibrary::clear( const bool p_notify )
	{
		...
		if ( p_notify )
		{
			VTX_EVENT( VTX::App::Old::Event::Global::RENDER_EFFECT_LIBRARY_CLEARED );
			_notifyDataChanged();
		}
	}
	...
	void RenderEffectLibrary::_onPresetChange( const App::Old::Core::Event::VTXEvent * const p_event )
	{
		if ( p_event->name == App::Old::Event::Model::DATA_CHANGE )
			_notifyViews( App::Old::Event::Model::SUBITEM_DATA_CHANGE );
		else
