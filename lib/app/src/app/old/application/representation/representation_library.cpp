...
{
	RepresentationLibrary & RepresentationLibrary::get() { return App::Old::VTXApp::get().getRepresentationLibrary(); };

	RepresentationLibrary::RepresentationLibrary() : BaseModel( App::Old::ID::Model::MODEL_REPRESENTATION_LIBRARY )
	{
	...
	void RepresentationLibrary::addRepresentation( RepresentationPreset * const p_representation, const bool p_notify )
	{
		...
		Core::View::CallbackView<RepresentationPreset, RepresentationLibrary> * const callbackView
			= VTX::MVC_MANAGER().instantiateView<Core::View::CallbackView<RepresentationPreset, RepresentationLibrary>>(
				p_representation, VTX::App::Old::ID::View::REPRESENTATION_LIBRARY_ON_ITEMS );
	...
	RepresentationPreset * RepresentationLibrary::removeRepresentation( const int p_index, const bool p_notify )
	{
		...
		if ( 0 <= p_index && p_index < _representations.size() )
		{
			...
			VTX::MVC_MANAGER().deleteView( _representations[ p_index ],
										   VTX::App::Old::ID::View::REPRESENTATION_LIBRARY_ON_ITEMS );
	...
	void RepresentationLibrary::applyDefault( const bool p_notify )
	{
		...
		if ( isExistingName( name ) == false )
		{
			...
			ballsAndsticks->getData().setColorMode( VTX::Core::ChemDB::Color::COLOR_MODE::CHAIN );
		...
		if ( isExistingName( name ) == false )
		{
			...
			cartoon->getData().setColorMode( VTX::Core::ChemDB::Color::COLOR_MODE::CHAIN );
		...
		if ( isExistingName( name ) == false )
		{
			...
			stickAndCartoon->getData().setColorMode( VTX::Core::ChemDB::Color::COLOR_MODE::CHAIN );
		...
		if ( isExistingName( name ) == false )
		{
			...
			sas->getData().setColorMode( VTX::Core::ChemDB::Color::COLOR_MODE::CHAIN );
		...
		if ( isExistingName( name ) == false )
		{
			...
			ses->getData().setColorMode( VTX::Core::ChemDB::Color::COLOR_MODE::PROTEIN );
		...
		if ( isExistingName( name ) == false )
		{
			...
			stick->getData().setColorMode( VTX::Core::ChemDB::Color::COLOR_MODE::ATOM_CHAIN );
		...
		if ( isExistingName( name ) == false )
		{
			...
			vdw->getData().setColorMode( VTX::Core::ChemDB::Color::COLOR_MODE::ATOM_CHAIN );
