...
namespace VTX::App::Old::Application::Representation
{
	...
	void BaseRepresentable::computeRepresentationTargets()
	{
		...
		for ( App::Old::Component::Chemistry::Residue * const residue : _molecule->getResidues() )
		{
			// Skip hidden items.
			...
			if ( dataFlag & TARGETED_DATA_FLAG::ENUM::RIBBON )
			{
				...
				App::Old::Component::Chemistry::SecondaryStructure & secondaryStructure = _molecule->getSecondaryStructure();
			...
			if ( dataFlag & TARGETED_DATA_FLAG::ENUM::SES )
			{
					 ...
					 || categoryEnum == VTX::Core::ChemDB::Category::TYPE::CARBOHYDRATE )
				{
					...
					const App::Old::Component::Chemistry::SolventExcludedSurface & ses
						= _molecule->getSolventExcludedSurface( categoryEnum );
					const std::vector<App::Old::Component::Chemistry::SolventExcludedSurface::Range> & atomsToTriangles
	...
	void BaseRepresentable::refreshRepresentationTargets()
	{
		...
		for ( const auto & representationTargetPair : _molecule->_representationTargets )
		{
			...
			if ( dataFlag & TARGETED_DATA_FLAG::ENUM::SES )
			{
				...
				for ( const App::Old::Component::Chemistry::Residue * const residue : _molecule->getResidues() )
				{
					// Skip hidden items.
					if ( residue == nullptr || !_isResidueVisible( *residue ) )
					{
	...
	bool BaseRepresentable::_isResidueVisible( const App::Old::Component::Chemistry::Residue & p_residue ) const
	{
		const App::Old::Component::Chemistry::Molecule * const molecule = p_residue.getMoleculePtr();
