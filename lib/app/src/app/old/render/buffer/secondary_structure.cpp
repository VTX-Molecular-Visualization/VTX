...
namespace VTX::App::Old::Render::Buffer
{
	void SecondaryStructure::_generate()
	{
		...
		_vao.setVertexBuffer( ATTRIBUTE_LOCATION::RESIDUE_COLOR, _vboColors, sizeof( Util::Color::Rgba ) );
		...
		_vao.setVertexBuffer( ATTRIBUTE_LOCATION::RESIDUE_ID, _vboIds, sizeof( App::Old::Core::Model::ID ) );
	...
	void SecondaryStructure::setColors( const std::vector<Util::Color::Rgba> & p_colors )
	{
	...
	void SecondaryStructure::setIds( const std::vector<App::Old::Core::Model::ID> & p_ids )
	{
		_updateBuffer( _vboIds, p_ids );
	}
