...
namespace VTX::App::Old::Render::Buffer
{
	void Molecule::_generate()
	{
		...
		_vao.setVertexBuffer( ATTRIBUTE_LOCATION::ATOM_COLOR, _vboAtomColors, sizeof( Util::Color::Rgba ) );
		...
		_vao.setVertexBuffer( ATTRIBUTE_LOCATION::ATOM_ID, _vboAtomIds, sizeof( App::Old::Core::Model::ID ) );
	...
	void Molecule::setAtomRadius( const std::vector<float> & p_radius ) { _updateBuffer( _vboAtomRadius, p_radius ); }

	void Molecule::setAtomColors( const std::vector<Util::Color::Rgba> & p_colors )
	{
	...
	void Molecule::setAtomIds( const std::vector<App::Old::Core::Model::ID> & p_ids )
	{
		_updateBuffer( _vboAtomIds, p_ids );
	}
