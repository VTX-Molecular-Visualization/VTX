...
#include "app/old/render/renderer/rt/integrators/ao_integrator.hpp"
#include "app/old/render/renderer/rt/integrators/direct_lighting_integrator.hpp"
#include "app/old/render/renderer/rt/integrators/raycast_integrator.hpp"
#include "app/old/render/renderer/rt/lights/directional_light.hpp"
#include "app/old/render/renderer/rt/lights/point_light.hpp"
#include "app/old/render/renderer/rt/lights/quad_light.hpp"
#include "app/old/render/renderer/rt/materials/flat_color_material.hpp"
#include "app/old/render/renderer/rt/materials/matte.hpp"
#include "app/old/render/renderer/rt/primitives/cylinder.hpp"
#include "app/old/render/renderer/rt/primitives/molecule_rt.hpp"
#include "app/old/render/renderer/rt/primitives/plane.hpp"
#include "app/old/render/renderer/rt/primitives/sphere.hpp"
#include "app/old/render/renderer/rt/primitives/triangle_mesh.hpp"
...
namespace VTX::App::Old::Render::Renderer::RT
{
	...
	{
	  public:
		CameraRayTracing( const App::Old::Component::Render::Camera & p_camera,
						  const uint								  p_width,
						  const uint								  p_height ) :
			_pos( p_camera.getPosition() ),
			_front( p_camera.getFront() ), _up( p_camera.getUp() ), _right( p_camera.getRight() ), _width( p_width ),
			_height( p_height )
		{
			//
			//
			...
			const Vec3f & camFront = Util::normalize( Vec3f( -14.541f, 14.0144f, 41.9241f ) - _pos );
			Vec3f		  camUp	   = p_camera.getUp();
			Vec3f		  camLeft  = Util::cross( camUp, camFront );
			camUp				   = Util::cross( camLeft, camUp );*/
	...
	void RayTracer::init( const uint p_width, const uint p_height, const GLuint p_outputFramebufferId )
	{
		...
		_integrator = new Integrator::RayCastIntegrator;
		//_integrator	  = new DirectLightingIntegrator;
		//_aoIntegrator = new AOIntegrator( 50.f, 12 );

		VTX_INFO( "Init Scene" );
		_initScene( App::Old::VTXApp::get().getScene() );

		_texture.create( p_width,
						 p_height,
						 App::Old::Render::Renderer::GL::Texture2D::InternalFormat::RGB16F,
						 App::Old::Render::Renderer::GL::Texture2D::Wrapping::CLAMP_TO_EDGE,
						 App::Old::Render::Renderer::GL::Texture2D::Wrapping::CLAMP_TO_EDGE,
						 App::Old::Render::Renderer::GL::Texture2D::Filter::NEAREST,
						 App::Old::Render::Renderer::GL::Texture2D::Filter::NEAREST );

		getOutputFramebuffer().attachTexture( _texture,
											  App::Old::Render::Renderer::GL::Framebuffer::Attachment::COLOR0 );

		VTX_INFO( "Ray tracer initialized" );
	}

	void RayTracer::renderFrame( const App::Old::Application::Scene & p_scene )
	{
		...
		_texture.fill( _pixels.data(),
					   App::Old::Render::Renderer::GL::Texture2D::Format::RGB,
					   App::Old::Render::Renderer::GL::Texture2D::Type::FLOAT );

		getOutputFramebuffer().bind();
		getOutputFramebuffer().setDrawBuffers( { App::Old::Render::Renderer::GL::Framebuffer::Attachment::COLOR0 } );
	...
	void RayTracer::_initScene( const App::Old::Application::Scene & p_scene )
	{
		_scene.clean();
...
#ifdef RIBBON_TEST
		for ( const std::pair<const App::Old::Component::Chemistry::Molecule *, float> & pairMol :
			  p_scene.getMolecules() )
		{
			_scene.addObject( new TriangleMesh( pairMol.first ) );
		}
#else
		for ( const std::pair<const App::Old::Component::Chemistry::Molecule * const, float> & pairMol :
			  p_scene.getMolecules() )
		{
			_scene.addObject( new Primitive::MoleculeRT( pairMol.first ) );
		}
#endif
		...
		// 6VSB
		_scene.addLight( new Light::PointLight( Vec3f( 150.f, -200.f, 90.f ), COLOR_WHITE, 1000000.f ) );
		_scene.addLight( new Light::PointLight( Vec3f( 150.f, -200.f, 300.f ), COLOR_WHITE, 1000000.f ) );
		_scene.addLight( new Light::PointLight( Vec3f( -450.f, -200.f, -38.f ), COLOR_WHITE, 1000000.f ) );
		_scene.addLight( new Light::QuadLight( Vec3f( -450.f, -200.f, -38.f ),
	...
	Util::Color::Rgba RayTracer::_renderPixel( const CameraRayTracing & p_camera,
											   ...
											   const uint				p_nbPixelSamples )
	{
		Util::Color::Rgba color = COLOR_BLACK;
