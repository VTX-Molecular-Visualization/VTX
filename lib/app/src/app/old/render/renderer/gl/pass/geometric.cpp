#include "app/old/render/renderer/gl/pass/geometric.hpp"
...
namespace VTX::App::Old::Render::Renderer::GL::Pass
{
	...
	void Geometric::render( const App::Old::Application::Scene & p_scene, const GL & p_renderer )
	{
		_fbo.bind();
		_fbo.clear( Framebuffer::ClearBuffer::COLOR_DEPTH );

		//_gl->glPolygonMode( GL_FRONT_AND_BACK, GL_LINE );
		for ( const App::Old::Application::Scene::PairMoleculePtrFloat & pair : p_scene.getMolecules() )
		{
			pair.first->render( p_scene.getCamera() );
		}
		for ( const App::Old::Application::Scene::MeshTrianglePtr & mesh : p_scene.getMeshes() )
		{
			mesh->render( p_scene.getCamera() );
		}
		for ( const App::Old::Application::Scene::HelperPtr & helper : p_scene.getHelpers() )
		{
