#include "app/old/render/renderer/gl/pass/ssao.hpp"
...
namespace VTX::App::Old::Render::Renderer::GL::Pass
{
	void SSAO::init( const uint p_width, const uint p_height, const GL & )
	{
		...
		for ( uint i = 0; i < _kernelSize; i++ )
		{
			...
			Vec3f v = App::Old::Internal::Math::Sampler::cosineWeightedHemisphere( Util::Math::randomFloat(),
	...
	void SSAO::render( const App::Old::Application::Scene & p_scene, const GL & p_renderer )
	{
		_fbo.bind();
