...
namespace VTX::App::Old::Render::Worker
{
	...
	const void Snapshoter::_takeSnapshotGL() const
	{
		...
		//		if ( activeAA == false )
		//		{
		//			VTX_ACTION( new App::Old::Action::Setting::ActiveAA( true ) );
		//		}
		//
		//		// Make current.
		...
		//		App::Old::VTXApp::get().getScene().getCamera().setScreenSize( width, height );
		...
		//		glWidget.getRenderer().renderFrame( App::Old::VTXApp::get().getScene() );
		...
		//		if ( activeAA == false )
		//		{
		//			VTX_ACTION( new App::Old::Action::Setting::ActiveAA( false ) );
		//		}
		//
		//		// Add watermark.
		...
		//		if ( VTX_RENDER_EFFECT().getBackgroundColor().brightness() < 0.5f && _exportData.getBackgroundOpacity()
		//> 0.5f )
		//		{
		...
		//		QRect rect = QRect(
		//			QPoint( p_image.size().width() - watermarkImg.width(), p_image.size().height() -
		// watermarkImg.height()
		//), 			watermarkImg.size() ); 		QPainter imagePainter = QPainter( &p_image );
