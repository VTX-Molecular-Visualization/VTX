...
namespace VTX::App::Old::Component::Object3D::Helper
{
	...
	void Grid::refresh()
	{
		...
		_vbo.set<Voxel>( voxels, App::Old::Render::Renderer::GL::Buffer::Usage::STATIC_DRAW );
	...
	void Grid::_generate()
	{
		...
		_vao.setAttributeFormat(
			ATTRIBUTE_LOCATION::VOXEL_MIN, 3, App::Old::Render::Renderer::GL::VertexArray::Type::FLOAT );
		...
		_vao.setAttributeFormat(
			ATTRIBUTE_LOCATION::VOXEL_MAX, 3, App::Old::Render::Renderer::GL::VertexArray::Type::FLOAT );
		_vao.setAttributeBinding( ATTRIBUTE_LOCATION::VOXEL_MAX, ATTRIBUTE_LOCATION::VOXEL_MAX );

		refresh();
	}

	void Grid::render( const App::Old::Component::Render::Camera & p_camera ) const
	{
		_program->use();
		...
		_vao.bind();
		_vao.drawArray( App::Old::Render::Renderer::GL::VertexArray::DrawMode::POINTS, 0, GLsizei( _size ) );
		_vao.unbind();
	}
