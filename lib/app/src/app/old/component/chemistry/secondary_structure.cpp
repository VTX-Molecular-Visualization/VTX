...
#include "app/old/render/view/ribbon.hpp"
...
namespace VTX::App::Old::Component::Chemistry
{
	SecondaryStructure::SecondaryStructure( Molecule * const p_molecule ) :
		BaseModel3D( App::Old::ID::Model::MODEL_SECONDARY_STRUCTURE ), _molecule( p_molecule )
	{
	}
		...
		for ( uint chainIdx = 0; chainIdx < _molecule->getChainCount(); ++chainIdx )
		{
			...
			for ( uint residueIdx = 0; residueIdx < residueCount; ++residueIdx )
			{
				if ( createVectors )
				{
					...
					ids			  = std::vector<App::Old::Core::Model::ID>();
				...
				// Find alpha carbon.
				const Chemistry::Atom * const CA = residue->getAlphaCarbon();
				...
				if ( CA == nullptr ) /// TODO: what to do ?
				{
					...
					const std::string msg
						= "Missing carbon alpha in chain " + chain->getName() + " residue " + residue->getSymbolName();
					// VTX_DEBUG( msg );
					continue;
				}

				// Find oxygen.
				const Chemistry::Atom * const O = residue->findFirstAtomByName( "O" );
				// Missing oxygen atom.
				...
				visibilities.emplace_back( uint( _molecule->isVisible() && chain->isVisible() && residue->isVisible()
												 && CA->isVisible() && O->isVisible() ) );
											...
											const std::vector<App::Old::Core::Model::ID> & p_ids )
	{
		if ( p_caPositions.size() >= 4 )
		{
			...
			_bufferCaODirections.insert( _bufferCaODirections.end(), p_caODirections.cbegin(), p_caODirections.cend() );
	...
	App::Old::Component::Object3D::Helper::AABB & SecondaryStructure::getAABB() const { return _molecule->getAABB(); }

	const App::Old::Internal::Math::Transform & SecondaryStructure::getTransform() const
	{
	...
	{
		_addRenderable( VTX::MVC_MANAGER().instantiateView<App::Old::Render::View::Ribbon>(
			this, VTX::App::Old::ID::View::D3_RIBBON_PATCH ) );
	}
		...
		for ( const SSDataPair & pair : _data )
		{
			for ( const uint residueIdx : pair.second )
			{
				...
				VTX::Core::ChemDB::Color::SECONDARY_STRUCTURE_COLOR_MODE colorMode
					= VTX::App::Old::Application::Setting::SS_COLOR_MODE_DEFAULT;
	...
	void SecondaryStructure::refreshVisibilities()
	{
		...
		for ( const SSDataPair & pair : _data )
		{
			...
			for ( const uint residueIdx : pair.second )
			{
				...
				const Chemistry::Atom * const CA	  = residue->getAlphaCarbon();
				const Chemistry::Atom * const O		  = residue->findFirstAtomByName( "O" );
				_bufferVisibilities.emplace_back( _molecule->isVisible() && chain->isVisible() && residue->isVisible()
												  && CA->isVisible() && O->isVisible() );
			}
		}
