...
#include "app/old/internal/algorithm/marching_cube.hpp"
#include "app/old/render/view/triangle.hpp"
#include "app/old/render/worker/gpu_buffer_initializer.hpp"
#include "app/old/render/worker/gpu_computer.hpp"
...
namespace VTX::App::Old::Component::Chemistry
{
	...
	const App::Old::Internal::Math::Transform & SolventExcludedSurface::getTransform() const
	{
	...
	void SolventExcludedSurface::_init()
	{
		...
		const App::Old::Component::Object3D::Helper::AABB molAABB = _category->getAABB();
		...
		App::Old::Component::Object3D::Helper::Grid gridAtoms
			= App::Old::Component::Object3D::Helper::Grid( gridMin, Vec3f( atomGridCellSize ), atomGridSize );
		...
		App::Old::Component::Object3D::Helper::Grid gridSES
			= App::Old::Component::Object3D::Helper::Grid( gridMin, Vec3f( VOXEL_SIZE ), sesGridSize );

		/////////////////////
		// Worker: create SDF.
		App::Old::Render::Worker::GpuComputer workerCreateSDF( FilePath( "ses/create_sdf.comp" ) );

		// Create SSBOs.
		using VTX::App::Old::Render::Renderer::GL::Buffer;
		// Output.
		...
		//////////////////////
		// Worker: refine SDF.
		App::Old::Render::Worker::GpuComputer workerRefineSDF( FilePath( "ses/refine_sdf.comp" ) );
		...
		////////////////////////////
		// Worker: reduce grid.
		App::Old::Render::Worker::GpuComputer workerReduceGrid( FilePath( "ses/reduce_grid.comp" ) );
		...
		App::Old::Render::Worker::GpuComputer workerGridCompaction( FilePath( "ses/grid_compaction.comp" ) );
		...
		App::Old::Render::Worker::GpuComputer workerMarchingCube( FilePath( "ses/marching_cube.comp" ) );
		...
		App::Old::Render::Worker::GpuComputer workerBufferCompaction( FilePath( "ses/buffer_compaction.comp" ) );
		...
		uint * const ptrIndices
			= bufferIndices.map<uint>( 0,
		...
		for ( uint i = 1; i < _indiceCount; ++i )
		{
			...
			if ( Util::Math::length2( ptrPositions[ indexCurrent ] - ptrPositions[ indexNext ] ) < EPSILON * EPSILON )
			{
		...
		chrono2.stop();
		VTX_DEBUG(
			"{} duplicates over {} vertices detected in {}s", duplicateCounter, _indiceCount, chrono2.elapsedTime() );
		chrono2.start();
...
#if VTX_SES_NORMALS_GPU
		{
			...
			App::Old::Render::Worker::GpuComputer workerComputeNormals( FilePath( "ses/compute_normals.comp" ) );
			...
			App::Old::Render::Worker::GpuComputer workerNormalizeNormals( FilePath( "ses/normalize_normals.comp" ) );
...
#else
		{
			...
			ptrPositions = bufferPositions.map<Vec4f>( 0, _indiceCount * sizeof( Vec4f ), Buffer::Flags::MAP_READ_BIT );
			...
			{
				Vec3f normal
					= Util::cross( Vec3f( ptrPositions[ ptrIndices[ i + 1 ] ] - ptrPositions[ ptrIndices[ i + 2 ] ] ),
	...
	void SolventExcludedSurface::refreshColors()
	{
		...
		_buffer->memoryBarrier( GL_SHADER_STORAGE_BARRIER_BIT );
		VTX_WORKER( new App::Old::Render::Worker::GpuBufferInitializer( bufferCounters, _indiceCount ) );
		_buffer->memoryBarrier( GL_SHADER_STORAGE_BARRIER_BIT );
		VTX_WORKER( new App::Old::Render::Worker::GpuBufferInitializer(
			bufferColorsUint, _indiceCount, "uvec4", "std140", "uvec4(0)" ) );
		_buffer->memoryBarrier( GL_SHADER_STORAGE_BARRIER_BIT );
		...
		App::Old::Render::Worker::GpuComputer worker( FilePath( "ses/apply_color.comp" ) );
		...
		worker = App::Old::Render::Worker::GpuComputer( FilePath( "ses/apply_color_divide.comp" ) );
	...
	void SolventExcludedSurface::refreshVisibilities()
	{
		...
		_buffer->memoryBarrier( GL_SHADER_STORAGE_BARRIER_BIT );
		VTX_WORKER( new App::Old::Render::Worker::GpuBufferInitializer( bufferVisibilities, _indiceCount ) );
		_buffer->memoryBarrier( GL_SHADER_STORAGE_BARRIER_BIT );
		...
		App::Old::Render::Worker::GpuComputer worker( FilePath( "ses/apply_visibility.comp" ) );
	...
	void SolventExcludedSurface::refreshSelections()
	{
		...
		_buffer->memoryBarrier( GL_SHADER_STORAGE_BARRIER_BIT );
		VTX_WORKER( new App::Old::Render::Worker::GpuBufferInitializer( bufferSelections, _indiceCount ) );
		_buffer->memoryBarrier( GL_SHADER_STORAGE_BARRIER_BIT );
		...
		App::Old::Render::Worker::GpuComputer worker( FilePath( "ses/apply_selection.comp" ) );
	...
	{
		_addRenderable( VTX::MVC_MANAGER().instantiateView<App::Old::Render::View::TriangleSES>(
			this, VTX::App::Old::ID::View::D3_TRIANGLE_SES ) );
	}
} // namespace VTX::App::Old::Component::Chemistry
