...
#ifndef __VTX_APP_RENDER_RENDERER_RT_RAY_TRACER__
#define __VTX_APP_RENDER_RENDERER_RT_RAY_TRACER__

#include "app/render/renderer/rt/bvh.hpp"
#include <algorithm>
#include <thread>
...
namespace VTX::App::Render::Renderer::RT
{
	...
	struct BVHBuildNode
	{
		...
		BVHBuildNode( const uint									 p_idFirstPrim,
					  const uint									 p_nbPrims,
					  const App::Component::Object3D::Helper::AABB & p_aabb ) :
			_idFirstPrim( p_idFirstPrim ),
			_nbPrims( p_nbPrims ), _aabb( p_aabb )
		{
			++leafNodes;
			++totalLeafNodes;
		...
		{
			_aabb = App::Component::Object3D::Helper::AABB::join( p_child0->_aabb, p_child1->_aabb );
			++interiorNodes;
		}
	...
	{
		BVHPrimInfo() {}
		BVHPrimInfo( const uint p_idPrimitive, const App::Component::Object3D::Helper::AABB & p_aabb ) :
	...
	void BVH::build( const std::vector<Primitive::BasePrimitive *> & p_prims,
					 ...
					 const SplitMethod								 p_splitMethod )
	{
		...
		std::vector<Primitive::BasePrimitive *> outPrims;
											...
											std::vector<Primitive::BasePrimitive *> & p_outPrims )
	{
		...
		else
		{
			...
			App::Component::Object3D::Helper::AABB centroidsAABB;
			for ( uint i = p_begin; i < p_end; ++i )
			{
			...
			else
			{
				...
				{
					App::Component::Object3D::Helper::AABB aabb0;
					uint								   count0 = 0;
					for ( uint j = 0; j <= i; ++j )
					{
					...
					App::Component::Object3D::Helper::AABB aabb1;
					uint								   count1 = 0;
				...
				{
					// Split it !
					BVHPrimInfo * nodeSplit
						= std::partition( &p_primsInfo[ p_begin ],
										  &p_primsInfo[ p_end - 1 ] + 1,
										  [ & ]( const BVHPrimInfo & p )
										  {
												 ...
												 uint &						   totalNodes ) const
	{
		...
		for ( uint i = p_begin; i < p_end; ++i )
		{
			const float splitCentroid
				= ( p_treeletRoots[ i ]->_aabb.getMin()[ splitDim ] + p_treeletRoots[ i ]->_aabb.getMax()[ splitDim ] )
				  * 0.5f;
			uint b = uint( nbBins
		...
		BVHBuildNode ** pmid = std::partition(
			...
			{
				const float centroid = ( node->_aabb.getMin()[ splitDim ] + node->_aabb.getMax()[ splitDim ] ) * 0.5f;
				uint		b		 = uint( nbBins
