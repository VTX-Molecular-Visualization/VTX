...
namespace VTX
{
	namespace Model
	{
		...
		Molecule::Molecule( const VTX::ID::VTX_ID & p_typeId ) : BaseModel3D( VTX::ID::Model::MODEL_MOLECULE )
		{
			...
			// TODO VTX_LOGIC Manage setting out of this class
			...
			if ( _atomPositionsFrames.size() > 0 )
			{
...

				{
					...
					for ( Model::Category * const category : getCategories() )
					{
						...
						// TODO VTX_LOGIC Manage default representation for categories in a better way.
						...
						const Model::Representation::Representation * const defaultRepresentation = nullptr;
		...
		// TODO VTX_LOGIC View instantiation out of this class
		...
		// TODO VTX_LOGIC Manage buffers out of this class
		void Molecule::refreshBondsBuffer()
		{
			// TODO VX_LOGIC Send signal when data for buffers has changed
			// New (maybe use mask ?)
			//_notifyViews(new VTXEvent(Event::BOND_INFO_CHANGED);
			// Old
			//_buffer->setBonds( _bufferBonds );
		}
		...
		void Molecule::_fillBufferAtomSelections( const Model::Selection::MapChainIds * const p_selection )
		{
			...
			// TODO VX_LOGIC Send signal when data for buffers has changed
			// New (maybe use mask ?)
			//_notifyViews(new VTXEvent(Event::SELECTION_INFO_CHANGED);
			// Old
			//_buffer->setAtomSelections( _bufferAtomSelections );
		}
		...
		void Molecule::setFrame( const uint p_frameIdx )
		{
			...
			// TODO VX_LOGIC Send signal when data for buffers has changed
			// New (maybe use mask ?)
			//_notifyViews(new VTXEvent(Event::ATOMS_POSITION_CHANGED);
			// Old
			// if ( _buffer != nullptr )
			//	_buffer->setAtomPositions( _atomPositionsFrames[ _currentFrame ] );
			...
			// TODO VTX_LOGIC Manage render mask out of this class
		...
		// TODO VTX_LOGIC Render out of this project
