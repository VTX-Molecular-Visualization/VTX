...
namespace VTX::App::Core::View
{
	...
	class CallbackView : public App::Core::View::BaseView<T1>
	{
		VTX_VIEW

	  public:
		void setCallback( T2 * const p_caller,
						  void ( T2::*p_notifyCallback )( const App::Core::Event::VTXEvent * const ) )
		{
		...
		void setCallback( T2 * const p_caller,
						  void ( T2::*p_notifyCallback )( const T1 * const p_model,
														  const App::Core::Event::VTXEvent * const ) )
		{
			_notifyTarget			 = p_caller;
	  ...
	  protected:
		CallbackView( T1 * const p_model ) : App::Core::View::BaseView<T1>( p_model ) {};

	  private:
		T2 * _notifyTarget														  = nullptr;
		void ( T2::*_notifyCallback )( const App::Core::Event::VTXEvent * const ) = nullptr;
		void ( T2::*_notifyWithModelCallback )( const T1 * const p_model, const App::Core::Event::VTXEvent * const )
			= nullptr;
	};
