...
#include "atom.hpp"
#include "chain.hpp"
#include "manager/selection/selection_manager.hpp"
#include "model/math/aabb.hpp"
#include "model/math/marching_cube.hpp"
#include "model/object3d/scene.hpp"
	...
	namespace Model
	{
		...
		void SolventExcludedSurface::_refreshCPU()
		{
			...
			for ( uint x = 0; x < uint( gridSES.size.x ); ++x )
			{
				for ( uint y = 0; y < uint( gridSES.size.y ); ++y )
				{
					for ( uint z = 0; z < uint( gridSES.size.z ); ++z )
					{
						...
						for ( int ox = int( -cellsToVisitCount.x ); ox <= cellsToVisitCount.x && !found; ++ox )
						{
							for ( int oy = int( -cellsToVisitCount.y ); oy <= cellsToVisitCount.y && !found; ++oy )
							{
								for ( int oz = int( -cellsToVisitCount.z ); oz <= cellsToVisitCount.z && !found; ++oz )
								{
			...
			for ( const uint sesGridHash : sesGridDataBoundary )
			{
				...
				bool		  found						  = false;
				for ( int ox = int( -cellsToVisitCount.x ); ox <= cellsToVisitCount.x; ++ox )
				{
					for ( int oy = int( -cellsToVisitCount.y ); oy <= cellsToVisitCount.y; ++oy )
					{
						for ( int oz = int( -cellsToVisitCount.z ); oz <= cellsToVisitCount.z; ++oz )
						{
		...
		void SolventExcludedSurface::refreshColors()
		{
			...
			// New
			//_notifyViews(new VTXEvent(Event::COLOR_INFO_CHANGED);
			// Old
		...
		void SolventExcludedSurface::refreshVisibilities()
		{
			...
			// TODO VX_LOGIC Send signal when data for buffers has changed
			// New
			//_notifyViews(new VTXEvent(Event::VISIBILITY_INFO_CHANGED);
			// Old
		...
		void SolventExcludedSurface::refreshSelection( const Model::Selection::MapChainIds * const p_selection )
		{
			...
			// TODO VX_LOGIC Send signal when data for buffers has changed
			// New
			//_notifyViews(new VTXEvent(Event::SELECTION_INFO_CHANGE);
			// Old
