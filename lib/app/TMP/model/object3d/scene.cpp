...
#include "model/math/transform.hpp"
#include <algorithm>
...
namespace VTX::Object3D
{
	...
	{
		// TODO Manage item in scene with just one vector (or a map of vector to sort items per type)
		// old
	...
	void Scene::addSceneItem( Generic::BaseSceneItem * const ) {}
	void Scene::removeSceneItem( Generic::BaseSceneItem * const ) {}

	void Scene::_updateGraphicMask() const
	{
		// TODO VTX_LOGIC Call event instead of set mask here
		// VTXApp::get().MASK |= VTX_MASK_NEED_UPDATE;
	}
	...
	void Scene::sortItemsBySceneIndex( std::vector<Generic::BaseSceneItem *> & p_items ) const
	{
		for ( int i = 0; i < p_items.size(); i++ )
		{
			int smallerIndexInScene = getItemPosition( *p_items[ i ] );
			int indexInVector		= i;

			for ( int j = i + 1; j < p_items.size(); j++ )
			{
				const int currentIndexInScene = getItemPosition( *p_items[ j ] );
			...
			Generic::BaseSceneItem * const tmp = p_items[ i ];
			p_items[ i ]					   = p_items[ indexInVector ];
			p_items[ indexInVector ]		   = tmp;
		}
	}
	...
	{
		_aabb.invalidate();
		// TODO VTX_LOGIC Manage that with _sceneItemsPerType
	...
	{
		// TODO VX_LOGIC Manage that shit I don't know how. From VTX_APP certainly ?
