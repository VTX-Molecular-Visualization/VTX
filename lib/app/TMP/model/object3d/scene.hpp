...
namespace VTX::Object3D
{
	...
	class Scene : public Generic::BaseUpdatable
	{
	  ...
	  public:
		...
		// TODO VTX_LOGIC Manage scene item with BaseSceneItem class instead of a different vector for each
		// old
		...
		// new
		void addSceneItem( Generic::BaseSceneItem * const );
		void removeSceneItem( Generic::BaseSceneItem * const );
		...
		void sortItemsBySceneIndex( std::vector<Generic::BaseSceneItem *> & p_items ) const;
	  ...
	  private:
		...
		std::map<ID::VTX_ID, std::vector<Generic::BaseSceneItem *>> _sceneItemsPerType
			= std::map<ID::VTX_ID, std::vector<Generic::BaseSceneItem *>>();
