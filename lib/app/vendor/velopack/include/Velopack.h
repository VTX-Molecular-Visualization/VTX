...
/**
 * User delegate for freeing a release feed. This function should free the feed string returned by
...
typedef struct vpkc_asset_t
{
	...
	uint64_t Size;
	/**
	 * The release notes in markdown format, as passed to Velopack when packaging the release. This may be an empty
	 * string.
	 */
	char * NotesMarkdown;
	/**
	 * The release notes in HTML format, transformed from Markdown when packaging the release. This may be an empty
	 * string.
	 */
	char * NotesHTML;
} vpkc_asset_t;
 ...
 * Progress
 */
typedef bool ( *vpkc_download_asset_delegate_t )(
	void *						p_user_data,
...
/**
 * VelopackLocator provides some utility functions for locating the current app important paths (eg. path to packages,
 * update binary, and so forth).
 */
...
/**
 * Holds information about the current version and pending updates, such as how many there are, and access to release
 * notes.
 */
typedef struct vpkc_update_info_t
{
	...
	/**
	 * True if the update is a version downgrade or lateral move (such as when switching channels to the same version
	 * number). In this case, only full updates are allowed, and any local packages on disk newer than the downloaded
 ...
 */
typedef void ( *vpkc_log_callback_t )( void * p_user_data, const char * psz_level, const char * psz_message );
...
extern "C"
{
	 ...
	 * for the lifetime of any UpdateManager's that use this source. You should call `vpkc_free_source` to free the
	 * source, but note that if the source is still in use by an UpdateManager, it will not be freed until the
	 * UpdateManager is freed. Therefore to avoid possible issues, it is recommended to create this type of source once
	 ...
	 * @returns A new vpkc_update_source_t instance, or null on error. If null, the error will be available via
	 * `vpkc_get_last_error`.
	 */
	vpkc_update_source_t * vpkc_new_source_custom_callback(
		vpkc_release_feed_delegate_t   cb_release_feed,
	 ...
	 * @returns True if the update manager was created successfully, false otherwise. If false, the error will be
	 * available via `vpkc_get_last_error`.
	 */
	bool vpkc_new_update_manager(
		const char *				   psz_url_or_path,
	 ...
	 * available via `vpkc_get_last_error`.
	 */
	bool vpkc_new_update_manager_with_source(
		vpkc_update_source_t *		   p_source,
		struct vpkc_update_options_t * p_options,
		struct vpkc_locator_config_t * p_locator,
		vpkc_update_manager_t **	   p_manager
	);
	 ...
	 * @returns The number of characters written to `psz_version` (including null terminator), or the required buffer
	 * size if the buffer is too small.
	 */
	 ...
	 * @returns The number of characters written to `psz_id` (including null terminator), or the required buffer size if
	 ...
	 * UpdateInfo object containing the latest available release, and any delta updates that can be applied if they are
	 * available.
	 * @param p_manager The update manager instance.
	 * @param p_update A pointer to where the new vpkc_update_info_t* instance will be stored if an update is available.
	 * @returns A `vpkc_update_check_t` value indicating the result of the check. If an update is available, the value
	 * will be `HasUpdate` and `p_update` will be populated.
	 */
	vpkc_update_check_t vpkc_check_for_updates(
		vpkc_update_manager_t *		 p_manager,
		struct vpkc_update_info_t ** p_update
	);

	/**
	 * Downloads the specified updates to the local app packages directory. Progress is reported back to the caller via
	 * an optional callback. This function will acquire a global update lock so may fail if there is already another
	 ...
	 * @param p_manager The update manager instance.
	 * @param p_asset The asset to apply. This can be from `vpkc_update_pending_restart` or
	 * `vpkc_update_info_get_target_asset`.
	 ...
	 * @returns true on success, false on failure. If false, the error will be available via `vpkc_get_last_error`.
	 */
	bool vpkc_wait_exit_then_apply_updates(
		vpkc_update_manager_t * p_manager,
	 ...
	 * @returns true on success, false on failure. If false, the error will be available via `vpkc_get_last_error`.
	 */
	bool vpkc_unsafe_apply_updates(
		vpkc_update_manager_t * p_manager,
		struct vpkc_asset_t *	p_asset,
		bool					b_silent,
	...
	/**
	 * VelopackLocator provides some utility functions for locating the current app important paths (eg. path to
	 * packages, update binary, and so forth).
	 ...
	 * @param cb_after_install The callback to run after the app is installed. The callback takes a user data pointer
	 ...
	 * If your code has not completed within 30 seconds, it will be terminated.
	 * Only supported on windows; On other operating systems, this will never be called.
	 * @param cb_before_uninstall The callback to run before the app is uninstalled. The callback takes a user data
	 * pointer and the version of the app as a string.
	 */
	 ...
	 * If your code has not completed within 30 seconds, it will be terminated.
	 * Only supported on windows; On other operating systems, this will never be called.
	 * @param cb_before_update The callback to run before the app is updated. The callback takes a user data pointer and
	 * the version of the app as a string.
	 */
	 ...
	 * If your code has not completed within 30 seconds, it will be terminated.
	 * Only supported on windows; On other operating systems, this will never be called.
	 * @param cb_after_update The callback to run after the app is updated. The callback takes a user data pointer and
	 * the version of the app as a string.
	 */
	 ...
	 * @param cb_first_run The callback to run on first run. The callback takes a user data pointer and the version of
	 * the app as a string.
	 */
	 ...
	 * @param cb_restarted The callback to run after the app is restarted. The callback takes a user data pointer and
	 * the version of the app as a string.
	 */
	 ...
	 * @returns The number of characters written to `psz_error` (including null terminator). If the return value is
	 * greater than `c_error`, the buffer was too small and the message was truncated.
	 */
	 ...
	 * @param cb_log The callback to call with log messages. The callback takes a user data pointer, a log level, and
