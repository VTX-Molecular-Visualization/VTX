...
namespace Velopack
{
	...
	static inline void free_c_string( char * str ) { delete[] str; }
	...
	static inline std::vector<VelopackLocatorConfig> to_cpp_VelopackLocatorConfig_vec(
		const vpkc_locator_config_t * const * arr,
		size_t								  c
	)
	{
		if ( arr == nullptr || c < 1 )
		{
			return std::vector<VelopackLocatorConfig>();
		}
	...
	static inline vpkc_locator_config_t * alloc_c_VelopackLocatorConfig(
		const std::optional<VelopackLocatorConfig> & dto
	)
	{
		if ( !dto.has_value() )
		{
	...
	static inline vpkc_locator_config_t ** alloc_c_VelopackLocatorConfig_vec(
		const std::vector<VelopackLocatorConfig> & dto,
		size_t *								   count
	)
	{
		if ( dto.empty() )
	...
	static inline std::vector<VelopackAsset> to_cpp_VelopackAsset_vec( const vpkc_asset_t * const * arr, size_t c )
	{
		if ( arr == nullptr || c < 1 )
		{
			return std::vector<VelopackAsset>();
		}
	...
	/** Holds information about the current version and pending updates, such as how many there are, and access to
	 * release notes. */
	struct UpdateInfo
	{
		...
		/**
		 * True if the update is a version downgrade or lateral move (such as when switching channels to the same
		 * version number). In this case, only full updates are allowed, and any local packages on disk newer than the
	...
	struct UpdateOptions
	{
		 ...
		 * For example, if the current release was packaged with '--channel beta', then the default channel will be
		 * 'beta'. This allows users to automatically receive updates from the same channel they installed from. This
		 * options allows you to explicitly switch channels, for example if the user wished to switch back to the
		 * 'stable' channel without having to reinstall the application.
		 */
	...
	static inline std::vector<UpdateOptions> to_cpp_UpdateOptions_vec(
		const vpkc_update_options_t * const * arr,
		size_t								  c
	)
	{
		if ( arr == nullptr || c < 1 )
		{
			return std::vector<UpdateOptions>();
		}
	...
	static inline vpkc_update_options_t ** alloc_c_UpdateOptions_vec(
		const std::vector<UpdateOptions> & dto,
		size_t *						   count
	)
	{
		if ( dto.empty() )
	...
	class VelopackApp
	{
	  ...
	  public:
		 ...
		 */
		static VelopackApp Build() { return VelopackApp(); };
		 ...
		 */
		void Run( void * pUserData = 0 ) { vpkc_app_run( pUserData ); };
	};
	...
	class IUpdateSource
	{
	  ...
	  public:
		 ...
		 */
		virtual ~IUpdateSource() { vpkc_free_source( m_pSource ); }
		/**
		 * Default constructor for IUpdateSource. This will create a new custom source that calls back into the virtual
		...
		IUpdateSource()
		{
			m_pSource = vpkc_new_source_custom_callback(
				...
				[]( void * userData, char * pszFeed ) { free_c_string( pszFeed ); },
				[]( void *						userData,
					const struct vpkc_asset_t * pAsset,
					const char *				pszLocalPath,
					size_t						progressCallbackId )
				{
					IUpdateSource *				   source			 = reinterpret_cast<IUpdateSource *>( userData );
	...
	/**
	 * Provides functionality for checking for updates, downloading updates, and applying updates to the current
	 * application. This class is the main entry point for interacting with Velopack.
	 */
	class UpdateManager
	{
	  private:
	  ...
	  public:
		...
		UpdateManager(
			...
			const UpdateOptions *		  options = nullptr,
			const VelopackLocatorConfig * locator = nullptr
		)
		{
		...
		UpdateManager(
			std::unique_ptr<T>			  pUpdateSource,
			const UpdateOptions *		  options = nullptr,
			const VelopackLocatorConfig * locator = nullptr
		)
		{
		 ...
		 */
		bool IsPortable() noexcept { return vpkc_is_portable( m_pManager ); };
		...
		/**
		 * Checks for updates, returning null if there are none available. If there are updates available, this method
		 * will return an UpdateInfo object containing the latest available release, and any delta updates that can be
		 * applied if they are available.
		...
		std::optional<UpdateInfo> CheckForUpdates()
		{
			...
			switch ( result )
			{
			case vpkc_update_check_t::UPDATE_ERROR: throw_last_error(); return std::nullopt;
			case vpkc_update_check_t::NO_UPDATE_AVAILABLE:
			case vpkc_update_check_t::REMOTE_IS_EMPTY: return std::nullopt;
		...
		/**
		 * Downloads the specified updates to the local app packages directory. Progress is reported back to the caller
		 * via an optional callback. This function will acquire a global update lock so may fail if there is already
		...
		void DownloadUpdates(
			...
			vpkc_progress_callback_t progress  = nullptr,
			void *					 pUserData = 0
		)
		{
		...
		)
		{
			...
			bool		   result		= vpkc_wait_exit_then_apply_updates(
				m_pManager, vpkc_asset, silent, restart, pRestartArgs, cRestartArgs
			);
		...
		)
		{
			...
			char **		   pRestartArgs = alloc_c_string_vec( restartArgs, &cRestartArgs );
			vpkc_asset_t * vpkc_asset	= alloc_c_VelopackAsset( asset );
			bool		   result		= vpkc_unsafe_apply_updates(
				m_pManager, vpkc_asset, silent, waitPid, restart, pRestartArgs, cRestartArgs
			);
			free_c_string_vec( pRestartArgs, cRestartArgs );
			free_c_VelopackAsset( vpkc_asset );
			if ( !result )
