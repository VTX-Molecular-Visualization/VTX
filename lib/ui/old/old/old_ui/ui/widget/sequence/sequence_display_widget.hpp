...
namespace VTX::UI::Widget::Sequence
{
	class SequenceDisplayWidget : public QLabel
	{
	  ...
	  public:
		...
		App::Old::Component::Chemistry::Residue * const getResidueAtPos( const QPoint & p_pos );
		App::Old::Component::Chemistry::Residue * const getClosestResidueFromPos( const QPoint & p_pos,
																			 const bool		p_takeForward );

		void updateSelection( const std::vector<App::Old::Component::Chemistry::Residue *> & p_selection )
		{
			_moleculeSelection = &p_selection;
			update();
		};
		QPoint getResiduePos( const App::Old::Component::Chemistry::Residue & p_residue,
	  ...
	  private:
		...
		const std::vector<App::Old::Component::Chemistry::Residue *> * _moleculeSelection = nullptr;

		uint									   _getCharIndex( const uint p_residueIndex ) const;
		App::Old::Component::Chemistry::Residue &	   _getResidue( const uint p_localResidueIndex ) const;
		App::Old::Component::Chemistry::Residue * const _getResidueFromLocaleXPos( const int p_localeXPos ) const;
		uint _getLocalResidueIndexFromResidue( const App::Old::Component::Chemistry::Residue & p_globalResIndex ) const;
	};
} // namespace VTX::UI::Widget::Sequence
