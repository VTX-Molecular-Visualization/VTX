...
namespace VTX::Network::Request
{
	CheckUpdate::CheckUpdate( const bool p_showPopupIfNoUpdate ) :
		NetworkRequest( std::string( App::Old::Application::VTX_VERSION_URL ) ),
		_showPopupIfNoUpdate( p_showPopupIfNoUpdate )
	{
		VTX_INFO( "Checking for update" );
	}

	void CheckUpdate::_success( App::Old::Core::Network::NetworkReply * const p_reply )
	{
		try
		{
			...
			if ( major > App::Old::Application::VTX_VERSION_MAJOR )
			{
				needUpdate = true;
			}
			else if ( major == App::Old::Application::VTX_VERSION_MAJOR && minor > App::Old::Application::VTX_VERSION_MINOR )
			{
				needUpdate = true;
			}
			else if ( major == App::Old::Application::VTX_VERSION_MAJOR && minor == App::Old::Application::VTX_VERSION_MINOR
					  && revision > App::Old::Application::VTX_VERSION_REVISION )
			{
				needUpdate = true;
			}
			...
			if ( needUpdate )
			{
				const std::string consoleMessage = "VTX " + std::to_string( major ) + "." + std::to_string( minor )
												   ...
												   + App::Old::Application::VTX_RELEASES_URL + "\nor\n"
												   + App::Old::Application::VTX_WEBSITE_URL;

				const QString dialogMessage = QString::fromStdString(
					"VTX " + std::to_string( major ) + "." + std::to_string( minor ) + "." + std::to_string( revision )
					+ " is available at: " + App::Old::Application::VTX_RELEASES_CLICKABLE_URL + "<br>or "
					+ App::Old::Application::VTX_WEBSITE_CLICKABLE_URL );
