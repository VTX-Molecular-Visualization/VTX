														  ...
														  std::vector<App::Old::Core::Model::ID> & p_vectorId ) const
	{
		for ( const QItemSelectionRange & modelRange : p_selection )
		{
			...
			for ( const QModelIndex & modelIndex : modelRange.indexes() )
			{
				...
				if ( VTX::MVC_MANAGER().getModelTypeID( modelId ) == App::Old::ID::Model::MODEL_CATEGORY )
				{
					const App::Old::Component::Chemistry::Category & category
						= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Category>( modelId );
					const App::Old::Component::Chemistry::Molecule * const molecule = category.getMoleculePtr();

					for ( const uint chainIndex : category.getChains() )
					{
						const App::Old::Component::Chemistry::Chain * const chainPtr = molecule->getChain( chainIndex );
