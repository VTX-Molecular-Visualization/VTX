...
namespace VTX::View::UI::Widget
{
	PathSceneView::PathSceneView( App::Old::Component::Video::Path * const p_model, QWidget * const p_parent ) :
		App::Old::Core::View::BaseView<App::Old::Component::Video::Path>( p_model ), SceneItemWidget( p_parent )
	{
		...
		if ( p_column == 0 )
		{
			...
			if ( VTX::MVC_MANAGER().getModelTypeID( idTarget ) == App::Old::ID::Model::MODEL_VIEWPOINT )
			{
				...
				if ( itemTxt != viewpoint.getDefaultName() )
				{
					...
					{
						VTX_ACTION( new App::Old::Action::Viewpoint::Rename( viewpoint, itemTxt ) );
					}
	...
	void PathSceneView::_onCustomContextMenuCalled( const QPoint & p_clicPos )
	{
		...
		const App::Old::Core::Model::ID & itemID = _getModelIDFromItem( *targetedItem );

		const QPoint globalClicPos = mapToGlobal( p_clicPos );

		if ( VTX::MVC_MANAGER().getModelTypeID( itemID ) == App::Old::ID::Model::MODEL_PATH )
		{
			App::Old::Component::Video::Path & pathTargeted
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Video::Path>( itemID );
			VTX::UI::ContextualMenu::pop( VTX::UI::ContextualMenu::Menu::Path, &pathTargeted, globalClicPos );
		}
		else if ( VTX::MVC_MANAGER().getModelTypeID( itemID ) == App::Old::ID::Model::MODEL_VIEWPOINT )
		{
			App::Old::Component::Object3D::Viewpoint & viewpointTargeted
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Object3D::Viewpoint>( itemID );
