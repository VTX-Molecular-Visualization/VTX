...
{
	MoleculeSceneView::MoleculeSceneView( App::Old::Component::Chemistry::Molecule * const p_model,
										  QWidget * const							  p_parent ) :
		App::Old::Core::View::BaseView<App::Old::Component::Chemistry::Molecule>( p_model ),
	...
	void MoleculeSceneView::notify( const VTX::App::Old::Core::Event::VTXEvent * const p_event )
	{
		if ( p_event->name == VTX::App::Old::Event::Model::MOLECULE_VISIBILITY )
		{
			_refreshItemVisibility( _getMoleculeTreeWidgetItem(), _model->isVisible() );
		}
		else if ( p_event->name == VTX::App::Old::Event::Model::CATEGORY_VISIBILITY )
		{
			const VTX::App::Old::Core::Event::VTXEventArg<VTX::Core::ChemDB::Category::TYPE> * const castedEventData
				= dynamic_cast<const VTX::App::Old::Core::Event::VTXEventArg<VTX::Core::ChemDB::Category::TYPE> *>(
					p_event );
			const App::Old::Component::Chemistry::Category & category = _model->getCategory( castedEventData->get() );
		...
		else if ( p_event->name == VTX::App::Old::Event::Model::CHAIN_VISIBILITY )
		{
			const VTX::App::Old::Core::Event::VTXEventArg<uint> * const castedEventData
				= dynamic_cast<const VTX::App::Old::Core::Event::VTXEventArg<uint> *>( p_event );
			const uint									index	 = castedEventData->get();
			const App::Old::Component::Chemistry::Chain &	chain	 = *_model->getChain( index );
			const App::Old::Component::Chemistry::Category & category = *( _model->getCategoryFromChain( chain ) );
		...
		else if ( p_event->name == VTX::App::Old::Event::Model::RESIDUE_VISIBILITY )
		{
			const VTX::App::Old::Core::Event::VTXEventArg<uint> * const castedEventData
				= dynamic_cast<const VTX::App::Old::Core::Event::VTXEventArg<uint> *>( p_event );
			const uint								   index   = castedEventData->get();
			const App::Old::Component::Chemistry::Residue & residue = *_model->getResidue( index );
		...
		else if ( p_event->name == VTX::App::Old::Event::Model::ATOM_VISIBILITY )
		{
			const VTX::App::Old::Core::Event::VTXEventArg<uint> * const castedEventData
				= dynamic_cast<const VTX::App::Old::Core::Event::VTXEventArg<uint> *>( p_event );
			const App::Old::Component::Chemistry::Atom & atom = *_model->getAtom( castedEventData->get() );
		...
		else if ( p_event->name == VTX::App::Old::Event::Model::VISIBILITY )
		{
	...
	void MoleculeSceneView::receiveEvent( const VTX::App::Old::Core::Event::VTXEvent & p_event )
	{
		...
		if ( p_event.name == VTX::App::Old::Event::Global::SETTINGS_CHANGE )
		{
			...
			if ( castedEvent.get().find( VTX::App::Old::Application::Setting::PARAMETER ::SYMBOL_DISPLAY_MODE )
				 != castedEvent.get().end() )
			{
	...
	bool MoleculeSceneView::containsModel( const App::Old::Core::Model::BaseModel & p_model ) const
	{
		const VTX::App::Old::VTX_ID &					modelTypeID = p_model.getTypeId();
		const App::Old::Component::Chemistry::Molecule * linkedMolecule;

		if ( modelTypeID == App::Old::ID::Model::MODEL_MOLECULE )
			linkedMolecule = static_cast<const App::Old::Component::Chemistry::Molecule *>( &p_model );
		else if ( modelTypeID == App::Old::ID::Model::MODEL_CATEGORY )
			linkedMolecule = static_cast<const App::Old::Component::Chemistry::Category &>( p_model ).getMoleculePtr();
		else if ( modelTypeID == App::Old::ID::Model::MODEL_CHAIN )
			linkedMolecule = static_cast<const App::Old::Component::Chemistry::Chain &>( p_model ).getMoleculePtr();
		else if ( modelTypeID == App::Old::ID::Model::MODEL_RESIDUE )
			linkedMolecule = static_cast<const App::Old::Component::Chemistry::Residue &>( p_model ).getMoleculePtr();
		else if ( modelTypeID == App::Old::ID::Model::MODEL_ATOM )
			linkedMolecule = static_cast<const App::Old::Component::Chemistry::Atom &>( p_model ).getMoleculePtr();
	...
	std::vector<App::Old::Core::Model::ID> MoleculeSceneView::getAllItemsFrom(
		const App::Old::Core::Model::BaseModel & p_model ) const
	{
		...
		if ( typeID == App::Old::ID::Model::MODEL_CATEGORY )
		{
			const App::Old::Component::Chemistry::Category & category
				= static_cast<const App::Old::Component::Chemistry::Category &>( p_model );
			res.emplace_back( category.getId() );
			_selectAllCategoriesFrom( res, category );
		}
		else if ( typeID == App::Old::ID::Model::MODEL_CHAIN )
		{
			const App::Old::Component::Chemistry::Chain & chain
				= static_cast<const App::Old::Component::Chemistry::Chain &>( p_model );
			res.emplace_back( chain.getId() );
			_selectAllChainsFrom( res, chain );
		}
		else if ( typeID == App::Old::ID::Model::MODEL_RESIDUE )
		{
			const App::Old::Component::Chemistry::Residue & residue
				= static_cast<const App::Old::Component::Chemistry::Residue &>( p_model );
			res.emplace_back( residue.getId() );
			_selectAllResiduesFrom( res, residue );
			_selectAllChainsFrom( res, *residue.getChainPtr() );
		}
		else if ( typeID == App::Old::ID::Model::MODEL_ATOM )
		{
			const App::Old::Component::Chemistry::Atom & atom
				= static_cast<const App::Old::Component::Chemistry::Atom &>( p_model );
	...
	std::vector<App::Old::Core::Model::ID> MoleculeSceneView::getAllItemsTo(
		const App::Old::Core::Model::BaseModel & p_model ) const
	{
		const App::Old::VTX_ID & typeID = p_model.getTypeId();
		...
		if ( p_column == 0 )
		{
			if ( p_item == _getMoleculeTreeWidgetItem() )
			{
				...
				if ( itemTxt != _model->getDisplayName() )
				{
					...
					{
						VTX_ACTION( new App::Old::Action::Molecule::Rename( *_model, itemTxt ) );
					}
	...
	{
		const App::Old::Core::Model::ID & modelId	 = _getModelIDFromItem( *p_item );
	...
	void MoleculeSceneView::_onItemExpanded( QTreeWidgetItem * const p_item )
	{
		...
		_refreshSelection( App::Old::Application::Selection::SelectionManager::get().getSelectionModel() );
	...
	void MoleculeSceneView::_onCustomContextMenuCalled( const QPoint & p_clicPos )
	{
		...
		const App::Old::Core::Model::ID & modelId	   = _getModelIDFromItem( *targetedItem );
		const App::Old::VTX_ID &			 modelTypeId   = VTX::MVC_MANAGER().getModelTypeID( modelId );
		...
		{
			App::Old::Component::Chemistry::Category & category
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Category>( modelId );
		...
		{
			App::Old::Component::Chemistry::Chain & chain
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Chain>( modelId );
		...
		{
			App::Old::Component::Chemistry::Residue & residue
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Residue>( modelId );
		...
		{
			App::Old::Component::Chemistry::Atom & atom
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Atom>( modelId );
	...
	void MoleculeSceneView::_updateCategoryStructure( const App::Old::Component::Chemistry::Category & p_category,
													  QTreeWidgetItem &							  p_item )
	{
		const App::Old::Component::Chemistry::Molecule & molecule	 = *p_category.getMoleculePtr();
		...
		while ( currentChildIndex < p_item.childCount() )
		{
			...
			const App::Old::Core::Model::ID &				   chainItemID = _getModelIDFromItem( *currentChainItem );
			const App::Old::Component::Chemistry::Chain * const chain	   = _model->getChain( chainIndex );
	...
	void MoleculeSceneView::_updateChainStructure( const App::Old::Component::Chemistry::Chain & p_chain,
												   QTreeWidgetItem &						p_item )
	{
		const App::Old::Component::Chemistry::Molecule & molecule			= *p_chain.getMoleculePtr();
		...
		while ( currentChildIndex < p_item.childCount() )
		{
			...
			const App::Old::Core::Model::ID &					 residueItemID = _getModelIDFromItem( *currentResidueItem );
	...
	void MoleculeSceneView::_updateResidueStructure( const App::Old::Component::Chemistry::Residue & p_residue,
													 QTreeWidgetItem &							p_item )
	{
		App::Old::Component::Chemistry::Molecule & molecule			= *p_residue.getMoleculePtr();
		...
		while ( currentChildIndex < p_item.childCount() )
		{
			...
			const App::Old::Core::Model::ID &				  atomItemID = _getModelIDFromItem( *currentAtomItem );
			const App::Old::Component::Chemistry::Atom * const atom
	...
	{
		const App::Old::VTX_ID & modelTypeId = VTX::MVC_MANAGER().getModelTypeID( _getModelIDFromItem( *p_from ) );
	...
	void MoleculeSceneView::_expandMolecule( QTreeWidgetItem * const p_moleculeItem, const bool p_forceExpandChildren )
	{
		...
		else
		{
			...
			const std::vector<App::Old::Component::Chemistry::Category *> categories	   = _model->getFilledCategories();
		...
		for ( QTreeWidgetItem * const item : *itemsPtr )
		{
			...
			const App::Old::Core::Model::ID &				categoryID = _getModelIDFromItem( *item );
			const App::Old::Component::Chemistry::Category & category
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Category>( categoryID );
	...
	void MoleculeSceneView::_expandCategory( QTreeWidgetItem * const p_categoryItem, const bool p_forceExpandChildren )
	{
		...
		else
		{
			...
			const App::Old::Core::Model::ID &				categoryId = _getModelIDFromItem( *p_categoryItem );
			const App::Old::Component::Chemistry::Category & category
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Category>( categoryId );
			...
			{
				const App::Old::Component::Chemistry::Chain * const chainPtr	 = _model->getChain( chainIndex );
		...
		for ( QTreeWidgetItem * const item : *itemsPtr )
		{
			...
			const App::Old::Core::Model::ID &			 chainID = _getModelIDFromItem( *item );
			const App::Old::Component::Chemistry::Chain & chain
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Chain>( chainID );
	...
	void MoleculeSceneView::_expandChain( QTreeWidgetItem * const p_chainItem, const bool p_forceExpandChildren )
	{
		...
		else
		{
			...
			const App::Old::Core::Model::ID &			 chainId = _getModelIDFromItem( *p_chainItem );
			const App::Old::Component::Chemistry::Chain & chain
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Chain>( chainId );
			...
			{
				const App::Old::Component::Chemistry::Residue * const residuePtr	 = _model->getResidue( i );
		...
		for ( QTreeWidgetItem * const item : *itemsPtr )
		{
			...
			const App::Old::Core::Model::ID &			   residueID = _getModelIDFromItem( *item );
			const App::Old::Component::Chemistry::Residue & residue
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Residue>( residueID );
	...
	void MoleculeSceneView::_expandResidue( QTreeWidgetItem * const p_residueItem, const bool p_forceExpandChildren )
	{
		...
		else
		{
			...
			const App::Old::Core::Model::ID &			   residueId = _getModelIDFromItem( *p_residueItem );
			const App::Old::Component::Chemistry::Residue & residue
				= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Residue>( residueId );
			...
			{
				const App::Old::Component::Chemistry::Atom * const atomPtr  = _model->getAtom( i );
		...
		for ( QTreeWidgetItem * const item : *itemsPtr )
		{
			...
			const App::Old::Core::Model::ID &			atomID = _getModelIDFromItem( *item );
			const App::Old::Component::Chemistry::Atom & atom
	...
	void MoleculeSceneView::_applyMoleculeDataOnItem( const App::Old::Component::Chemistry::Molecule & p_molecule,
													  QTreeWidgetItem &							  p_item ) const
	{
		p_item.setData( 0, MODEL_ID_ROLE, QVariant::fromValue<VTX::App::Old::Core::Model::ID>( p_molecule.getId() ) );
	...
	void MoleculeSceneView::_applyCategoryDataOnItem( const App::Old::Component::Chemistry::Category & p_category,
													  QTreeWidgetItem &							  p_item ) const
	{
	...
	void MoleculeSceneView::_applyChainDataOnItem( const App::Old::Component::Chemistry::Chain & p_chain,
												   QTreeWidgetItem &						p_item,
	...
	void MoleculeSceneView::_applyResidueDataOnItem( const App::Old::Component::Chemistry::Residue & p_residue,
													 QTreeWidgetItem &							p_item ) const
	{
	...
	void MoleculeSceneView::_applyAtomDataOnItem( const App::Old::Component::Chemistry::Atom & p_atom,
												  QTreeWidgetItem &						  p_item ) const
	{
		...
		const VTX::Core::ChemDB::Residue::SYMBOL_DISPLAY_MODE & p_symbolDisplayMode ) const
	{
		...
		default:
			VTX_WARNING( "Symbol style {} not managed in MoleculeSceneView::_applyResidueNameOnItem.",
						 int( p_symbolDisplayMode ) );
	...
	void MoleculeSceneView::_doEnableStateChangeAction( const QTreeWidgetItem * const p_item ) const
	{
		...
		const App::Old::Action::VISIBILITY_MODE visibilityMode
			= modelEnabled ? App::Old::Action::VISIBILITY_MODE::SHOW : App::Old::Action::VISIBILITY_MODE::HIDE;
		...
		if ( modelTypeId == App::Old::ID::Model::MODEL_MOLECULE )
		{
			...
			if ( selection.isMoleculeFullySelected( model ) )
				VTX_ACTION(
					new App::Old::Action::Selection::ChangeVisibility( selection, model, modelTypeId, visibilityMode ) );
			else
				VTX_ACTION( new App::Old::Action::Molecule::ChangeVisibility( model, visibilityMode ) );
		}
		else if ( modelTypeId == App::Old::ID::Model::MODEL_CATEGORY )
		{
			App::Old::Component::Chemistry::Category & model
			...
			if ( selection.isCategoryFullySelected( model ) )
				...
				VTX_ACTION( new App::Old::Action::Category::ChangeVisibility( model, visibilityMode ) );
		}
		else if ( modelTypeId == App::Old::ID::Model::MODEL_CHAIN )
		{
			App::Old::Component::Chemistry::Chain & model
			...
			if ( selection.isChainFullySelected( model ) )
				...
				VTX_ACTION( new App::Old::Action::Chain::ChangeVisibility( model, visibilityMode ) );
		}
		else if ( modelTypeId == App::Old::ID::Model::MODEL_RESIDUE )
		{
			App::Old::Component::Chemistry::Residue & model
			...
			if ( selection.isResidueFullySelected( model ) )
				...
				VTX_ACTION( new App::Old::Action::Residue::ChangeVisibility( model, visibilityMode ) );
		}
		else if ( modelTypeId == App::Old::ID::Model::MODEL_ATOM )
		{
			App::Old::Component::Chemistry::Atom & model
			...
			if ( selection.isAtomSelected( model ) )
				...
				VTX_ACTION( new App::Old::Action::Atom::ChangeVisibility( model, visibilityMode ) );
		}
	}
	void MoleculeSceneView::_reformatMoleculeName( std::string & p_moleculeName ) const
	{
		...
		// Clamp size
		if ( p_moleculeName.size() > VTX::UI::Style::MOLECULE_NAME_MAXIMUM_SIZE )
			p_moleculeName = p_moleculeName.substr( 0, VTX::UI::Style::MOLECULE_NAME_MAXIMUM_SIZE );
	}

	void MoleculeSceneView::_fillItemSelection( const App::Old::Application::Selection::SelectionModel & p_selection,
												QItemSelection &									p_itemSelection )
	{
		const App::Old::Application::Selection::SelectionModel::MapMoleculeIds & items = p_selection.getMoleculesMap();
		const App::Old::Application::Selection::SelectionModel::MapMoleculeIds::const_iterator itMoleculeItem
		...
		if ( itMoleculeItem != items.end() )
		{
			...
			{
				const App::Old::Component::Chemistry::Category * topCategory				= nullptr;
				const App::Old::Component::Chemistry::Category * previousCategory		= nullptr;
				QModelIndex									topCategoryItemIndex	= QModelIndex();
				QModelIndex									bottomCategoryItemIndex = QModelIndex();

				for ( const App::Old::Component::Chemistry::Category * const category : _model->getCategories() )
				{
					if ( category->isEmpty() )
					{
						continue;
					...
					const App::Old::Component::Chemistry::Chain * topChain			  = nullptr;
					const App::Old::Component::Chemistry::Chain * previousChain		  = nullptr;
					QModelIndex								 topChainItemIndex	  = QModelIndex();
					QModelIndex								 bottomChainItemIndex = QModelIndex();

					for ( const App::Old::Application::Selection::SelectionModel::PairChainIds & pairChain :
						  itMoleculeItem->second )
					{
						const App::Old::Component::Chemistry::Chain & chain = *_model->getChain( pairChain.first );
						...
						const App::Old::Component::Chemistry::Residue * topResidue			  = nullptr;
						const App::Old::Component::Chemistry::Residue * previousResidue		  = nullptr;
						QModelIndex								   topResidueItemIndex	  = QModelIndex();
						QModelIndex								   bottomResidueItemIndex = QModelIndex();
						for ( const App::Old::Application::Selection::SelectionModel::PairResidueIds & pairResidue :
							  pairChain.second )
						{
							const App::Old::Component::Chemistry::Residue & residue
							...
							const App::Old::Component::Chemistry::Atom * topAtom				= nullptr;
							const App::Old::Component::Chemistry::Atom * previousAtom		= nullptr;
							...
							{
								const App::Old::Component::Chemistry::Atom & atom = *_model->getAtom( atomId );
		...
		const VTX::Core::ChemDB::Residue::SYMBOL_DISPLAY_MODE & p_displayMode )
	{
		...
		if ( modelTypeId == App::Old::ID::Model::MODEL_CHAIN )
		{
			for ( int i = 0; i < p_item->childCount(); i++ )
			{
				QTreeWidgetItem * const					   child   = p_item->child( i );
				const App::Old::Core::Model::ID &			   modelId = _getModelIDFromItem( *child );
				const App::Old::Component::Chemistry::Residue & residue
					= VTX::MVC_MANAGER().getModel<App::Old::Component::Chemistry::Residue>( modelId );
	...
	QTreeWidgetItem * const MoleculeSceneView::_getTreeWidgetItem(
		const App::Old::Component::Chemistry::Category & p_category ) const
	{
		return topLevelItem( 0 )->child( int( p_category.getCategoryEnum() ) );
	}
	QTreeWidgetItem * const MoleculeSceneView::_getTreeWidgetItem(
		const App::Old::Component::Chemistry::Chain & p_chain ) const
	{
		const App::Old::Component::Chemistry::Category * const category = _model->getCategoryFromChain( p_chain );
		const int childIndex									   = category->getChainInnerIndex( p_chain.getIndex() );

		return _getTreeWidgetItem( *category )->child( childIndex );
	}
	QTreeWidgetItem * const MoleculeSceneView::_getTreeWidgetItem(
		const App::Old::Component::Chemistry::Residue & p_residue ) const
	{
		const App::Old::Component::Chemistry::Chain * const chain = p_residue.getChainPtr();
		const uint childIndex								 = p_residue.getIndex() - chain->getIndexFirstResidue();

		return _getTreeWidgetItem( *chain )->child( childIndex );
	}
	QTreeWidgetItem * const MoleculeSceneView::_getTreeWidgetItem(
		const App::Old::Component::Chemistry::Atom & p_atom ) const
	{
		const App::Old::Component::Chemistry::Residue * const residue	= p_atom.getResiduePtr();
	...
	bool MoleculeSceneView::_isCategoryExpanded( const App::Old::Component::Chemistry::Category & p_category ) const
	{
		return _isMoleculeExpanded() && _getTreeWidgetItem( p_category )->childCount() > 0;
	}
	bool MoleculeSceneView::_isChainExpanded( const App::Old::Component::Chemistry::Chain & p_chain ) const
	{
		const App::Old::Component::Chemistry::Category * const category = _model->getCategoryFromChain( p_chain );
		return _isCategoryExpanded( *category ) && _getTreeWidgetItem( p_chain )->childCount() > 0;
	}
	bool MoleculeSceneView::_isResidueExpanded( const App::Old::Component::Chemistry::Residue & p_residue ) const
	{
		const App::Old::Component::Chemistry::Chain * const chain = p_residue.getChainPtr();
	...
	VTX::Core::ChemDB::Category::TYPE MoleculeSceneView::_getCategoryFromItem( const QTreeWidgetItem & p_item ) const
	{
		const QVariant & dataID = p_item.data( 0, CATEGORY_ROLE );
		return VTX::Core::ChemDB::Category::TYPE( dataID.value<int>() );
	}

	void MoleculeSceneView::_selectAllCategoriesFrom( std::vector<App::Old::Core::Model::ID> &		  p_selection,
													  const App::Old::Component::Chemistry::Category & p_itemFrom ) const
	{
		const std::vector<App::Old::Component::Chemistry::Category *> & categories = _model->getCategories();

		for ( int iCategory = int( p_itemFrom.getCategoryEnum() ) + 1; iCategory < categories.size(); iCategory++ )
		{
			p_selection.emplace_back( _model->getCategory( VTX::Core::ChemDB::Category::TYPE( iCategory ) ).getId() );
		}
	}
	void MoleculeSceneView::_selectAllChainsFrom( std::vector<App::Old::Core::Model::ID> &	   p_selection,
												  const App::Old::Component::Chemistry::Chain & p_itemFrom ) const
	{
		for ( uint iChain = p_itemFrom.getIndex() + 1; iChain < _model->getChainCount(); iChain++ )
		{
			const App::Old::Component::Chemistry::Chain * const chain = _model->getChain( iChain );
			if ( chain != nullptr )
				p_selection.emplace_back( chain->getId() );
		}
	}
	void MoleculeSceneView::_selectAllResiduesFrom( std::vector<App::Old::Core::Model::ID> &		   p_selection,
													const App::Old::Component::Chemistry::Residue & p_itemFrom ) const
	{
		const App::Old::Component::Chemistry::Chain & chain = *( p_itemFrom.getChainPtr() );

		for ( uint iResidue = p_itemFrom.getIndex() + 1; iResidue <= chain.getIndexLastResidue(); iResidue++ )
		{
			const App::Old::Component::Chemistry::Residue * const residue = _model->getResidue( iResidue );
			if ( residue != nullptr )
				p_selection.emplace_back( residue->getId() );
		}
	}
	void MoleculeSceneView::_selectAllAtomsFrom( std::vector<App::Old::Core::Model::ID> &	 p_selection,
												 const App::Old::Component::Chemistry::Atom & p_itemFrom ) const
	{
		const App::Old::Component::Chemistry::Residue & residue = *( p_itemFrom.getResiduePtr() );

		for ( uint iAtom = p_itemFrom.getIndex() + 1; iAtom < residue.getIndexFirstAtom() + residue.getAtomCount();
			  iAtom++ )
		{
			const App::Old::Component::Chemistry::Atom * const atom = _model->getAtom( iAtom );
			if ( atom != nullptr )
				p_selection.emplace_back( atom->getId() );
		}
	}

	void MoleculeSceneView::_selectAllCategoriesTo( std::vector<App::Old::Core::Model::ID> &			p_selection,
													const App::Old::Component::Chemistry::Category & p_itemFrom ) const
	{
		const std::vector<App::Old::Component::Chemistry::Category *> & categories = _model->getCategories();
	...
	void MoleculeSceneView::_selectAllChainsTo( std::vector<App::Old::Core::Model::ID> &		 p_selection,
												const App::Old::Component::Chemistry::Chain & p_itemFrom ) const
	{
	...
	void MoleculeSceneView::_selectAllResiduesTo( std::vector<App::Old::Core::Model::ID> &		 p_selection,
												  const App::Old::Component::Chemistry::Residue & p_itemFrom ) const
	{
		const App::Old::Component::Chemistry::Chain & chain = *( p_itemFrom.getChainPtr() );
	...
	void MoleculeSceneView::_selectAllAtomsTo( std::vector<App::Old::Core::Model::ID> &	   p_selection,
											   const App::Old::Component::Chemistry::Atom & p_itemFrom ) const
	{
		const App::Old::Component::Chemistry::Residue & residue = *( p_itemFrom.getResiduePtr() );
