...
Q_DECLARE_METATYPE( VTX::App::Entity ) // used to allow QVariant conversion
...
namespace VTX::UI::QT::Dialog
{
	TrajectoryAssociation::TrajectoryAssociation( FilePath p_path ) : _path( std::move( p_path ) )
	{
		...
		_cbSystem->addItem( "New structure", QVariant::fromValue( VTX::App::Entity( entt::null ) ) );
		...
		connect(
			...
			[ this ]()
			{
				App::Entity entity = _cbSystem->currentData().value<App::Entity>();
				if ( entity == entt::null )
