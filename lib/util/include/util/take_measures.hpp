...
{
  public:
	TakeMeasures( const int p_measureID, const std::string & p_filePath, const int p_measureTime = 1 ) :
		_measureID( p_measureID ), _filePath( p_filePath ), _measureTime( p_measureTime )
	...
	void measure( float deltaTime, VTX::Renderer::InstructionsDurationRanges const & p_durations )
	{
		...
		Filesystem::writeStringToFile( _filePath, std::to_string( _duration.duration ) + "," );
